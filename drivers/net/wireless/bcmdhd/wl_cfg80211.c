 ...
 *
 * $Id: wl_cfg80211.c 671237 2016-11-21 07:45:18Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
/* JHKIM 2016.08.16
   enable scan debug info temporary by changing WL_SCAN to WL_ERR
*/
	WL_ERR(("Preparing Scan request\n"));
	WL_SCAN(("nprobes=%d\n", params->nprobes));
...
#define SCAN_EBUSY_RETRY_LIMIT 20
	if (err == -EBUSY) {
		if (busy_count++ > SCAN_EBUSY_RETRY_LIMIT) {
			struct ether_addr bssid;
			s32 ret = 0;
#if defined(DHD_DEBUG) && (defined(BCMPCIE) || defined(BCMSDIO)) && \
	...
	enum nl80211_iftype iface_type, s32 del, u8 *addr)
{
	...
	if (iface_type == NL80211_IFTYPE_AP) {
		iface.iftype = WL_INTERFACE_TYPE_AP;
		iface.flags = WL_INTERFACE_CREATE_AP;
	} else {
		iface.iftype = WL_INTERFACE_TYPE_STA;
		iface.flags = WL_INTERFACE_CREATE_STA;
	}

	if (del) {
		ret = wldev_iovar_setbuf(ndev, "interface_remove",
			NULL, 0, cfg->ioctl_buf, WLC_IOCTL_MEDLEN, &cfg->ioctl_buf_sync);
	} else {
		if (addr) {
...
s32
wl_cfg80211_bss_up(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	...
	u8 *mac_addr, const char *name)
{
	...
	mode = (iface_type == NL80211_IFTYPE_STATION) ?
		WL_MODE_BSS : WL_MODE_AP;
	if (mode == WL_MODE_AP) {
	...
	wl_config_ifmode(cfg, new_ndev, iface_type);
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (cfg->wdev->wiphy->features & NL80211_FEATURE_FW_4WAY_HANDSHAKE) {
		if ((pval == AES_ENABLED) && (dev == bcmcfg_to_prmry_ndev(cfg)))
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (chan) {
		...
		if (cfg->rcc_enabled) {
			 ...
			 * to using a single channel based join. But RCC would help choose
			 * a better AP for a given ssid. For a given SSID there might multiple
			 * APs on different channels and RCC would scan all those channels
		...
		*/
		...
		if (wl_get_drv_status(cfg, CONNECTING, dev) ||
			wl_get_drv_status(cfg, CONNECTED, dev)) {
				...
				/* Wait for 200ms to receive the deauth frame */
				OSL_SLEEP(200);
		}
	}
#ifdef CUSTOM_SET_CPUCORE
...
#ifdef APSTA_DISABLE_FOR_SOFTAP
	uint32 apsta = 0;
#endif /* APSTA_DISABLE_FOR_SOFTAP */
	s32 err = BCME_OK;
...
#endif /* SOFTAP_UAPSD_OFF */
#ifdef APSTA_DISABLE_FOR_SOFTAP
		err = wldev_iovar_setbuf_bsscfg(dev, "apsta", &apsta, sizeof(apsta),
			cfg->ioctl_buf, WLC_IOCTL_SMLEN, bssidx, &cfg->ioctl_buf_sync);
		if (err < 0) {
			WL_ERR(("failed to disable apsta, error=%d\n", err));
		}
#endif /* APSTA_DISABLE_FOR_SOFTAP */
...
#ifdef SEND_DIASSOC_FOR_EXPECTED_ROAM
			else {
				/* Send diassoc to abort unexpencted roam */
				scb_val_t scbval;
				if (event == WLC_E_DEAUTH_IND &&
					ndev == bcmcfg_to_prmry_ndev(cfg)) {
					WL_ERR(("link down from" MACDBG ": send diassoc\n",
						MAC2STRDBG((const u8*)(&e->addr))));
...
#endif /* SEND_DIASSOC_FOR_EXPECTED_ROAM */
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (n_pfn_results > 0) {
		...
		pnetinfo = (wl_pfn_net_info_t *)((char *)data + sizeof(wl_pfn_scanresults_t)
...
#ifdef WL_NAN
	wl_cfgnan_stop_handler(ndev, wl_get_cfg(ndev), NULL, 0, NULL);
#endif /* WL_NAN */
...
{
	return wl_cfgnan_cmd_handler(ndev, wl_get_cfg(ndev), cmd, cmd_len);
}
#endif /* WL_NAN */

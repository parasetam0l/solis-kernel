 ...
 *
 * $Id: dhd_common.c 663716 2016-10-06 18:38:10Z $
 */
#include <typedefs.h>
#include <osl.h>
...
/* For CUSTOMER_HW4 with DHD_LOG_DUMP not enabled, do not enable DHD_EVENT_VAL */
		...
		if (ioc->cmd == WLC_GET_VAR || ioc->cmd == WLC_SET_VAR) {
			...
			if (ioc->buf != NULL) {
				...
				if (ioc->cmd != WLC_GET_MAGIC && ioc->cmd != WLC_GET_VERSION)
					DHD_ERROR_EX(("WLC_IOCTL: cmd: %d, val: %d(%s), "
						"len: %d, set: %d\n",
            ...
            void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_SCAN_TO): {
		dhd_get_scan_to_val(dhd_pub, &int_val);
		bcopy(&int_val, arg, val_size);
		break;
	}
	case IOV_SVAL(IOV_SCAN_TO): {
		dhd_set_scan_to_val(dhd_pub, int_val);
		break;
	}
	case IOV_GVAL(IOV_JOIN_TO): {
		dhd_get_join_to_val(dhd_pub, &int_val);
		bcopy(&int_val, arg, val_size);
		break;
	}
	case IOV_SVAL(IOV_JOIN_TO): {
		dhd_set_join_to_val(dhd_pub, int_val);
		break;
	}
	case IOV_GVAL(IOV_CMD_TO): {
		dhd_get_cmd_to_val(dhd_pub, &int_val);
		bcopy(&int_val, arg, val_size);
		break;
	}
	case IOV_SVAL(IOV_CMD_TO): {
		dhd_set_cmd_to_val(dhd_pub, int_val);
		break;
	}
	case IOV_GVAL(IOV_OQS_TO): {
		dhd_get_bus_to_val(dhd_pub, &int_val);
		bcopy(&int_val, arg, val_size);
		break;
	}
	case IOV_SVAL(IOV_OQS_TO): {
		dhd_set_bus_to_val(dhd_pub, int_val);
		break;
	}
#endif /* REPORT_FATAL_TIMEOUTS */
	...
	case IOV_GVAL(IOV_BPADDR):
		{
			...
			bcmerror = dhd_bus_readwrite_bp_addr(dhd_pub, addr, size, &int_val, TRUE);
	...
	case IOV_SVAL(IOV_BPADDR):
		{
			...
			bcmerror = dhd_bus_readwrite_bp_addr(dhd_pub, addr, size, &sdreg.value,
				FALSE);
	...
	void *raw_event_ptr, char *eventmask)
{
	...
	case WLC_E_ACTION_FRAME_COMPLETE:
		DHD_EVENT(("MACEVENT: %s %d, MAC %s, status %d, reason %d, auth %d\n",
		       event_name, event_type, "no info", (int)status, (int)reason,
		       (int)auth_type));
		break;
...
dhd_get_clminfo(dhd_pub_t *dhd, char *clm_path)
{
	...
	if ((len > 0) && (len < MAX_CLMINFO_BUF_SIZE) && memblock) {
		...
		/* If platform is SLP or tizen, clm_path overwrite the default path */
		strncpy(clm_path, CONFIG_BCMDHD_CLM_PATH, strlen(CONFIG_BCMDHD_CLM_PATH));
...
{
	/* first stop the join, scan and bus ioctl timers
	* as failing to do so may cause a kernel panic if
	* an rmmod is done before ioctl completion happens
	*/
	dhd_stop_join_timer(pub);
	dhd_stop_scan_timer(pub);
	dhd_stop_bus_timer(pub);
...
dhd_cmd_timeout(void *ctx)
{
	...
	if (pub->timeout_info && pub->timeout_info->cmd_timer_active) {
		...
		dhd_bus_set_device_wake(pub->bus, TRUE);
#endif /* PCIE_OOB */
		dhd_stop_cmd_timer(pub);
...
dhd_start_cmd_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->cmd_timeout_val == 0) {
		...
		DHD_ERROR(("DHD: Command Timeout Disabled\n"));
		goto exit;
	}
...
dhd_stop_cmd_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->cmd_timer_active) {
		...
		DHD_ERROR(("DHD: CMD timer is not active\n"));
		ret = BCME_ERROR;
	}
	if (ret == BCME_OK) {
...
__dhd_stop_join_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->join_timer_active) {
		...
		DHD_ERROR(("DHD: JOIN timer is not active\n"));
		ret = BCME_ERROR;
	}
	if (ret == BCME_OK) {
...
dhd_join_timeout(void *ctx)
{
	...
	if (pub->timeout_info && pub->timeout_info->join_timer_active) {
...
dhd_start_join_timer(dhd_pub_t *pub)
{
	int ret = BCME_OK;
	unsigned long flags;
	uint32 join_to_ms = pub->timeout_info->join_timeout_val;
	...
	if (pub->timeout_info->join_timeout_val == 0) {
		...
		DHD_ERROR(("DHD: Join Timeout Disabled\n"));
	} else {
...
dhd_start_scan_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->scan_timeout_val == 0) {
		...
		DHD_ERROR(("DHD: Scan Timeout Disabled\n"));
	} else {
...
dhd_stop_scan_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->scan_timer_active) {
		...
		DHD_ERROR(("DHD: SCAN timer is not active\n"));
		ret = BCME_ERROR;
	}
...
dhd_start_bus_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->bus_timeout_val == 0) {
		...
		DHD_ERROR(("DHD: Bus Timeout Disabled\n"));
		goto exit;
	}
...
dhd_stop_bus_timer(dhd_pub_t *pub)
{
	...
	if (pub->timeout_info->bus_timer_active) {
		...
		DHD_ERROR(("DHD: BUS timer is not active\n"));
		ret = BCME_ERROR;
	}
	if (ret == BCME_OK) {

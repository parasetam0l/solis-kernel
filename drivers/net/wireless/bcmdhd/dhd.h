 ...
 *
 * $Id: dhd.h 668434 2016-11-03 07:44:38Z $
 */
...
#ifdef SHOW_LOGTRACE
extern void dhd_os_trace_log_lock(dhd_pub_t *pub);
extern void dhd_os_trace_log_unlock(dhd_pub_t *pub);
...
extern void dhd_log_dbg_info_data(dhd_pub_t *dhdp, void *data, uint16 length);
...
extern int dhd_check_module_bcm4361(char *module_type, int index);
...
#ifdef SHOW_LOGTRACE
#define MAX_TRACE_BUF_COUNT 500
#define TRACE_LOG_MAGIC_NUMBER	0xEAE47C06
typedef struct file_trace_header {
	int magic_num;
	int buf_size;
	int seq_num;
} file_trace_header_t;
...
typedef struct trace_log_info {
	int buf_count; /* Number of buffers allocated. */
	int last_read_done; /* Last log buffer is already read */
	trace_node_t *head; /* Start of link list */
	trace_node_t *last_node; /* Last node in the link list */
	trace_node_t *read_node; /* Node in the link list from where read will start */
	spinlock_t trace_log_lock; /* Till date, this lock is used only in LINUX */
} trace_log_info_t;
#endif /* SHOW_LOGTRACE */
...
#endif /* !DHD_LB_STATS */
#ifdef SHOW_LOGTRACE
int dhd_log_trace(dhd_pub_t *dhd_pub, void *buf, int buf_size, int seq_num);
int dhd_get_read_buf_ptr(dhd_pub_t *dhd_pub, trace_buf_info_t *read_buf_info);
#endif /* SHOW_LOGTRACE */
...
#ifdef PCIE_OOB
extern void dhd_bus_set_device_wake(struct dhd_bus *bus, bool val);
#endif /* PCIE_OOB */
#endif /* _dhd_h_ */

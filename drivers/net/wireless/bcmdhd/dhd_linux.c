 ...
 *
 * $Id: dhd_linux.c 671287 2016-11-21 12:38:31Z $
 */
...
struct dhd_log_dump_buf g_dld_buf;
...
#else
uint dhd_arp_mode = ARP_OL_AGENT | ARP_OL_PEER_AUTO_REPLY | ARP_OL_HOST_AUTO_REPLY;
	...
	struct bcmstrbuf *strbuf, uint32 **hist)
{
	...
	per_cpu_total = (uint32 *)kmalloc(sizeof(uint32) * num_cpus, GFP_KERNEL);
	if (!per_cpu_total) {
...
{
#if !defined(SUPPORT_PM2_ONLY) || defined(PLATFORM_SLP)
	int power_mode = PM_MAX;
#endif /* SUPPORT_PM2_ONLY || PLATFORM_SLP */
...
#define CUSTOM_INTR_WIDTH 100
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
				...
				dhd_wl_ioctl_cmd(dhd, WLC_GET_PM, (char *)&power_mode,
					sizeof(power_mode), FALSE, 0);

				if (power_mode != PM_FAST) {
					power_mode = PM_FAST;
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (DHD_BUS_CHECK_SUSPEND_OR_SUSPEND_IN_PROGRESS(&dhd->pub)) {
		DHD_TRACE(("%s: bus is in suspend(%d) or suspending(0x%x) state!!\n",
...
static void
dhd_event_logtrace_process(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
	dhd_pub_t *dhdp = (dhd_pub_t *)event_info;
	struct sk_buff *skb;

	if (event != DHD_WQ_WORK_EVENT_LOGTRACE) {
		DHD_ERROR(("%s: unexpected event \n", __FUNCTION__));
		return;
	}
...
dhd_event_logtrace_enqueue(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	/* Schedule workqueue to process event trace logs */
	dhd_deferred_schedule_work(dhd->dhd_deferred_wq, (void *)dhdp,
		DHD_WQ_WORK_EVENT_LOGTRACE, dhd_event_logtrace_process,
...
#endif /* DHD_RX_DUMP || DHD_8021X_DUMP || DHD_DHCP_DUMP || DHD_WAKE_STATUS */
#ifdef DHD_MCAST_REGEN
...
dhd_open(struct net_device *net)
{
	...
	if (!dhd_download_fw_on_driverload &&
		...
		if (ret == BCME_OK) {
			dhd_init_static_strs_array(dhd->pub.osh, &dhd->event_data, st_str_file_path,
				map_file_path);
...
#endif /* DHD_LB */
#ifdef SHOW_LOGTRACE
	dhd->trace_log = MALLOC(osh, sizeof(trace_log_info_t));
	if (dhd->trace_log == NULL) {
		DHD_ERROR(("%s: Failed to alloc trace_log\n", __FUNCTION__));
	} else {
		memset(dhd->trace_log, 0, sizeof(trace_log_info_t));
		dhd->trace_log->buf_count = 0;
		dhd->trace_log->last_read_done = 0;
		dhd->trace_log->head = NULL;
		dhd->trace_log->last_node = NULL;
		dhd->trace_log->read_node = NULL;
		spin_lock_init(&dhd->trace_log->trace_log_lock);
	}
#endif /* SHOW_LOGTRACE */
#ifdef REPORT_FATAL_TIMEOUTS
...
void dhd_detach(dhd_pub_t *dhdp)
{
	...
	skb_queue_purge(&dhd->evt_trace_queue);
...
#endif /* DHDTCPACK_SUPPRESS */
#ifdef SHOW_LOGTRACE
	/* Free all the allocated trace buffers here */
	if (dhd->trace_log) {
		/* It can be a circular link list. So always
		 * preserve head to compare it with last nodes->next
		 */
		trace_node_t* curr_node;
		trace_node_t* next_node;
		curr_node = dhd->trace_log->head;
		next_node = dhd->trace_log->head;

		while (curr_node != NULL) {
			next_node = curr_node->next;
			curr_node->next = NULL;
			if (curr_node->info_buf && curr_node->size) {
				MFREE(dhdp->osh, curr_node->info_buf, curr_node->size);
				curr_node->info_buf = NULL;
				curr_node->size = 0;
			}
			MFREE(dhdp->osh, curr_node, sizeof(trace_node_t));
			if (next_node == dhd->trace_log->head) {
				/* Its wrap around of link list.
				 * last nodes->next is pointing to the head
		...
		MFREE(dhdp->osh, dhd->trace_log, sizeof(trace_log_info_t));
		dhd->trace_log = NULL;
	}
#endif /* SHOW_LOGTRACE */
...
deferred_module_init(dhd_module_init);
	...
	* CONFIG_ARCH_MSM8996 || CONFIG_SOC_EXYNOS8895
	*/
...
#ifdef SHOW_LOGTRACE
void
dhd_os_trace_log_lock(dhd_pub_t *pub)
{
	dhd_info_t *dhd;
	dhd = (dhd_info_t *)(pub->info);
	spin_lock_bh(&dhd->trace_log->trace_log_lock);
}

void
dhd_os_trace_log_unlock(dhd_pub_t *pub)
{
	dhd_info_t *dhd;
	dhd = (dhd_info_t *)(pub->info);
	spin_unlock_bh(&dhd->trace_log->trace_log_lock);
}
#endif /* SHOW_LOGTRACE */
...
#if !defined(CUSTOM_DUMP_FILE_LIMIT)
...
#ifdef CUSTOMER_HW4_DEBUG
#ifdef CUSTOM_DUMP_FILE_LIMIT
	snprintf(memdump_path, sizeof(memdump_path), "%s.%d",
		DHD_COMMON_DUMP_PATH "mem_dump", dhd->memdump_index);
	if( ++dhd->memdump_index >= CUSTOM_DUMP_FILE_MAX_CNT)
		dhd->memdump_index = 0;
#else
...
#if defined(BOARD_PANDA)
		/* For android on panda board, only delay >= 6 sec works for safely taking
		 * mem_dump
		 */
		OSL_DELAY(7000000);
#endif /* OEM_ANDROID && BOARD_PANDA */
		BUG_ON(1);
	}
...
do_dhd_log_dump(dhd_pub_t *dhdp)
{
...
#ifdef CUSTOM_DUMP_FILE_LIMIT
	snprintf(dump_path, sizeof(dump_path), "%s.%d",
		DHD_COMMON_DUMP_PATH "debug_dump", dhdp->memdump_index);
#else
	do_gettimeofday(&curtime);
	...
	if (g_dld_buf.wraparound) {
		size = DHD_LOG_DUMP_BUFFER_SIZE;
	} else {
		size = (unsigned int)(g_dld_buf.present - g_dld_buf.front);
	}

	ret = vfs_write(fp, g_dld_buf.buffer, size, &pos);
	if (ret < 0) {
		DHD_ERROR(("write file error, err = %d\n", ret));
		goto exit;
	}
	...
	spin_lock_irqsave(&g_dld_buf.lock, flags);
	g_dld_buf.wraparound = 0;
	g_dld_buf.present = g_dld_buf.front;
	g_dld_buf.remain = DHD_LOG_DUMP_BUFFER_SIZE;
	bzero(g_dld_buf.buffer, DHD_LOG_DUMP_BUFFER_SIZE);
	spin_unlock_irqrestore(&g_dld_buf.lock, flags);
	ret = BCME_OK;
...
void dhd_log_dbg_info_data(dhd_pub_t *dhdp, void *data, uint16 length)
{
#ifdef SHOW_LOGTRACE
	dhd_info_t *dhd = (dhd_info_t *) dhdp->info;
	dhd_dbg_trace_evnt_handler(dhdp, data, &dhd->event_data, length);
#endif
}
...
dhd_get_read_buf_ptr(dhd_pub_t *dhd_pub, trace_buf_info_t *trace_buf_info)
{
	...
	if (dhd->trace_log == NULL) {
		DHD_INFO(("Ecounters log pointer is NULL\n"));
		goto exit;
	}
	/* If we already received */
	if (dhd->trace_log->read_node == NULL) {
		DHD_INFO(("No log data available\n"));
		goto exit;
	}
	dhd_os_trace_log_lock(dhd_pub);
	if (dhd->trace_log->read_node == dhd->trace_log->last_node) {
		/* Dont send the same buffer again and again */
		if (dhd->trace_log->last_read_done != 1) {
			/* This condition will met when only one unread log bug is avail */
			trace_buf_info->availability = NEXT_BUF_NOT_AVAIL;
			trace_buf_info->size = (((file_trace_header_t *)
				(dhd->trace_log->read_node->info_buf))->buf_size) +
				sizeof(file_trace_header_t);
			memcpy(trace_buf_info->buf, dhd->trace_log->read_node->info_buf,
					trace_buf_info->size);
			dhd->trace_log->last_read_done = 1;
		} else {
			trace_buf_info->availability = BUF_NOT_AVAILABLE;
			trace_buf_info->size = 0;
		}
	} else {
		if (dhd->trace_log->last_read_done == 1) {
			/* If last_read_done is 1 then current node is already read */
			dhd->trace_log->read_node = dhd->trace_log->read_node->next;
			dhd->trace_log->last_read_done = 0;
		}
		trace_buf_info->size = (((file_trace_header_t *)
			(dhd->trace_log->read_node->info_buf))->buf_size) +
	...
	dhd_os_trace_log_unlock(dhd_pub);
	return BCME_OK;
exit:
	trace_buf_info->availability = BUF_NOT_AVAILABLE;
	trace_buf_info->size = 0;
...
int
dhd_log_trace(dhd_pub_t *dhd_pub, void *buf, int buf_size, int seq_num)
{
	trace_node_t *trace_node;
	void *info_buf;
	dhd_info_t *dhd = dhd_pub->info;
	int alloc_size = 0;
	file_trace_header_t file_header;

	/* Allocate extra memory for Magic No.,  and size parameter */
	alloc_size = buf_size + sizeof(file_trace_header_t);

	if (dhd->trace_log == NULL) {
		DHD_INFO(("Ecounters log pointer is NULL\n"));
		return BCME_ERROR;
	}
	if (dhd->trace_log->buf_count < MAX_TRACE_BUF_COUNT) {
		/* Allocate new node */
		trace_node = (trace_node_t *)MALLOC(dhd_pub->osh, sizeof(trace_node_t));
		if (trace_node == NULL) {
			DHD_ERROR(("ERROR:Ecounters node allocation failed\n"));
			goto exit;
		}
		/* Allocate new buffer */
		info_buf = MALLOC(dhd_pub->osh, alloc_size);
		if (info_buf == NULL) {
			DHD_ERROR(("ERROR:Info buf allocation failed\n"));
			MFREE(dhd_pub->osh, trace_node, sizeof(trace_node_t));
			goto exit;
		}
		memset(trace_node, 0, sizeof(trace_node_t));
		memset(info_buf, 0, alloc_size);
		dhd_os_trace_log_lock(dhd_pub);
		/* If we are entering this function first time initialise pointers */
		if (dhd->trace_log->head == NULL) {
			dhd->trace_log->head = trace_node;
			dhd->trace_log->read_node = trace_node;
			dhd->trace_log->last_node = trace_node;
		} else {
			dhd->trace_log->last_node->next = trace_node;
			dhd->trace_log->last_node = dhd->trace_log->last_node->next;
		}
		dhd->trace_log->last_node->info_buf = info_buf;
		dhd->trace_log->last_node->size = alloc_size;

		file_header.magic_num = TRACE_LOG_MAGIC_NUMBER;
		file_header.buf_size = buf_size;
		file_header.seq_num = seq_num;

		memcpy((char *)dhd->trace_log->last_node->info_buf,
				&file_header, sizeof(file_trace_header_t));
		memcpy((char *)dhd->trace_log->last_node->info_buf +
				sizeof(file_trace_header_t), buf, buf_size);
		dhd->trace_log->buf_count++;
		if (dhd->trace_log->buf_count == MAX_TRACE_BUF_COUNT) {
			/* Create a circular link list if all nodes are allocated. */
			dhd->trace_log->last_node->next = dhd->trace_log->head;
		}
		dhd_os_trace_log_unlock(dhd_pub);
	} else {
		dhd_os_trace_log_lock(dhd_pub);
		/* Read node should always point to the oldest data buffer which is not read */
		if (dhd->trace_log->last_node->next == dhd->trace_log->read_node) {
			dhd->trace_log->read_node = dhd->trace_log->read_node->next;
		}
		if (dhd->trace_log->last_node->next->size < alloc_size) {
			DHD_INFO(("Short Buffer allocating again\n"));
			MFREE(dhd_pub->osh, dhd->trace_log->last_node->next->info_buf,
					dhd->trace_log->last_node->next->size);
			dhd->trace_log->last_node->next->info_buf = NULL;
			dhd->trace_log->last_node->next->size = 0;
			/* Allocate new buffer */
			info_buf = MALLOC(dhd_pub->osh, alloc_size);
			if (info_buf == NULL) {
				DHD_ERROR(("ERROR:Info buf allocation failed\n"));
			...
			memset(info_buf, 0, alloc_size);
			dhd->trace_log->last_node->next->info_buf = info_buf;
			dhd->trace_log->last_node->next->size = alloc_size;
		}
		file_header.magic_num = TRACE_LOG_MAGIC_NUMBER;
		file_header.buf_size = buf_size;
		file_header.seq_num = seq_num;
...
{
	spin_lock_init(&g_dld_buf.lock);
#if defined(CONFIG_DHD_USE_STATIC_BUF) && defined(DHD_USE_STATIC_MEMDUMP)
	g_dld_buf.buffer = DHD_OS_PREALLOC(dhd,
		DHD_PREALLOC_DHD_LOG_DUMP_BUF, DHD_LOG_DUMP_BUFFER_SIZE);
#else
	g_dld_buf.buffer = kmalloc(DHD_LOG_DUMP_BUFFER_SIZE, GFP_KERNEL);
#endif /* CONFIG_DHD_USE_STATIC_BUF && DHD_USE_STATIC_MEMDUMP */
	...
	if (!g_dld_buf.buffer) {
		...
		if (!g_dld_buf.buffer) {
			DHD_ERROR(("Failed to allocate memory for g_dld_buf.\n"));
			return;
		}
	}

	g_dld_buf.wraparound = 0;
	g_dld_buf.max = (unsigned long)g_dld_buf.buffer + DHD_LOG_DUMP_BUFFER_SIZE;
	g_dld_buf.present = g_dld_buf.buffer;
	g_dld_buf.front = g_dld_buf.buffer;
	g_dld_buf.remain = DHD_LOG_DUMP_BUFFER_SIZE;
	g_dld_buf.enable = 1;
}
...
#if defined(CONFIG_DHD_USE_STATIC_BUF) && defined(DHD_USE_STATIC_MEMDUMP)
	DHD_OS_PREFREE(dhd,
		g_dld_buf.buffer, DHD_LOG_DUMP_BUFFER_SIZE);
#else
	kfree(g_dld_buf.buffer);
#endif /* CONFIG_DHD_USE_STATIC_BUF && DHD_USE_STATIC_MEMDUMP */
}
...
dhd_log_dump_print(const char *fmt, ...)
{
	...
	spin_lock_irqsave(&g_dld_buf.lock, flags);
	if (g_dld_buf.remain < len) {
		g_dld_buf.wraparound = 1;
		g_dld_buf.present = g_dld_buf.front;
		g_dld_buf.remain = DHD_LOG_DUMP_BUFFER_SIZE;
	}

	strncpy(g_dld_buf.present, tmp_buf, len);
	g_dld_buf.remain -= len;
	g_dld_buf.present += len;
	spin_unlock_irqrestore(&g_dld_buf.lock, flags);

	/* double check invalid memory operation */
	ASSERT((unsigned long)g_dld_buf.present <= g_dld_buf.max);
	va_end(args);
}
...
dhd_write_file(const char *filepath, char *buf, int buf_len)
{
	...
	fp = filp_open(filepath, O_RDWR | O_CREAT | O_SYNC, 0664);
	if (IS_ERR(fp)) {

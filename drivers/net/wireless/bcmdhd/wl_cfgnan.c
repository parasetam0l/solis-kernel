 ...
 *
 * $Id: wl_cfgnan.c 614926 2016-01-25 11:20:54Z $
 */
...
void print_event_name(u32 event_num)
{
	...
	case WL_NAN_EVENT_WINDOW_BEGIN_RANGING:
		...
		break;
#ifdef NAN_DP
	case WL_NAN_EVENT_DATA_IF_ADD:
		event_name = NAME_TO_STR(WL_NAN_EVENT_DATA_IF_ADD);
		break;
	case WL_NAN_EVENT_DATA_PEER_ADD:
		event_name = NAME_TO_STR(WL_NAN_EVENT_DATA_PEER_ADD);
		break;
#endif /* NAN_DP */
...
wl_cfgnan_set_vars_cbfn(void *ctx, uint8 *buf, uint16 type, uint16 len)
{
	...
	case WL_NAN_XTLV_REASON:
		memcpy(&ndata->reason_code, buf, len);
		break;
...
#ifdef NAN_DP
		event_mask |= NAN_EVENT_BIT(WL_NAN_EVENT_DATA_IF_ADD);
		event_mask |= NAN_EVENT_BIT(WL_NAN_EVENT_DATA_PEER_ADD);
#endif /* NAN_DP */
#ifdef NAN_P2P_CONFIG
...
static int
wl_cfgnan_set_master_preference(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	if(!cmd_data->master_pref){ /* Generate Random MasteR Preference */
		cmd_data->master_pref = RANDOM32();
		/* Floor the random master prefernce to 2 to 254 (both inclusive) */
		cmd_data->master_pref =
			(cmd_data->master_pref % (NAN_MAXIMUM_MASTER_PREFERENCE - 2)) + 2;
	}
	cmd_data->attr.type = WL_NAN_XTLV_MASTER_PREF;
	ret = wl_cfgnan_set_config_handler(ndev, cfg, NULL, 0, cmd_data);
	if (unlikely(ret)) {
		WL_ERR((" set nan master pref failed, master pref = %d, "
				"error = %d \n", cmd_data->master_pref, ret));
	} else {
		WL_DBG((" set nan master pref successful, "
				"master pref = %d \n", cmd_data->master_pref));
	}
...
static int
wl_cfgnan_set_nan_band(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	if (cmd_data->nan_band != NAN_BAND_INVALID) {
		cmd_data->attr.type = WL_NAN_XTLV_OPERATING_BAND;
		ret = wl_cfgnan_set_config_handler(ndev, cfg, NULL, 0, cmd_data);
		if (unlikely(ret)) {
			WL_ERR((" set nan band failed, nan band = %d, "
					"error = %d \n", cmd_data->nan_band, ret));
		} else {
			WL_DBG((" set nan band successful, "
					"nan band = %d \n", cmd_data->nan_band));
		}
	}
...
static int
wl_cfgnan_set_cluster_id(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	...
	WL_DBG(("CLUSTER ID = %02x:%02x:%02x:%02x:%02x:%02x\n",
			cmd_data->clus_id.octet[0], cmd_data->clus_id.octet[1],
			cmd_data->clus_id.octet[2], cmd_data->clus_id.octet[3],
			cmd_data->clus_id.octet[4], cmd_data->clus_id.octet[5]));
	cmd_data->attr.type = WL_NAN_XTLV_CLUSTER_ID;
	ret = wl_cfgnan_set_config_handler(ndev, cfg, NULL, 0, cmd_data);
	if (unlikely(ret)) {
		WL_ERR((" set nan cluster id failed, "
				"error = %d \n", ret));
	} else {
		WL_DBG((" set nan cluster id successful \n"));
	}
	return ret;
...
static int
wl_cfgnan_set_hop_count_limit(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	if (cmd_data->hop_count_limit) {
		cmd_data->attr.type = WL_NAN_XTLV_HOPCNT_LIMIT;
		ret = wl_cfgnan_set_config_handler(ndev, cfg, NULL, 0, cmd_data);
		if (unlikely(ret)) {
			WL_ERR((" set hop count limit failed, hop count limit = %d, "
					"error = %d\n", cmd_data->hop_count_limit, ret));
		} else {
			WL_DBG((" set hop count limit successful, hop count limit = %d \n",
...
static int
wl_cfgnan_set_sid_beacon_val(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	cmd_data->attr.type = WL_NAN_XTLV_SID_BEACON;
	ret = wl_cfgnan_set_config_handler(ndev, cfg, NULL, 0, cmd_data);
	if (unlikely(ret)) {
		WL_ERR((" set sid beacon val failed, sid beacon val = %d, "
					"error = %d\n", cmd_data->sid_beacon.sid_enable, ret));
	} else {
		WL_DBG((" set sid beacon val successful, sid beacon val = %d \n",
...
static int
wl_cfgnan_set_nan_oui(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, nan_cmd_data_t *cmd_data)
{
	s32 ret = BCME_OK;
	wl_nan_ioc_t *nanioc = NULL;
	...
	start = end = NAN_IOCTL_BUF_SIZE;
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = htod16(WL_NAN_CMD_OUI);
	pxtlv = nanioc->data;

	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_OUI,
			sizeof(uint32), &cmd_data->nan_oui,
...
fail:
	if (unlikely(ret)) {
		WL_ERR((" set nan oui failed \n"));
	} else {
		WL_DBG((" set nan oui successful \n"));
	}
	if (nanioc) {
		kfree(nanioc);
	}
	...
	struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	...
	if (cfg->nan_enable != true) {
		...
		ret = wl_cfgnan_set_master_preference(ndev, cfg, cmd_data);
		if (unlikely(ret)) {
			goto fail;
		}
		...
		/* setting nan band  */
		if (cmd_data->nan_band != NAN_BAND_INVALID) {
			ret = wl_cfgnan_set_nan_band(ndev, cfg, cmd_data);
			if (unlikely(ret)) {
				goto fail;
			}
		}
		...
		/* setting cluster ID */
		ret = wl_cfgnan_set_cluster_id(ndev, cfg, cmd_data);
		if (unlikely(ret)) {
			goto fail;
		}
		...
		if (cmd_data->hop_count_limit) {
			ret = wl_cfgnan_set_hop_count_limit(ndev, cfg, cmd_data);
			if (unlikely(ret)) {
				goto fail;
			}
		}
		...
		if (cmd_data->sid_beacon.sid_enable != NAN_SID_ENABLE_FLAG_INVALID) {
			ret = wl_cfgnan_set_sid_beacon_val(ndev, cfg, cmd_data);
			if (unlikely(ret)) {
				WL_ERR(("Failed in setting sid_beacon_val, error = %d\n", ret));
				goto fail;
			}
		}
		...
		/* setting nan oui */
		if (cmd_data->nan_oui != 0) {
			ret = wl_cfgnan_set_nan_oui(ndev, cfg, cmd_data);
			if (unlikely(ret)) {
				goto fail;
			}
		}
	...
	if (cfg->nan_running != true) {
		...
		WL_DBG((" nan is already joined \n"));
	}
fail:
	if (unlikely(ret)) { /* Do Cleanup on Start Handler Failure */
		WL_ERR((" nan start handler failed ... Cleaning Up \n"));
		/* Calling Stop Handler to revert back the nan state machine in DHD and Firmware */
		if (unlikely(wl_cfgnan_stop_handler(ndev, cfg, NULL, 0, NULL))) {
			WL_ERR((" ALERT !!! NAN cleaning failed ..."
					"Disable NAN explicitly before proceeding\n"));
		}
	}
	...
	struct bcm_cfg80211 *cfg, uint16 cmd_id, nan_cmd_data_t *cmd_data)
{
	...
	start = end = NAN_IOCTL_BUF_SIZE;
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = htod16(cmd_id);
	pxtlv = nanioc->data;

	if (cmd_data->period) {
		params.period = cmd_data->period;
	} else {
		params.period = 1;
	}
	if (cmd_data->ttl) {
		params.ttl = cmd_data->ttl;
	} else {
		params.ttl = WL_NAN_TTL_UNTIL_CANCEL;
	}
	params.flags = 0;
	params.flags = cmd_data->flags;
	WL_DBG(("flags %08x\n", params.flags));

	if (cmd_id == WL_NAN_CMD_PUBLISH) {
		params.instance_id = (wl_nan_instance_id_t)cmd_data->pub_id;
	}
	else if (cmd_id == WL_NAN_CMD_SUBSCRIBE) {
	...
	if (cmd_data->svc_info.data && cmd_data->svc_info.dlen) {
		uint16 len = cmd_data->svc_info.dlen;
		WL_DBG((" optional svc_info present, pack it \n"));
		ret = bcm_pack_xtlv_entry(&pxtlv,
			&end, WL_NAN_XTLV_SVC_INFO, len, cmd_data->svc_info.data,
			BCM_XTLV_OPTION_ALIGN32);
		if (unlikely(ret)) {
			goto fail;
		}
	...
	if (cmd_data->life_count) {
		...
		ret = bcm_pack_xtlv_entry(&pxtlv,
			&end, WL_NAN_XTLV_SVC_LIFE_COUNT, sizeof(cmd_data->life_count),
	...
	if (!cmd_data->use_srf) {
		...
		WL_DBG((" srf control %x\n", srf_control));
		if (cmd_data->srf_type == 1) {
			/* mac list */
			...
			if (cmd_data->mac_list.num_mac_addr < NAN_SRF_MAX_MAC) {
				...
				WL_ERR((" Too many mac addr %u !!!\n",
					cmd_data->mac_list.num_mac_addr));
				goto fail;
			}
			ret = bcm_pack_xtlv_entry(&pxtlv,
				&end, WL_NAN_XTLV_SR_FILTER, srf_size,
	...
	if (cmd_data->tx_match.dlen) {
		ret = bcm_pack_xtlv_entry(&pxtlv,
			&end, WL_NAN_XTLV_MATCH_TX, cmd_data->tx_match.dlen,
	...
	if (cmd_data->rx_match.dlen) {
		ret = bcm_pack_xtlv_entry(&pxtlv,
			&end, WL_NAN_XTLV_MATCH_RX, cmd_data->rx_match.dlen,
	...
	struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	...
	return wl_cfgnan_svc_handler(ndev, cfg, WL_NAN_CMD_PUBLISH, cmd_data);
}
	...
	struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	...
	return wl_cfgnan_svc_handler(ndev, cfg, WL_NAN_CMD_SUBSCRIBE, cmd_data);
}
	...
	struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	...
	*/
	...
	if (cmd_data->svc_info.data && cmd_data->svc_info.dlen) {
		WL_DBG((" optional svc_info present, pack it \n"));
		ret = bcm_pack_xtlv_entry(&pxtlv,
			&end, WL_NAN_XTLV_SVC_INFO, cmd_data->svc_info.dlen,
...
#ifdef NAN_DP
static int
wl_cfgnan_data_path_add_peer(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	wl_nan_ioc_t *nanioc = NULL;
	s32 ret = BCME_OK;
	...
	/*
	 * command to test
	 * wl: wl nan dp_add_peer
	 */
	start = end = NAN_IOCTL_BUF_SIZE;
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = htod16(WL_NAN_CMD_DATA_PATH_ADD_PEER);
	pxtlv = nanioc->data;
	/* parameters */
	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_MAC_ADDR,
			ETHER_ADDR_LEN, &cmd_data->mac_addr, BCM_XTLV_OPTION_ALIGN32);
	if (unlikely(ret)) {
		goto fail;
	}
	avail.time_ref = WL_TIME_REF_NAN_DW;
	avail.slots[0].start.intvl = 136;
	avail.slots[0].start.tmu = WL_TMU_TU;
	avail.slots[0].duration.intvl = 366;
	avail.slots[0].duration.tmu = WL_TMU_TU;
	avail.slots[0].chanspec = cmd_data->chanspec;
	WL_DBG(("chanspec set to [%x]\n", avail.slots[0].chanspec));
	avail.num_slots = 1;
	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_AVAIL,
			sizeof(avail), &avail, BCM_XTLV_OPTION_ALIGN32);
	if (unlikely(ret)) {
		goto fail;
	}
	nanioc->len = start - end;
	...
	ret = wldev_iovar_setbuf(ndev, "nan", nanioc, nanioc_size,
			cfg->ioctl_buf, WLC_IOCTL_MEDLEN, NULL);
	if (unlikely(ret)) {
		WL_ERR((" nan dp_add_peer failed, error = %d \n", ret));
		goto fail;
	} else {
		WL_ERR((" nan dp_add_peer successful \n"));
	}
fail:
	if (nanioc) {
		kfree(nanioc);
...
static int
wl_cfgnan_data_path_enable_disable(struct net_device *ndev,
		struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data,
		uint8 en)
{
	wl_nan_ioc_t *nanioc = NULL;
	...
	/*
	 * command to test
	 * wl: wl nan dp_enable 1/0
	 */

	/* nan dp_enable */
	start = end = NAN_IOCTL_BUF_SIZE;
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = htod16(WL_NAN_CMD_DATA_PATH_ENABLE);
	pxtlv = nanioc->data;
	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_ENABLED,
			sizeof(en), &en, BCM_XTLV_OPTION_ALIGN32);
	if (unlikely(ret)) {
	...
	ret = wldev_iovar_setbuf(ndev, "nan", nanioc, nanioc_size,
			cfg->ioctl_buf, WLC_IOCTL_MEDLEN, NULL);
	if (unlikely(ret)) {
		WL_ERR((" nan dp_enable_disable failed, error = %d \n", ret));
		goto fail;
	} else {
		WL_ERR((" nan dp %s successful \n", en ? "enable":"disable"));
	}
fail:
	if (nanioc) {
		kfree(nanioc);
		...
		struct bcm_cfg80211 *cfg, char *cmd, int size, nan_cmd_data_t *cmd_data)
{
	...
	if (cfg->nan_enable != true) {
		WL_ERR(("NAN is not Enabled, DP Enable blocked\n"));
		return BCME_ERROR;
	}
	if (cfg->nan_running != true) {
		WL_ERR(("NAN is not Joined, DP Enable blockedn"));
		return BCME_ERROR;
	}

	if (cfg->nan_dp_state == NAN_DP_STATE_ENABLED) {
		WL_ERR(("NAN DP Already Enabled\n"));
		return BCME_ERROR;
	}

	WL_DBG(("%s:%d state %d\n", __func__, __LINE__, cfg->nan_dp_state));
	if (cfg->nan_dp_state == NAN_DP_STATE_DISABLED) {
		cfg->ndp_peer.addr = cmd_data->mac_addr;
		cfg->ndp_peer.chanspec = cmd_data->chanspec;
		ret = wl_cfgnan_data_path_enable_disable(ndev, cfg, cmd, size,
				cmd_data, 1);
		if (ret != BCME_OK) {
			goto fail;
		}
	}
	timeout = wait_event_interruptible_timeout(cfg->ndp_if_change_event,
			(cfg->nan_dp_state == NAN_DP_STATE_ENABLED),
			msecs_to_jiffies(MAX_IF_ADD_WAIT_TIME));
	if (timeout <= 0 || (cfg->nan_dp_state == NAN_DP_STATE_DISABLED)) {
		WL_ERR(("timeout.Resetting NDP...\n"));
		goto fail;
	} else {
		cmd_data->chanspec = cfg->ndp_peer.chanspec;
		cmd_data->mac_addr = cfg->ndp_peer.addr;
		ret = wl_cfgnan_data_path_add_peer(bcmcfg_to_prmry_ndev(cfg), cfg,
				NULL, 0, cmd_data);
		if (ret != BCME_OK) {
			WL_ERR(("Failed to Add peer.Resetting NDP..."));
			goto fail;
		}
	}
	return ret;
fail:
	...
	if (ret == BCME_OK) {
		...
		WL_ERR(("%s:%d Alert!!Couldn't reset NDP\n",
				__func__, __LINE__));
	}
	return ret;
}
	...
	*/
	...
	case WL_NAN_XTLV_OPERATING_BAND:
		...
		ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_OPERATING_BAND,
				sizeof(cmd_data->nan_band), &cmd_data->nan_band,
				BCM_XTLV_OPTION_ALIGN32);
		break;
	...
	case WL_NAN_XTLV_HOPCNT_LIMIT:
		...
		ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_HOPCNT_LIMIT,
			sizeof(cmd_data->hop_count_limit), &cmd_data->hop_count_limit,
			BCM_XTLV_OPTION_ALIGN32);
		break;
	case WL_NAN_XTLV_SID_BEACON:
		ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_SID_BEACON,
			sizeof(cmd_data->sid_beacon), &cmd_data->sid_beacon,
			BCM_XTLV_OPTION_ALIGN32);
		break;
	default:
		ret = -EINVAL;
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *event, void *data)
{
	...
	case WL_NAN_EVENT_START:
	{
		nan_de_event_data_t de_evt_data;
		de_evt_data.nstatus = &tlv_data.nstatus;
		de_evt_data.nan_de_evt_type = NAN_EVENT_START;
		hal_event_id = GOOGLE_NAN_EVENT_DE_EVENT;
		nan_event_data = (void*)&de_evt_data;
	...
	case WL_NAN_EVENT_JOIN:
	{
		nan_de_event_data_t de_evt_data;
		de_evt_data.nstatus = &tlv_data.nstatus;
		de_evt_data.nan_de_evt_type = NAN_EVENT_JOIN;
		hal_event_id = GOOGLE_NAN_EVENT_DE_EVENT;
		nan_event_data = (void*)&de_evt_data;
		ptr += snprintf(ptr, end-ptr, SUPP_EVENT_PREFIX"%s " CLUS_ID_PREFIX MACF,
	...
	case WL_NAN_EVENT_DISCOVERY_RESULT:
	case WL_NAN_EVENT_REPLIED:
		if (event_num == WL_NAN_EVENT_DISCOVERY_RESULT) {
			 WL_DBG((" Recieved Service Discovery Result Event \n"));
		} else {
			WL_DBG((" Recieved Replied Event \n"));
		}
		hal_event_id = GOOGLE_NAN_EVENT_SUBSCRIBE_MATCH;
		nan_event_data = (void*)&(tlv_data);
	...
	case WL_NAN_EVENT_TERMINATED:
		...
		if ((ret = wl_cfgnan_get_inst_type(cfg, tlv_data.inst_id, &inst_type)) != BCME_OK) {
	...
	case WL_NAN_EVENT_RECEIVE:
		...
		nan_event_data = (void*)&(tlv_data);
		ptr += snprintf(ptr, end-ptr, SUPP_EVENT_PREFIX"%s " INSTANCE_ID_PREFIX"%d "
...
#ifdef NAN_DP
	case WL_NAN_EVENT_DATA_IF_ADD:
		hal_event_id = GOOGLE_NAN_EVENT_DATA_IF_ADD;
		nan_event_data = (void*)&(tlv_data);
		WL_DBG((" event:WL_NAN_EVENT_DATA_IF_ADD \n"));
		cfg->nan_dp_state = NAN_DP_STATE_ENABLED;
		break;
	case WL_NAN_EVENT_DATA_PEER_ADD:
		hal_event_id = GOOGLE_NAN_EVENT_DATA_PATH_OPEN;
		nan_event_data = (void*)&(tlv_data);
		WL_DBG((" event:WL_NAN_EVENT_DATA_PEER_ADD \n"));
		break;
#endif /* NAN_DP */
	default:

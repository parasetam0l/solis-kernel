 ...
 *
 * $Id: dhd_msgbuf.c 655590 2016-08-22 14:19:25Z $
 */
...
#define DHD_PROT_FUNCS	45
#else /* IDLE_TX_FLOW_MGMT */
...
typedef struct dhd_prot {
	...
	dhd_mb_ring_t	mb_2_ring_fn;	/* called when dongle needs to be notified of new msg */
	...
	dhd_pkttype_t pkttype)
{
	...
	nkey = dhd_pktid_map_reserve(dhd, handle, pkt);
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	prot->pktid_map_handle = DHD_NATIVE_TO_PKTID_INIT(dhd, MAX_PKTID_ITEMS);
...
static int
dhd_init_trap_buffer(dhd_pub_t *dhd)
{
	/* Initialize the trap buffer */
	bzero(&dhd->prot->fw_trap_buf, sizeof(dhd->prot->fw_trap_buf));

	/* FW going to DMA trap address, hence sizeof(uint32) (the size of the trap address) */
	return dhd_dma_buf_alloc(dhd, &dhd->prot->fw_trap_buf, sizeof(uint32));
}
...
dhd_set_host_cap(dhd_pub_t *dhd)
{
	...
	if (dhd->bus->api.fw_rev >= PCIE_SHARED_VERSION_6) {
		...
		/* allocate trap buffer where dongle will write trap data */
		ret = dhd_init_trap_buffer(dhd);
		if (ret != BCME_OK) {
			goto out;
		}
...
dhd_prot_init(dhd_pub_t *dhd)
{
	...
	if (dhd->dma_h2d_ring_upd_support || IDMA_ENAB(dhd)) {
		...
		/* If Implicit DMA is enabled, wait for FW to setup the DMA channel */
		if (IDMA_ENAB(dhd)) {
			dhdpcie_send_mb_data(dhd->bus, H2D_HOST_IDMA_INITED);

			timeleft = dhd_os_ioctl_resp_wait(dhd, &dhd->idma_inited);
			if ((!timeleft) || (!dhd->idma_inited)) {
				DHD_ERROR(("%s, IDMA init failed\n", __FUNCTION__));
			} else {
				DHD_ERROR(("%s, IDMA init success\n", __FUNCTION__));
			}
		}
	}
	...
	if (IFRM_ENAB(dhd)) {
		...
		timeleft = dhd_os_ioctl_resp_wait(dhd, &dhd->ifrm_inited);
		if ((!timeleft) || (!dhd->ifrm_inited)) {
			DHD_ERROR(("%s, IFRM init failed\n", __FUNCTION__));
		} else {
			DHD_ERROR(("%s, IFRM init success\n", __FUNCTION__));
		}
	}
...
void dhd_prot_detach(dhd_pub_t *dhd)
{
	...
	if (prot) {
		 ...
		 * they will be part of pktid_map_handle handler and PKT memory is allocated using
...
dhd_prot_reset(dhd_pub_t *dhd)
{
	...
	/* Reset PKTID map */
	DHD_NATIVE_TO_PKTID_RESET(dhd, prot->pktid_map_handle);
...
dhd_msgbuf_rxbuf_post_ts_bufs(dhd_pub_t *dhd)
{
	...
	prot->cur_ts_bufs_posted += dhd_msgbuf_rxbuf_post_ctrlpath(dhd, MSG_TYPE_TIMSTAMP_BUFPOST,
...
#ifndef IOCTLRESP_USE_CONSTMEM
		DHD_PKTID_AUDIT_RING_DEBUG(dhd, dhd->prot->pktid_map_handle, pktid,
...
#ifdef DHD_PKTID_AUDIT_RING
#ifndef IOCTLRESP_USE_CONSTMEM
	DHD_PKTID_AUDIT_RING_DEBUG(dhd, prot->pktid_map_handle, pkt_id,
...
workq_ring_full:
	...
	pkt = DHD_PKTID_TO_NATIVE(dhd, dhd->prot->pktid_map_handle, pktid,
...
#ifdef DHD_PKTID_AUDIT_RING
	DHD_PKTID_AUDIT_RING_DEBUG(dhd, dhd->prot->pktid_map_handle, bufid,
			DHD_DUPLICATE_FREE, msg, D2HRING_CTRL_CMPLT_ITEMSIZE);
#endif /* DHD_PKTID_AUDIT_RING */
	...
	uint16 nitems)
{
	...
	if (IDMA_ACTIVE(dhd) ||
		...
		db_index = IDMA_IDX0;
		prot->mb_2_ring_fn(dhd->bus, db_index);
	} else {
...
int dhd_prot_h2d_mbdata_send_ctrlmsg(dhd_pub_t *dhd, uint32 mb_data)
{
	...
	DHD_ERROR(("%s trying to send H2D MB data Req data 0x%04x\n",

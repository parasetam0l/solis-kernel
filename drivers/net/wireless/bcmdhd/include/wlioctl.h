 ...
 *
 * $Id: wlioctl.h 645241 2016-06-23 06:20:34Z $
 */
...
#define SWDIV_STATS_CURRENT_VERSION 0
typedef struct {
...
typedef struct wlc_rev_info {
	...
	uint		phyminorrev;	/* phy minor rev */
	uint		coreminorrev;	/* core minor rev */
} wlc_rev_info_t;
...
typedef struct wl_maccapture_params {
	uint8	gpio_sel;
	bool	la_mode;	/* TRUE: GPIO Out Enabled */
...
/* wl_pwr_pm_awake_stats_v2_t flags bit description */
...
typedef BWL_PRE_PACKED_STRUCT struct wl_mimo_meas_metrics {
	uint16 type;
	uint16 len;
...
} BWL_POST_PACKED_STRUCT wl_mimo_meas_metrics_t;
...
/* Maximum allowable number of SIDs */
#define WL_NAN_MAX_SIDS_IN_BEACONS 127
...
typedef struct sched_svc_timeslot_s {
...
#ifdef NAN_DP
	/* Nan Datapath commands */
	WL_NAN_CMD_DATA_PATH_ENABLE = 90,
	WL_NAN_CMD_DATA_PATH_ADD_PEER = 91
#endif
};
...
enum wl_nan_cmd_xtlv_id {
	...
	WL_NAN_XTLV_AVAIL = 0x17D, /* Availability */
	WL_NAN_XTLV_SVC_LIFE_COUNT = 0x17E, /* No of publish/subscribe match before expiry */
	WL_NAN_XTLV_HOPCNT_LIMIT = 0x17F, /* Hop count threshold */
	WL_NAN_XTLV_SID_BEACON = 0x180, /* flag for sending service id in beacon */
};
...
enum {
	...
	WL_PROXD_SESSION_FLAG_UNUSED_BIT6	= 0x00000040,
...
#define MSCH_PROFILER_VER	1

/* Flags used in wl_msch_register_params struct */
#define MSCH_REG_FLAGS_CHAN_CONTIGUOUS (1 << 0) /* Don't break up channels in chanspec_list */

typedef enum {
	MSCH_REG_BOTH_FIXED = 0,	/* both start and end time is fixed */
	MSCH_REG_START_FLEX = 1,	/* start time is flexible and duration is fixed */
	MSCH_REG_DUR_FLEX =  2,	/* start time is fixed and end time is flexible */
	MSCH_REG_BOTH_FLEX = 3,	/* both start and duration is flexible */
} msch_req_type_t;

#define MSCH_CMD_ENABLE_BIT	0x01
#define MSCH_CMD_PROFILE_BIT	0x02
#define MSCH_CMD_CALLBACK_BIT	0x04
#define MSCH_CMD_REGISTER_BIT	0x08
#define MSCH_CMD_ERROR_BIT	0x10
#define MSCH_CMD_DEBUG_BIT	0x20
#define MSCH_CMD_INFOM_BIT	0x40
#define MSCH_CMD_TRACE_BIT	0x80
#define MSCH_CMD_SIZE_MASK	0x00ff0000
#define MSCH_CMD_SIZE_SHIFT	16
#define MSCH_CMD_VER_MASK	0xff000000
#define MSCH_CMD_VER_SHIFT	24

typedef	BWL_PRE_PACKED_STRUCT struct wl_msch_register_params	{
...
} BWL_POST_PACKED_STRUCT wl_msch_register_params_t;

#define WLC_PROFILER_BUFFER_SIZE 2048

typedef BWL_PRE_PACKED_STRUCT struct wl_msch_collect_tlv {
	uint16	type;
	uint16	size;
	char	value[1];
} BWL_POST_PACKED_STRUCT wl_msch_collect_tlv_t;

/* type codes for WLC_PROFILER */
#define WLC_PROFILER_START		0	/* start event check */
#define WLC_PROFILER_EXIT		1	/* exit event check */
#define WLC_PROFILER_REQ		2	/* request event */
#define WLC_PROFILER_CALLBACK		3	/* call back event */
#define WLC_PROFILER_MESSAGE		4	/* message event */
#define WLC_PROFILER_PROFILE_START	5
#define WLC_PROFILER_PROFILE_END	6
#define WLC_PROFILER_REQ_HANDLE		7
#define WLC_PROFILER_REQ_ENTITY		8
#define WLC_PROFILER_CHAN_CTXT		9
#define WLC_PROFILER_TIMESLOT		10
#define WLC_PROFILER_REQ_TIMING		11
#define WLC_PROFILER_WLINDEX_SHIFT	8
#define WLC_PROFILER_WLINDEX_MASK	0xff00

typedef BWL_PRE_PACKED_STRUCT struct msch_profiler_data {
	uint32	time_lo;		/* Request time */
	uint32	time_hi;
} BWL_POST_PACKED_STRUCT msch_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_start_profiler_data {
	uint32	time_lo;		/* Request time */
	uint32	time_hi;
	uint32	status;
} BWL_POST_PACKED_STRUCT msch_start_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_message_profiler_data {
	uint32	time_lo;		/* Request time */
	uint32	time_hi;
	char	message[1];		/* message */
} BWL_POST_PACKED_STRUCT msch_message_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_req_param_profiler_data {
	uint16  flags;			/* Describe various request properties */
...
} BWL_POST_PACKED_STRUCT msch_req_param_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_timeslot_profiler_data {
	uint32 p_timeslot;
	uint32 timeslot_id;
	...
	uint32 fire_time_h;
	uint32 state;
} BWL_POST_PACKED_STRUCT msch_timeslot_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_req_timing_profiler_data {
	uint32 p_req_timing;
	uint32 p_prev;
	uint32 p_next;
	uint16 flags;
	...
	uint32 end_time_h;
	uint32 p_timeslot;
} BWL_POST_PACKED_STRUCT msch_req_timing_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_chan_ctxt_profiler_data {
	uint32 p_chan_ctxt;
	uint32 p_prev;
	uint32 p_next;
	uint16 chanspec;
...
} BWL_POST_PACKED_STRUCT msch_chan_ctxt_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_req_entity_profiler_data {
	uint32 p_req_entity;
...
} BWL_POST_PACKED_STRUCT msch_req_entity_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_req_handle_profiler_data {
	uint32 p_req_handle;
	uint32 p_prev;
	uint32 p_next;
	uint32 cb_func;
	...
	uint16 chan_cnt;
	uint32 flags;
} BWL_POST_PACKED_STRUCT msch_req_handle_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_profiler_profiler_data {
	uint32 time_lo;			/* Request time */
	uint32 time_hi;
...
} BWL_POST_PACKED_STRUCT msch_profiler_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_req_profiler_data {
	uint32 time_lo;			/* Request time */
	uint32 time_hi;
	uint16 chanspec_cnt;
	uint16 chanspec_ptr;
	uint16 req_param_ptr;
	uint16 pad;
} BWL_POST_PACKED_STRUCT msch_req_profiler_data_t;

typedef BWL_PRE_PACKED_STRUCT struct msch_callback_profiler_data {
	uint32 time_lo;			/* Request time */
	uint32 time_hi;
...
} BWL_POST_PACKED_STRUCT msch_callback_profiler_data_t;
#include <packed_section_end.h>
...
/* XTLV IDs for the Health Check top level contianer */
enum {

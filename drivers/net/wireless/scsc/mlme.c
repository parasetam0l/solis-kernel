...
void slsi_ap_obss_scan_done_ind(struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	scan_res = slsi_dequeue_cached_scan_result(ndev_vif->scan[scan_id].scan_results);
	while (scan_res) {
...
#define SLSI_FAPI_EPNO_NETWORK_MIN_SIZE (5)
	...
	if (count) {
		...
		if (epno_param) {
			...
			for (i = 0; i < count; i++) {
				...
				fapi_append_data(req, epno_param[i].ssid, epno_param[i].ssid_len);
			}
		} else if (epno_hs2_param) {
			u8 realm_length;

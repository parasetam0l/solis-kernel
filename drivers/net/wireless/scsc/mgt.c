...
static int slsi_mib_open_file(struct slsi_dev *sdev, char *mib_file_name, const struct firmware **fw);
static int slsi_mib_close_file(struct slsi_dev *sdev, const struct firmware *e);
static int slsi_mib_download_file(struct slsi_dev *sdev, char *mib_file_name);
...
void slsi_purge_scan_results(struct sk_buff_head *scan_result)
{
	...
	for (i = 0; i < SLSI_SCAN_RESULT_HEAD_MAX; i++)
		slsi_skb_queue_purge(&scan_result[i]);
}
...
struct sk_buff *slsi_dequeue_cached_scan_result(struct sk_buff_head *scan_result)
{
	...
	skb = slsi_skb_dequeue(&scan_result[SLSI_SCAN_RESULT_HIDDEN_SSID]);
	if (!skb)
		skb = slsi_skb_dequeue(&scan_result[SLSI_SCAN_RESULT_WITH_SSID]);
...
static void slsi_print_platform_id(struct slsi_dev *sdev)
{
	...
	if (r != 0) {
		SLSI_WARN(sdev, "Platform ID file NOT found\n");
		mx140_file_release_conf(sdev->maxwell_core, e);
		return;
	}
	if (!e) {
	...
	SLSI_INFO(sdev, "Platform ID loaded from %s: %s\n", path_name, (char *)e->data);
	mx140_file_release_conf(sdev->maxwell_core, e);
}
...
int slsi_start(struct slsi_dev *sdev)
{
	...
	err = slsi_mib_open_file(sdev, sdev->mib_file_name, &fw);
	if (err) {
	...
	SLSI_EC_GOTO(slsi_mib_download_file(sdev, sdev->mib_file_name), err, err_hip_started);
#endif
	/* Always try to download optional localmib file via mlme_set, ignore error */
	(void)slsi_mib_download_file(sdev, sdev->local_mib_file_name);
#endif
	/**
	 * Download MIB data, if any.
...
static int slsi_mib_download_file(struct slsi_dev *sdev, char *mib_file_name)
{
	...
	r = slsi_mib_open_file(sdev, mib_file_name, &e);
	if (r)
		return r;
	/**
...
static int slsi_mib_initial_get(struct slsi_dev *sdev)
{
	...
	int rxLength = 0;
	int r;
	static const struct slsi_mib_get_entry getValues[] = { { SLSI_PSID_UNIFI_CHIP_VERSION,            { 0, 0 } },
	...
	if (r == 0) {
		...
		if (values[0].type != SLSI_MIB_TYPE_NONE) {    /* CHIP_VERSION */
			WARN_ON(values[0].type != SLSI_MIB_TYPE_UINT);
			sdev->chip_info_mib.chip_version = values[0].u.uintValue;
		}
			...
			if (values[1].type == SLSI_MIB_TYPE_OCTET) {
				...
				for (i = 0; i < values[1].u.octetValue.dataLength/2; i++) {
					...
					if ((values[1].u.octetValue.data[i*2] > 14) &&
					    (values[1].u.octetValue.data[i*2 + 1] > 0)) {
		...
		if (values[2].type != SLSI_MIB_TYPE_NONE) /* HT enabled? */
			sdev->fw_ht_enabled = values[2].u.boolValue;
		else
			SLSI_WARN(sdev, "Error reading HT enabled mib\n");
		if (values[3].type != SLSI_MIB_TYPE_NONE)  /* VHT enabled? */
			sdev->fw_vht_enabled = values[3].u.boolValue;
		else
			SLSI_WARN(sdev, "Error reading VHT enabled mib\n");
		if (values[4].type == SLSI_MIB_TYPE_OCTET) { /* HT capabilities */
			if (values[4].u.octetValue.dataLength >= 4)
				memcpy(&sdev->fw_ht_cap, values[4].u.octetValue.data, 4);
			else
				SLSI_WARN(sdev, "Error reading HT capabilities\n");
		} else {
			SLSI_WARN(sdev, "Error reading HT capabilities\n");
		}
		if (values[5].type == SLSI_MIB_TYPE_OCTET) { /* VHT capabilities */
			if (values[5].u.octetValue.dataLength >= 4)
				memcpy(&sdev->fw_vht_cap, values[5].u.octetValue.data, 4);
			else
...
int slsi_auto_chan_select_scan(struct slsi_dev *sdev, int n_channels, struct ieee80211_channel *channels[])
{
	...
	if (r == 0) {
		...
		struct sk_buff *scan = slsi_dequeue_cached_scan_result(ndev_vif->scan[SLSI_SCAN_HW_ID].scan_results);
...
next_scan:
			scan = slsi_dequeue_cached_scan_result(ndev_vif->scan[SLSI_SCAN_HW_ID].scan_results);
		}
...
int slsi_set_uapsd_qos_info(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	SLSI_DBG1(sdev, SLSI_MLME, "QoS Info of ieee802.11 QoS capability:0x%x\n", sdev->device_config.qos_info);
...
int slsi_p2p_dev_null_ies(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	if (sdev->p2p_state == P2P_SCANNING) {
		...
		scan_result = slsi_dequeue_cached_scan_result(ndev_vif->scan[SLSI_SCAN_HW_ID].scan_results);
		while (scan_result) {

...
static ssize_t uart_error_cnt_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry(ourport, &drvdata_list, node) {
		...
		ret = sprintf(buf, "%03x %03x %03x %03x\n",
				port->icount.overrun, 0, port->icount.frame,
							 port->icount.brk);
...
static void uart_copy_local_buf(int dir, struct local_buf *local_buf,
					const unsigned int *buf, int len)
{
	...
	if (dir == 1)
		local_buf->index += sprintf(local_buf->buffer +
						local_buf->index, "[RX] ");
	else
		local_buf->index += sprintf(local_buf->buffer +
						local_buf->index, "[TX] ");
	...
	local_buf->index += sprintf(local_buf->buffer + local_buf->index,
			"[%5lu.%06lu] ", (unsigned long) t, nanosec_rem / 1000);

	for (a = 0; a < len; a++) {
		local_buf->index += sprintf(local_buf->buffer +
					local_buf->index, "%02X ", buf[a]);
	}
...
static inline void uart_clock_enable(struct s3c24xx_uart_port *ourport)
{
	...
	if (ourport->sclk_clk_rate != 0)
		clk_set_rate(ourport->clk, ourport->sclk_clk_rate);
...
static ssize_t s3c24xx_serial_bt_log(struct file *file, char __user *userbuf,
				size_t bytes, loff_t *off)
{
	int ret;
	struct s3c24xx_uart_port *ourport =
			&s3c24xx_serial_ports[BLUETOOTH_UART_PORT_LINE];
	...
	if (copied_bytes >= BT_LOG_BUFFER_SIZE) {
		...
		if (port && port->state->pm_state == UART_PM_STATE_ON &&
			ourport->port.line == BLUETOOTH_UART_PORT_LINE)
	...
	if (copied_bytes + bytes < BT_LOG_BUFFER_SIZE) {
		ret = copy_to_user(
			userbuf, ourport->local_buf.buffer + copied_bytes,
									bytes);
		if (ret) {
	...
	byte_to_read = BT_LOG_BUFFER_SIZE - copied_bytes;
...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	if (port_index == BLUETOOTH_UART_PORT_LINE) {
		...
		/* Allocate Data */
		ourport->local_buf.buffer =
				kzalloc(BT_LOG_BUFFER_SIZE, GFP_KERNEL);
		...
		ent = proc_create("log", 0644, bt_log_dir, &proc_fops_btlog);
		if (ent == NULL) {
	...
	if (of_property_read_u32(pdev->dev.of_node, "uart-sclk-frequency",
		...
		dev_err(&pdev->dev, "Set SCLK frequency to %u\n", ourport->sclk_clk_rate);

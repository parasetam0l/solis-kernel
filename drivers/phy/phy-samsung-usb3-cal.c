...
void samsung_exynos_cal_usb3phy_tune_dev(struct exynos_usbphy_info *usbphy_info)
{
	...
	phy_drd = container_of(usbphy_info, struct exynos_usbdrd_phy, usbphy_info);
	dev_info(phy_drd->dev, "Try to Device mode phytune\n");
	...
	if (usbphy_info->hs_tune) {
		...
		phyparam0 = readl(regs_base + EXYNOS_USBCON_PHYPARAM0);
		dev_info(phy_drd->dev, "Current PARAM 0x%0x for Dev hs_phytune\n", phyparam0);
		...
		writel(phyparam0, regs_base + EXYNOS_USBCON_PHYPARAM0);
		dev_info(phy_drd->dev, "Seted by 0x%0x Dev hs_phytune PARAM0\n", phyparam0);
	}
		...
		struct exynos_usbphy_info *usbphy_info)
{
	...
	phy_drd = container_of(usbphy_info, struct exynos_usbdrd_phy, usbphy_info);
	dev_info(phy_drd->dev, "Try to Host mode phytune\n");
	...
	if (usbphy_info->hs_tune) {
		...
		/* Set tune value for 2.0(HS/FS) function */
		phyparam0 = readl(regs_base + EXYNOS_USBCON_PHYPARAM0);
		dev_info(phy_drd->dev, "Current PARAM 0x%0x for Host hs_phytune\n", phyparam0);
		...
		writel(phyparam0, regs_base + EXYNOS_USBCON_PHYPARAM0);
		dev_info(phy_drd->dev, "Seted by 0x%0x Host hs_phytune PARAM0\n", phyparam0);
	}

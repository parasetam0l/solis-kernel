...
#define 	INVALID_PACKET_SIZE	2

MODULE_FIRMWARE(CY_FW_FILE_NAME);

#ifdef CYTTSP5_SECURE_TOUCH
static void cyttsp5_secure_touch_stop(
	struct cyttsp5_core_data *cd, int blocking);
static int cyttsp5_secure_touch_init(struct cyttsp5_core_data *cd);
...
static ssize_t cyttsp5_secure_touch_enable_show(struct device *dev,
					struct device_attribute *attr, char *buf);
static ssize_t cyttsp5_secure_touch_enable_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count);
static ssize_t cyttsp5_secure_touch_show(struct device *dev,
					struct device_attribute *attr, char *buf);
#endif
...
static int cyttsp5_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type);

static struct sleep_monitor_ops  cyttsp5_sleep_monitor_ops = {
	 .read_cb_func =  cyttsp5_get_sleep_monitor_cb,
};

static int cyttsp5_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type)
{
	struct cyttsp5_core_data *cd = priv;
	...
	*raw_val = -1;
	...
	if (check_level == SLEEP_MONITOR_CHECK_SOFT) {
		...
		*raw_val = state;
		...
		tsp_mode = cyttsp5_test_get_power_mode(cd->dev);
		if (tsp_mode < 0) {
			dev_err(cd->dev, "%s:get failed chip power mode.[%d]\n",
				__func__, tsp_mode);
			goto out;
		}
...
out:
	*raw_val = ((state & STATE_MARK) << STATE_BIT) |\
			(cd->release_cnt & CNT_MARK);

	if (cd->release_cnt > PRETTY_MAX)
		pretty = PRETTY_MAX;
	else
		pretty = cd->release_cnt;

	cd->release_cnt = 0;

	dev_dbg(cd->dev, "%s: raw_val[0x%08x], check_level[%d], state[%d], pretty[%d]\n",
...
#if defined(CYTEST_GETPOWERMODE)
static int cyttsp5_get_power_mode(struct cyttsp5_core_data *cd,
	enum cy_power_mode* pwrmode)
{
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_APP_COMMAND(HID_OUTPUT_GET_POWER_MODE),
	};
	int rc;
	...
	if (rc) {
		dev_err(cd->dev, "%s: fail send out and wait r=%d\n",
			__func__, rc);
		goto release;
	}

	*pwrmode = cd->response_buf[5];
	dev_dbg(cd->dev, "%s: power mode = %d\n",
		__func__, *pwrmode);
...
int cyttsp5_test_get_power_mode(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	enum cy_power_mode pwrmode = PWRMODE_ACTIVE;
	int rc;

	if ((cd->irq_wake) || (!cd->wait_until_wake))
		return PWRMODE_LOWPOWER;

	rc = cyttsp5_get_power_mode(cd, &pwrmode);
	if (rc < 0)
		dev_err(cd->dev, "%s: fail cyttsp5_get_power_mode, rc %d\n",
			__func__, rc);
	else {
		dev_dbg(cd->dev, "%s: powermode = %d\n",
			__func__, pwrmode);
		rc = pwrmode;
	}
...
int cyttsp5_get_firmware_crc(struct cyttsp5_core_data *cd,
	u32* firmware_crc)
{
	struct cyttsp5_hid_output hid_output = {
	HID_OUTPUT_APP_COMMAND(HID_OUTPUT_GET_FIRMWARE_CRC),
	};
	int rc;
	...
	*firmware_crc = get_unaligned_le32(&cd->response_buf[5]);

	dev_dbg(cd->dev, "%s: firmware crc = %08x\n",
	__func__, *firmware_crc);
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (command_code != hid_output->command_code) {
		dev_err(cd->dev,
			"%s: first HID output response, wrong command_code:%X\n",
			__func__, command_code);
		dev_err(cd->dev,
			"%s: second HID output response, wrong command_code:%X\n",
			__func__, hid_output->command_code);
		return -EPROTO;
	}
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0),
			msecs_to_jiffies(timeout_ms));
	if (IS_TMO(t)) {
		dev_err(cd->dev, "%s: HID output cmd execution timed out[0x%x][0x%x][%d]\n",
			__func__, hid_output->cmd_type, hid_output->command_code, IS_TMO(t));
		rc = -ETIME;
		goto error;
	}
...
static int cyttsp5_hid_output_run_self_test(
...
static int _cyttsp5_hid_output_run_self_test(
	struct device *dev, int protect, u8 test_id, u8 write_idac,
	u8 *result, u8 *available)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

	if (protect)
	return cyttsp5_hid_output_run_self_test(cd, test_id, write_idac,
		result, available);
	else
	return cyttsp5_hid_output_run_self_test_(cd, test_id, write_idac,
		result, available);
}
...
static int cyttsp5_hid_output_get_self_test_result(
		struct cyttsp5_core_data *cd, u16 read_offset, u16 read_length,
		u8 test_id, u16 *actual_read_len, u8 *read_buf)
{
	int rc;
...
static int _cyttsp5_hid_output_get_self_test_result(
	struct device *dev, int protect, u16 read_offset, u16 read_length,
	u8 test_id, u16 *actual_read_len, u8 *read_buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

	if (protect)
	return cyttsp5_hid_output_get_self_test_result(cd, read_offset,
		read_length, test_id, actual_read_len, read_buf);
	else
	return cyttsp5_hid_output_get_self_test_result_(cd, read_offset,
		read_length, test_id, actual_read_len, read_buf);
}
...
#ifdef CY_USE_CAL_FLAG_IN_USER_DATA
...
//#define USER_DATA_BUF_SIZE (CFG_BLK_HDR_SIZE + DDATA_SIZE)
...
#endif//CY_USE_CAL_FLAG_IN_USER_DATA

int cyttsp5_hid_output_get_cal_status(struct device *dev, u8* status)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
	u8 param[1] = {0};
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_APP_COMMAND(HID_OUTPUT_GET_CAL_STATUS),
		.write_length = 1,
		.write_buf = param,
		.timeout_ms = CY_HID_OUTPUT_CALIBRATE_IDAC_TIMEOT,
	};
	...
	rc = cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	if (rc) {
		dev_err(dev, "%s: timeout for hid_send(%d)\n", __func__, rc);
		return rc;
	}

	*status = cd->response_buf[5];
	dev_info(dev, "%s: done rc=%d status=%d\n", __func__,
		rc, *status);
	return 0;
}
...
recal:
	...
	dev_dbg(dev, "%s: do calibration\n", __func__);
	for (mode = 0; mode < 3; mode++) {
		dev_dbg(dev, "%s: calib mode %d\n", __func__, mode);
		rc = cyttsp5_hid_output_calibrate_idacs_(cd, mode);
		if (rc < 0) {
...
static int cyttsp5_hw_soft_reset(struct cyttsp5_core_data *cd)
{
	...
	dev_info(cd->dev, "%s: execute SOFT reset\n", __func__);

	if (cd->hid_desc.hid_desc_len == 0) {
		rc = cyttsp5_get_hid_descriptor_(cd, &cd->hid_desc);
		if (rc < 0) {
			dev_err(cd->dev, "%s: FAILED get_hid_descriptor\n",
					__func__);
			return rc;
		}
	}
...
static int cyttsp5_hw_hard_reset(struct cyttsp5_core_data *cd)
{
	...
	if (cd->cpdata->xres) {
		dev_info(cd->dev, "%s: execute HARD reset\n", __func__);
	...
	dev_dbg(cd->dev, "%s: FAILED to execute HARD reset\n", __func__);
	return -ENOSYS;
}
	...
	for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
		...
		if (field) {
			...
			dev_vdbg(cd->dev, "%s: ofs:%d size:%d min:%d max:%d bofs:%d report:%d",
				...
				(int)si->tch_abs[i].ofs, (int)si->tch_abs[i].size,
				(int)si->tch_abs[i].min, (int)si->tch_abs[i].max,
				(int)si->tch_abs[i].bofs, (int)si->tch_abs[i].report);
...
queue_startup:
	if (rc) {
		if (cd->irq_enabled && cd->device_enabled) {
			dev_err(cd->dev,
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->sleep_state == SS_SLEEP_OFF) {
		...
		mutex_unlock(&cd->system_lock);
		dev_err(cd->dev,
			"%s: failed entering deep sleep[%d]\n",
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: cyttsp5_put_device_into_sleep_ fail\n",
			__func__);
		cd->sleep_wake_error = true;
	}
	mutex_lock(&cd->system_lock);
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	if (size == INVALID_PACKET_SIZE) {
		dev_err(cd->dev, "%s: do nothing for 02 00 packet.[%d]\n",
			__func__, size);
		return 0;
	}
	...
	if (size == 0) {
		...
		if (!cd->hid_reset_cmd_state && !cd->hid_cmd_state) {
			mutex_unlock(&cd->system_lock);
			dev_info(cd->dev, "%s: Device Initiated Reset\n",
					__func__);
			return 0;
		} else {
			cd->hid_reset_cmd_state = 0;
			dev_info(cd->dev, "%s: clear hid_reset_cmd_state.\n",
					__func__);
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			dev_err(dev, "%s: Waiting device wake up\n", __func__);
...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
	...
	if (cd->cpdata->irq_stat &&
		...
		if (cd->device_enabled)
			dev_err(cd->dev,
				"%s: interrupt pin is"\
				" high before i2c read\n", __func__);
		return IRQ_HANDLED;
	}

	if (cd->irq_wake) {
#if defined(TOUCH_WAKEUP_BOOSTER)
		touch_wakeup_booster_turn_on();
#endif
		dev_info(cd->dev, "%s: Detected wake up event\n", __func__);
...
{
	struct cyttsp5_platform_data *pdata = cd->pdata;
	...
	if (cd->hw_power_state == true) {
		dev_dbg(cd->dev, "%s: cd->sysinfo.ready=[%d]\n", __func__, cd->sysinfo.ready);
		rc = cyttsp5_reset(cd);
		if (rc < 0) {
			dev_info(cd->dev, "%s: reset failed\n", __func__);
			goto error;
		}
	} else {
		dev_info(cd->dev, "%s: hw power on now\n", __func__);
		rc = cd->cpdata->power(cd->cpdata, 1, cd->dev, 0);
		if (rc < 0) {
			dev_err(cd->dev, "%s: Failed power on\n", __func__);
			goto error;
		}
		cd->hw_power_state = true;

		if (core_pdata->rst_gpio) {
			rc = cyttsp5_reset(cd);
			if (rc < 0) {
				dev_info(cd->dev, "%s: reset failed\n", __func__);
				goto error;
		...
		if (!cd->irq_enabled) {
			cd->irq_enabled = true;
			enable_irq(cd->irq);
			dev_info(cd->dev, "%s: irq enabled\n", __func__);
		}
	}

#ifdef CYTTSP5_ENABLE_HARDRESET
...
static int cyttsp5_core_wake_device_(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: cyttsp5_core_wake_device_from_deep_sleep_ fail\n",
			__func__);
		cd->sleep_wake_error = true;
	}
	return rc;
...
static int cyttsp5_si_get_samsung_tsp_info_(struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_hid_output_read_conf_block_(cd, 0, sizeof(read_buf),
		CY_DDATA_EBID, read_buf, &crc);
	if (rc) {
		dev_err(cd->dev, "%s: fail conf block, rc=%d\n",
					__func__, rc);
		goto exit;
	}
...
static int cyttsp5_check_silicon_id(struct cyttsp5_core_data *cd)
{
	...
	rc = _cyttsp5_request_hid_output_bl_get_information(cd->dev, 0,
								return_data);
	if (rc < 0){
		dev_err(cd->dev, "%s: Failed to get bl_get_information(%d)\n",
			__func__, rc);
		cd->silicon_id = DEFAULT_SILICON_ID;
	} else
		cd->silicon_id = get_unaligned_le16(&return_data[2]);

	dev_info(cd->dev, "%s:silicon_id:0x%04x\n", __func__, cd->silicon_id);
	return 0;
}
...
reset:
	...
	if (cd->sysinfo.ready && cd->device_enabled) { // booting is done.
	...
	rc = cyttsp5_hid_output_resume_scanning_(cd);
	if (rc) {
		dev_err(cd->dev, "%s: error on resume scan, rc=%d\n",
			__func__, rc);
		RETRY_OR_EXIT(retry--, reset, exit);
	}
...
int cyttsp5_core_stop(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_platform_data *pdata = cd->pdata;
	struct cyttsp5_core_platform_data *core_pdata = pdata->core_pdata;
	...
	if (core_pdata->rst_gpio) {
		rc = gpio_request_one(core_pdata->rst_gpio, GPIOF_OUT_INIT_LOW, "tsp_reset");
		if (rc < 0) {
			dev_err(dev, "%s: failed to get LCD reset GPIO\n", __func__);
			goto exit;
		}
		gpio_free(core_pdata->rst_gpio);
	}

	if (cd->pinctrl && cd->gpio_off) {
		if (pinctrl_select_state(cd->pinctrl, cd->gpio_off)) {
			dev_err(dev, "%s: failed to set gpio_off state\n", __func__);
			goto exit;
		}
	} else
		dev_err(dev, "%s: pinctrl or gpio_off is NULL\n", __func__);

exit:
	return 0;
}

extern int set_report_rate(struct cyttsp5_core_data *cd, int rate);
extern int set_glove_enable(struct cyttsp5_core_data *cd, bool enable);
int cyttsp5_core_start(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	...
	if (cd->report_rate != DEFAULT_REPORT_RATE)
		set_report_rate(cd, cd->report_rate);

    if (cd->glove_enable != false)
        set_glove_enable(cd, cd->glove_enable);

#ifdef CY_USE_CAL_FLAG_IN_USER_DATA
	if (cd->cal_flag == CAL_NOT_DONE) {
		dev_info(cd->dev, "%s: cal_flag is 0, do calibration.\n",
			__func__);
...
int cyttsp5_core_ambient_on(struct device *dev)
{
	...
	if (cd->pinctrl && cd->gpio_wake) {
		if (pinctrl_select_state(cd->pinctrl, cd->gpio_wake))
			dev_err(dev, "%s: failed to turn on gpio_wake\n", __func__);
	} else
		dev_err(dev, "%s: pinctrl or gpio_wake is NULL\n", __func__);
...
int cyttsp5_core_ambient_off(struct device *dev)
{
	...
	if (!cd->irq_wake) {
		dev_err(dev, "%s: irq_wake is disable\n", __func__);
		return 0;
	}
	...
	if (rc < 0) {
		dev_err(dev, "%s: cyttsp5_core_wake fail, queue startup\n",
			__func__);
	}
#ifdef CYTTSP5_WATCHDOG_ENABLE
	else
...
int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (!cd->device_enabled) {
		dev_dbg(dev, "%s: already device disabled.\n", __func__);
		return 0;
	}

	if (cd->irq_wake) {
		dev_dbg(dev, "%s: already irq_wake enabled\n", __func__);
		return 0;
	}
...
int cyttsp5_core_resume(struct device *dev)
{
	...
	if (!cd->irq_wake) {
		dev_dbg(dev, "%s: already irq_wake disabled\n", __func__);
		return 0;
	}

	if (cd->sleep_wake_error) {
		dev_err(dev, "%s: sleep wakeup error happend.\n",
			__func__);
		cd->sleep_wake_error = 0;
		cyttsp5_queue_startup(cd);
	}

	if (cd->touch_wake == true)
...
static struct cyttsp5_core_nonhid_cmd _cyttsp5_core_nonhid_cmd = {
	...
	.run_self_test = _cyttsp5_hid_output_run_self_test,
	.get_self_test_result = _cyttsp5_hid_output_get_self_test_result,
...
static void cyttsp5_secure_touch_notify(struct cyttsp5_core_data *cd)
{
	struct i2c_client *client = container_of(cd->dev, struct i2c_client, dev);
					...
					struct device_attribute *attr, char *buf)
{
	...
	dev_info(cd->dev, "%s: [%d]\n", __func__, atomic_read(&cd->st_enabled));

	return scnprintf(buf, PAGE_SIZE, "%d", atomic_read(&cd->st_enabled));
}

static int cyttsp5_secure_touch_clk_prepare_enable(struct cyttsp5_core_data *cd)
{
	int ret;
...
static void cyttsp5_secure_touch_clk_disable_unprepare(struct cyttsp5_core_data *cd)
{
					...
					const char *buf, size_t count)
{
	...
	if (!cd->st_initialized)
		return -EIO;

	dev_info(&client->dev, "%s:[%lu]\n", __func__, value);

	err = count;

	switch (value) {
	case 0:
		if (atomic_read(&cd->st_enabled) == 0)
			break;

		cyttsp5_secure_touch_clk_disable_unprepare(cd);
		pm_runtime_put_sync(client->adapter->dev.parent);
		atomic_set(&cd->st_enabled, 0);
		cyttsp5_secure_touch_notify(cd);
		cyttsp5_irq(client->irq, cd);
		complete(&cd->st_powerdown);
		break;
	case 1:
		if (atomic_read(&cd->st_enabled)) {
			err = -EBUSY;
		...
		if (pm_runtime_get_sync(client->adapter->dev.parent) < 0) {
			dev_err(&client->dev, "pm_runtime_get failed\n");
			err = -EIO;
			break;
		}

		if (cyttsp5_secure_touch_clk_prepare_enable(cd) < 0) {
			pm_runtime_put_sync(client->adapter->dev.parent);
			err = -EIO;
			break;
		}
		...
		INIT_COMPLETION(cd->st_irq_processed);
		atomic_set(&cd->st_enabled, 1);
		synchronize_irq(client->irq);
		atomic_set(&cd->st_pending_irqs, 0);
		break;
	default:
		dev_err(&client->dev,
			"%s: unsupported value: %lu\n", __func__, value);
		err = -EINVAL;
		break;
	}
					...
					struct device_attribute *attr, char *buf)
{
	...
	if (atomic_cmpxchg(&cd->st_pending_irqs, -1, 0) == -1)
		return -EINVAL;

	if (atomic_cmpxchg(&cd->st_pending_irqs, 1, 0) == 1)
		val = 1;
	else
		complete(&cd->st_irq_processed);

	return scnprintf(buf, PAGE_SIZE, "%u", val);
}

static DEVICE_ATTR(secure_touch_enable, S_IRUGO | S_IWUSR | S_IWGRP ,
			 cyttsp5_secure_touch_enable_show,
			 cyttsp5_secure_touch_enable_store);
static DEVICE_ATTR(secure_touch, S_IRUGO, cyttsp5_secure_touch_show, NULL);

static struct attribute *secure_attrs[] = {
	&dev_attr_secure_touch_enable.attr,
	&dev_attr_secure_touch.attr,
	NULL
};

static const struct attribute_group secure_attr_group = {
	.attrs = secure_attrs,
};
...
static int cyttsp5_secure_touch_init(struct cyttsp5_core_data *cd)
{
	...
	init_completion(&cd->st_irq_processed);
	/* Get clocks */
	cd->core_clk = clk_get(client->adapter->dev.parent, "core_clk");
	if (IS_ERR(cd->core_clk)) {
		ret = PTR_ERR(cd->core_clk);
		dev_err(cd->dev,
			"%s: error on clk_get(core_clk):%d\n", __func__, ret);
		goto out;
	}

	cd->iface_clk = clk_get(client->adapter->dev.parent, "iface_clk");
	if (IS_ERR(cd->iface_clk)) {
		ret = PTR_ERR(cd->iface_clk);
		dev_err(cd->dev,
			"%s: error on clk_get(iface_clk):%d\n", __func__, ret);
		goto err_iface_clk;
	}

	ret = sysfs_create_group(&client->dev.kobj, &secure_attr_group);
	if (ret > 0) {
		dev_err(cd->dev,
			"%s: error create secure sysfs\n", __func__);
		goto err_sysfs;
	}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->bus_ops = ops;
	cd->report_rate = pdata->core_pdata->report_rate;
	cd->fw_path = pdata->core_pdata->fw_path;
	...
	dev_dbg(dev, "%s: initialize irq=%d\n", __func__, cd->irq);
	...
	if (rc < 0)
		dev_err(dev, "%s: failed to enable_irq_wake\n", __func__);
	...
	cd->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(cd->pinctrl)) {
		dev_err(dev, "%s: failed to get decon pinctrl\n", __func__);
		cd->pinctrl = NULL;
	} else {
		cd->gpio_wake = pinctrl_lookup_state(cd->pinctrl, "wakeup");
		if (IS_ERR(cd->gpio_wake)) {
			dev_err(dev, "%s: failed to get gpio_wake pin state\n", __func__);
			cd->gpio_wake = NULL;
		}
		cd->gpio_off = pinctrl_lookup_state(cd->pinctrl, "off");
		if (IS_ERR(cd->gpio_off)) {
			dev_err(dev, "%s: failed to get gpio_off pin state\n", __func__);
			cd->gpio_off = NULL;
		}
	}
	...
	if (cd->pinctrl && cd->gpio_off) {
		if (pinctrl_select_state(cd->pinctrl, cd->gpio_off))
			dev_err(dev, "%s: failed to turn on gpio_off\n", __func__);
	} else
		dev_err(dev, "%s: pinctrl or gpio_off is NULL\n", __func__);
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(cd, &cyttsp5_sleep_monitor_ops,
			SLEEP_MONITOR_TSP);
#endif

	rc = input_register_device(cd->md.input);
	if (rc < 0) {
		...
		cd->md.input_device_registered = true;

#ifdef CYTTSP5_SECURE_TOUCH
	rc = cyttsp5_secure_touch_init(cd);
	if (rc < 0) {
		dev_err(dev, "%s: Error, failed int secure touch rc=%d\n",
			__func__, rc);
		goto err_secure_touch;
	}
	cyttsp5_secure_touch_stop(cd, true);
#endif
...
#ifdef CYTTSP5_SECURE_TOUCH
err_secure_touch:
	input_unregister_device(cd->md.input);
#endif

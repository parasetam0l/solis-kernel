...
static int cyttsp5_hw_power(struct device *dev, int onoff)
{
	...
	static bool pwr_1p8_gpio_state = 0;
	int ret;

	if (!avdd_vreg) {
		avdd_vreg = regulator_get(dev, "vdd_ldo16");
		if (IS_ERR(avdd_vreg)) {
			avdd_vreg = NULL;
	...
	if (!vddo_vreg && !core_pdata->pwr_1p8_gpio) {
		vddo_vreg = regulator_get(dev, "vdd_ldo17");
		if (IS_ERR(vddo_vreg)) {
			vddo_vreg = NULL;
			dev_err(dev, "failed to get vddo regulator\n");
			return -ENODEV;
		}
		ret = regulator_set_voltage(vddo_vreg, 1800000, 1800000);
		if (ret) {
			dev_err(dev,"unable to set voltage for vddo_vreg, %d\n",
				ret);
			return ret;
		}
	}
	...
	if (onoff) {
		...
		if (core_pdata->pwr_1p8_gpio) {
			if (pwr_1p8_gpio_state) {
				dev_err(dev, "GPIO TSP_1.8V is already enabled\n");
				return ret;
			} else {
				ret = gpio_request(core_pdata->pwr_1p8_gpio, "TSP_1.8V");
				if (ret)
					dev_err(dev, "unable to set GPIO TSP_1.8V, %d\n", ret);

				gpio_direction_output(core_pdata->pwr_1p8_gpio, 1);
				pwr_1p8_gpio_state = 1;
			}
		} else {
		...
		if (core_pdata->pwr_1p8_gpio) {
			if (!pwr_1p8_gpio_state) {
				dev_err(dev, "GPIO TSP_1.8V is already disabled\n");
				return ret;
			} else {
				gpio_set_value(core_pdata->pwr_1p8_gpio, 0);
				gpio_free(core_pdata->pwr_1p8_gpio);
				pwr_1p8_gpio_state = 0;
			}
		} else {
	...
	if (core_pdata->pwr_1p8_gpio)
		dev_info(dev, "%s:[%s] vddo:[%s], avdd:[%s]\n", __func__,
			onoff ? "ON":"OFF",
			gpio_get_value(core_pdata->pwr_1p8_gpio) ? "on":"off",
			regulator_is_enabled(avdd_vreg) ? "on":"off");
	else
		dev_info(dev, "%s:[%s] avdd:[%s] vddo:[%s]\n", __func__,
			onoff ? "ON":"OFF",
			regulator_is_enabled(avdd_vreg) ? "on":"off",
			regulator_is_enabled(vddo_vreg) ? "on":"off");
		...
		struct device *dev)
{
	...
	if (pdata->rst_gpio) {
		rc = gpio_request_one(pdata->rst_gpio, GPIOF_OUT_INIT_HIGH, "tsp_reset");
		if (rc < 0) {
			dev_err(dev, "failed to get LCD reset GPIO\n");
			goto exit;
		}
		...
		struct device_node *core_node)
{
	...
	if (!gpio_is_valid(pdata->irq_gpio)) {
		pr_err("%s: failed to get irq_gpio property\n", __func__);
		rc = ENXIO;
		goto fail_free;
	}
	...
	rc = of_property_read_u32(core_node, "cy,pwr_1p8_gpio", &value);
	if (!rc)
		pdata->pwr_1p8_gpio = value;
	...
	rc = of_property_read_u32(core_node, "cy,report_rate", &value);
	if (!rc)
		pdata->report_rate = value;
	else
		pdata->report_rate = DEFAULT_REPORT_RATE;

	rc = of_property_read_string(core_node, "cy,fw_path", &pdata->fw_path);
	if (rc) {
		pdata->fw_path = CY_CSP_FW_FILE_NAME;
		pr_err("%s: Failed to get fw_paht[%d]\n", __func__, rc);
	}
...
static int __init cyttsp5_i2c_init(void)
{
	...
	pr_debug("%s: Cypress TTSP v5 I2C Driver (Built %s) rc=%d\n",

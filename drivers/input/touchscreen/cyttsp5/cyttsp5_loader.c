...
#define CYTTSP5_LOADER_FW_UPGRADE_RETRY_COUNT 5
#define CY_REQUEST_FIRMWARE_WAIT
...
#define DEFAULT_HW_VER 0x01
#define CY_CSP_FW_VERSION 0x0400
#ifdef SAMSUNG_INIT_FW_UPGRADE
#define INIT_FW_VERSION 0x0000
#endif

#define HW_VER_OFFSET	1
#define SW_VER_OFFSET	2
#define CFG_VER_OFFSET	4
...
static struct cyttsp5_touch_firmware cyttsp5_firmware = {
	...
	.hw_version = DEFAULT_HW_VER,
	.fw_version = CY_CSP_FW_VERSION,
};
#else
static struct cyttsp5_touch_firmware cyttsp5_firmware = {
	.img = NULL,
		...
		int fw_size)
{
	...
	if (rc < 0) {
		/*pm_runtime_put(dev);*/
		dev_err(dev, "%s: request_exclusive failed rc=%d\n", __func__, rc);
		goto exit;
	}
	...
	while (retry--) {
		...
		if (rc < 0)
			dev_err(dev, "%s: Firmware update failed rc=%d, retry:%d/%d\n",
				__func__, rc, retry, CYTTSP5_LOADER_FW_UPGRADE_RETRY_COUNT);
		else
			break;
...
static void _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
		dev_err(dev, "%s: No binary firmware\n", __func__);
		...
		const struct firmware *fw)
{
	...
	cyttsp5_firmware.fw_version = fw->data[SW_VER_OFFSET] << 8;
	cyttsp5_firmware.fw_version |= fw->data[SW_VER_OFFSET+1];
	cyttsp5_firmware.hw_version = fw->data[HW_VER_OFFSET];
	...
	dev_info(dev, "%s: phone hw ver=0x%02x, tsp hw ver=0x%02x\n",
		__func__, cyttsp5_firmware.hw_version, sti->hw_version);
	dev_info(dev, "%s: phone fw ver=0x%04x, tsp fw ver=0x%04x\n",
		__func__, cyttsp5_firmware.fw_version,
		get_unaligned_be16(&sti->fw_versionh));

	if (cyttsp5_firmware.hw_version != sti->hw_version) {
		dev_info(dev, "%s: hw_version mismatch.\n", __func__);
		goto required;
	}

	if (cyttsp5_firmware.fw_version > get_unaligned_be16(&sti->fw_versionh)) {
		dev_info(dev, "%s: found new firmware.[0x%x]\n",
			__func__, cyttsp5_firmware.fw_version);
		goto required;
	}
#ifdef SAMSUNG_INIT_FW_UPGRADE
	if ((cyttsp5_firmware.fw_version == INIT_FW_VERSION) &&
	     (cyttsp5_firmware.fw_version != get_unaligned_be16(&sti->fw_versionh)) &&
	     (cyttsp5_firmware.hw_version == sti->hw_version)) {
		dev_info(dev, "%s: fw_version is init version. enable force update.\n",
			__func__);
		goto required;
	}
#endif
	dev_dbg(dev, "%s: upgrade not required\n", __func__);
	return 0;
							...
							bool forcedUpgrade)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
#ifdef CY_REQUEST_FIRMWARE_WAIT
	const struct firmware *fw;
#endif
	int retval;
	...
	if (cd->fw_path != NULL) {
#ifdef CY_REQUEST_FIRMWARE_WAIT
		retval = request_firmware(&fw, cd->fw_path, dev);
		_cyttsp5_firmware_cont_binary(fw, dev);
#else

...
#define SAMSUNG_INIT_FW_UPGRADE
#define CYTEST_GETPOWERMODE
#define TOUCH_BOOSTER

//#define CYTTSP5_ENABLE_HARDRESET
//#define CYTTSP5_SECURE_TOUCH

#define DEFAULT_REPORT_RATE		90
#define CY_USE_CAL_FLAG_IN_USER_DATA
...
#define BGA_SILICON_ID		0x0268
#define DEFAULT_SILICON_ID	CSP_SILICON_ID

#define SEC_TSP_NAME "sec_touchscreen"

#define CY_CSP_FW_FILE_NAME "tsp_cypress/cyttsp5_csp.fw"
#define CY_FW_FILE_PATH "/opt/usr/media/cyttsp5_fw.bin"
...
#define TOUCHMODE_FINGER_GLOVE	0x03
#define TOUCHMODE_FINGER_ONLY	0x01
...
enum hid_output {
	...
	HID_OUTPUT_GET_POWER_MODE,
	HID_OUTPUT_GET_FIRMWARE_CRC,
	...
	HID_OUTPUT_GET_CAL_STATUS = 0x30,
...
struct cyttsp5_sfd_self_test_data {
	u8 *buf;
	s16 min;
	s16 max;
};
...
struct cyttsp5_samsung_factory_data {
	...
	struct cyttsp5_sfd_self_test_data mutual_cap;
...
struct cyttsp5_core_nonhid_cmd {
	...
	int (*get_config_row_size) (struct device *dev, int protect,
			u16 *row_size);
	int (*run_self_test) (struct device *dev, int protect,
		u8 test_id, u8 write_idac, u8 *result, u8 *available);
	int (*get_self_test_result) (struct device *dev, int protect,
		u16 read_offset, u16 read_length,u8 test_id,
...
int cyttsp5_get_firmware_crc(struct cyttsp5_core_data *cd, u32* firmware_crc);

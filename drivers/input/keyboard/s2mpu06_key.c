/*
 * Driver for Power keys on s2mpu06 IC by PWRON rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mpu06_keys.c
 * S2MPU06 Keyboard Driver
 *
...
static struct power_keys_platform_data *power_keys_get_devtree_pdata(
		struct s2mpu06_dev *mfd_dev)
{
	/* to do */
	#define S2MPU06_SUPPORT_KEY_NUM	(1)
	...
	key_node = of_find_node_by_name(p_node, "s2mpu06-keys");
	if (!key_node) {
		dev_err(dev, "could not find s2mpu06-keys sub-node\n");
		error = -ENOENT;
		goto err_out;
	}

	nbuttons = of_get_child_count(key_node);
	if (nbuttons > S2MPU06_SUPPORT_KEY_NUM)
		dev_warn(dev, "s2mpu06-keys support only one button.\n");
...
power_keys_get_devtree_pdata(struct s2mpu06_dev *mfd_dev)
{
...
static int power_keys_probe(struct platform_device *pdev)
{
	...
	struct s2mpu06_platform_data *mpdata = s2mpu06->pdata;
	...
	if (!pdata) {
		pdata = power_keys_get_devtree_pdata(s2mpu06);
		if (IS_ERR(pdata))
	...
	input->phys	= "s2mpu06-keys/input0";
	...
	ddata->irq_pwronr = mpdata->irq_base + S2MPU06_PMIC_IRQ_PWRONR_INT1;
	ddata->irq_pwronf = mpdata->irq_base + S2MPU06_PMIC_IRQ_PWRONF_INT1;
...
MODULE_DESCRIPTION("Keyboard driver for s2mpu06");
MODULE_ALIAS("platform:s2mpu06 Power key");

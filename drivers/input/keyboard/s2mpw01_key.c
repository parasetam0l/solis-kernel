...
static DEVICE_ATTR(sec_powerkey_pressed, 0664, key_pressed_show, NULL);
static DEVICE_ATTR(sec_powerkey_pressed_code, 0664, key_pressed_show_code, NULL);
...
static struct attribute *sec_power_key_attrs[] = {
	...
	&dev_attr_sec_powerkey_pressed_code.attr,
...
static void power_keys_power_report_event(struct power_button_data *bdata)
{
...
#ifdef CONFIG_DISPLAY_EARLY_DPMS
	{
		...
		if (!ddata->resume_state && bdata->isr_status && !!state) {
...
#ifdef CONFIG_SEC_SYSFS
	sec_power_key = sec_device_create(ddata, "sec_powerkey");
	if (IS_ERR(sec_power_key))
		dev_err(dev, "%s failed to create sec_powerkey\n", __func__);
...
#ifdef CONFIG_DISPLAY_EARLY_DPMS
	device_set_early_complete(ddata->dev, EARLY_COMP_SLAVE);
#endif
#ifdef CONFIG_SLEEP_MONITOR

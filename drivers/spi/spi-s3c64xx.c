...
#if defined(CONFIG_SENSORS_SSP_BBD) &&	\
		defined(CONFIG_TIZEN_SEC_KERNEL_ENG)
	/* Forcely mark status whether to got msg or not */
	if (master->bus_num == CONFIG_SENSORS_BBD_SPI_PORT)
		msg->status = 1;
#endif
...
#if defined(CONFIG_DISPLAY_EARLY_DPMS) && defined(CONFIG_SENSORS_BBD_SPI_PORT)
	if (sdd->port_id == CONFIG_SENSORS_BBD_SPI_PORT)

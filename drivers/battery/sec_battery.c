...
//#define TEMPERATURE_SW_AGING
//#define FEATURE_NON_CHARGER_BLOCK
...
static struct device_attribute sec_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(factory_image),
	SEC_BATTERY_ATTR(store_mode),
	SEC_BATTERY_ATTR(update),
	SEC_BATTERY_ATTR(fg_switch),
...
#endif
	SEC_BATTERY_ATTR(batt_capacity),
	SEC_BATTERY_ATTR(usb_chg_onoff),
	SEC_BATTERY_ATTR(chg_retail_mode),
	SEC_BATTERY_ATTR(chg_extreme_onoff),
	SEC_BATTERY_ATTR(chg_extreme_mode),
	SEC_BATTERY_ATTR(chg_on),
	SEC_BATTERY_ATTR(chg_suspend),
};
...
extern int s2mpw01_charger_en;
extern int s2mpw01_charger_current;
extern int s2mpw01_charger_float;
extern int s2mpw01_charger_topoff;
...
/* booting check */
unsigned int batt_booting_chk = 0;
				...
				bool enable)
{
	...
	if (enable && !battery->chg_on) {
		pr_err("%s : charging block\n", __func__);
		return -1;
	}
	...
	if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
		value.intval = SEC_INITIAL_MODE;
		psy_do_property("wpc", set,
			POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
	}

	if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
		if (battery->swelling_mode || (battery->status == POWER_SUPPLY_STATUS_NOT_CHARGING)) {
			if (!battery->retail_mode)
				value.intval = SEC_SWELLING_MODE;
			else
				value.intval = SEC_SWELLING_LDU_MODE;
			psy_do_property("wpc", set,
				POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
		} else {
#if defined(CONFIG_POP)
			if (battery->delivered_cable_type == POWER_SUPPLY_TYPE_INCOMPATIBLE) {
				if (battery->status == POWER_SUPPLY_STATUS_FULL) {
					value.intval = SEC_INCOMPATIBLE_TX_FULL_MODE;
					psy_do_property("wpc", set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
				} else if (battery->capacity >= 96) {
					value.intval = SEC_INCOMPATIBLE_TX_HIGH_BATT_MODE;
					psy_do_property("wpc", set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
				} else {
					value.intval = SEC_INCOMPATIBLE_TX_MODE;
					psy_do_property("wpc", set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
				}
			} else if (!battery->retail_mode) {
#else
			if (!battery->retail_mode) {
#endif
				if (battery->status == POWER_SUPPLY_STATUS_CHARGING) {
					if (battery->charging_mode == SEC_BATTERY_CHARGING_NONE) {
						value.intval = SEC_FULL_NONE_MODE;
						psy_do_property("wpc", set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
					} else {
						if (battery->voltage_now < battery->vrect_mid_voltage_threshold) {
							value.intval = SEC_LOW_BATT_MODE;
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						} else if ((battery->voltage_now >= battery->vrect_mid_voltage_threshold) && (battery->voltage_now < battery->vrect_high_voltage_threshold)) {
							value.intval = SEC_MID_BATT_MODE;
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						} else if (battery->voltage_now >= battery->vrect_high_voltage_threshold) {
							value.intval = SEC_HIGH_BATT_MODE;
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						}
					}
					...
					if (battery->delivered_cable_type != POWER_SUPPLY_TYPE_INCOMPATIBLE) {
						if (battery->charging_mode == SEC_BATTERY_CHARGING_NONE) {
							...
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						} else if ((battery->is_recharging == 1) ||
									(battery->charging_mode == SEC_BATTERY_CHARGING_2ND)) {
							value.intval = SEC_FULL_NORMAL_MODE;
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						}
					}
...
static void sec_booting_check_work(struct work_struct *work)
{
	struct sec_battery_info *battery =
		container_of(work, struct sec_battery_info,
			batt_booting_check_work.work);
	union power_supply_propval value;

	psy_do_property("sec-fuelgauge", get,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, value);
	if ((lpcharge == 0) && (value.intval < 3600)) {
		if (battery->booting_check == 0) {
			battery->booting_check = 1;
			schedule_delayed_work(&battery->batt_booting_check_work,
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		/* Add for charging up to 50 % of LDU model
		   Becuase charger on when charger off condidtion */
		if (battery->ldu_charger_on == true) {
			pr_info("%s: battery->ldu_charger_on is true\n", __func__);
			value.intval = SEC_VBAT_MONITORING_DISABLE_MODE;
			psy_do_property("wpc", set,
				POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			sec_bat_set_charge(battery, true);
		}
	}
	wake_unlock(&battery->booting_check_lock);
}
...
static void sec_bat_set_charging_status(struct sec_battery_info *battery,
		int status) {
	union power_supply_propval value;
	int prev_status = battery->status;
	switch (status) {
...
#ifdef CONFIG_ENERGY_MONITOR
	if (battery->status != prev_status)
		energy_monitor_marker(ENERGY_MON_TYPE_BATTERY);
#endif
}

#if defined(CONFIG_WIRELESS_CHARGER_P9220)
static void sec_bat_wpc_mode_check(struct sec_battery_info *battery)
{
	union power_supply_propval value;

	if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
		dev_err(battery->dev, "%s: Charging Disabled [%d]\n",
			__func__, battery->cable_type);
		if (!battery->chg_on) {
			value.intval = P9220_WATCHDOG;
			psy_do_property("wpc", set,
				POWER_SUPPLY_PROP_PRESENT, value);
		}
		return;
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
		if (battery->status == POWER_SUPPLY_STATUS_CHARGING && battery->capacity >= 96) {
		...
		} else if (battery->retail_mode == true) {
#else
		if (battery->retail_mode == true) {
#endif
			psy_do_property("wpc", get,
				POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);

			if ((battery->capacity < battery->pdata->ldu_full_soc) && (value.intval != SEC_FULL_NONE_LDU_MODE)) {
				value.intval = SEC_LOW_BATT_LDU_MODE;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			} else if ((battery->capacity >= battery->pdata->ldu_full_soc) && (value.intval != SEC_FULL_NONE_LDU_MODE)) {
				value.intval = SEC_FULL_NONE_LDU_MODE;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			} else if (battery->capacity < battery->pdata->ldu_recharge_soc) {
				value.intval = SEC_FULL_NORMAL_LDU_MODE;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			}
		} else {
			if (!battery->swelling_mode && (battery->status == POWER_SUPPLY_STATUS_CHARGING)) {
				if (battery->voltage_now < battery->vrect_mid_voltage_threshold) {
					value.intval = SEC_LOW_BATT_MODE;
					psy_do_property("wpc", set,
						POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
					...
					if (battery->delivered_cable_type == POWER_SUPPLY_TYPE_INCOMPATIBLE) {
						...
						if (battery->capacity < 94)
							value.intval = SEC_HIGH_BATT_MODE;
						else if (battery->capacity >= 94)
							value.intval = SEC_VERY_HIGH_BATT_MODE;
					}
					psy_do_property("wpc", set,
						POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
				}
...
#if defined(CONFIG_FORCE_TO_FULL)
			battery->full_start_time = 0;
#endif
#if defined(CONFIG_P9220_VOUT_CONTROL)
			if (battery->vout_toggle == P9220_VOUT_OFF) {
				value.intval = P9220_VOUT_ON;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_PRESENT, value);
				battery->vout_toggle = P9220_VOUT_ON;
			}
#endif
		}
	}
	...
	if (sec_bat_check_recharge(battery)) {
...
#if defined(CONFIG_FORCE_TO_FULL)
		{
			struct timespec ts;
				...
				battery->full_start_time = ts.tv_sec;
		}
#endif
#if defined(CONFIG_P9220_VOUT_CONTROL)
		if (battery->vout_toggle == P9220_VOUT_OFF) {
...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	/* Add for factory mode because no need to turn off charger */
	if (battery->is_jig_on == true) {
		pr_info("%s: skip swelling because jig_is_on\n", __func__);
		return;
	}
	...
	pr_info("%s: status(%d), swell_mode(%d:%d:%lu), cv(%dmV), temp(%d)\n",
		__func__, battery->status, battery->swelling_mode, battery->swelling_mode_cur,
		battery->charging_start_time, val.intval, temperature);
	...
	if ((battery->status == POWER_SUPPLY_STATUS_DISCHARGING) ||\
		...
		battery->swelling_mode_cur = false;
		goto skip_swelling_chek;
	}

	/* 1.  Swelling mode - voltage*/
	if (!battery->swelling_mode) {
	...
	/* 2.  Swelling mode - current*/
	if (!battery->swelling_mode_cur) {
		if (temperature >= battery->pdata->swelling_high_temp_cur_block &&
			battery->pdata->temp_check_type) {
			pr_info("%s: swelling high current-reduce start.\n", __func__);
			battery->swelling_mode_cur = true;
			val.intval = battery->pdata->swelling_high_chg_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
		} else if (temperature <= battery->pdata->swelling_low_temp_cur_block &&
			battery->pdata->temp_check_type) {
			pr_info("%s: swelling low current-reduce start.\n", __func__);
			battery->swelling_mode_cur = true;
	...
	if (battery->swelling_mode_cur) {
		...
		if ((temperature <= battery->pdata->swelling_high_temp_cur_recov) &&
			(temperature >= battery->pdata->swelling_low_temp_cur_recov)) {
			pr_info("%s: swelling current-reduce end. restore charging-current\n", __func__);
			battery->swelling_mode_cur = false;
			val.intval = battery->pdata->swelling_normal_chg_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
		} else if (temperature >= battery->pdata->swelling_high_temp_cur_recov &&
			battery->pdata->temp_check_type && val.intval != battery->pdata->swelling_high_chg_current) {
			val.intval = battery->pdata->swelling_high_chg_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
		} else if (temperature <= battery->pdata->swelling_low_temp_cur_recov &&
			battery->pdata->temp_check_type && val.intval != battery->pdata->swelling_low_chg_current) {
			val.intval = battery->pdata->swelling_low_chg_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
		}
...
#define STOP_AGING_CHECK_LOW_TEMPERATURE 50
static void sec_bat_aging_check(struct sec_battery_info *battery)
{
	...
	if (battery->temperature < STOP_AGING_CHECK_LOW_TEMPERATURE) {
				...
				struct sec_battery_info *battery)
{
	int temp_value;
	int pre_health;
#ifdef FEATURE_NON_CHARGER_BLOCK
	union power_supply_propval val;
#endif
	...
	if ((battery->health == POWER_SUPPLY_HEALTH_OVERHEAT) ||
		...
		if (battery->status != POWER_SUPPLY_STATUS_NOT_CHARGING) {
			...
			dev_info(battery->dev,
				"%s: Unsafe Temperature for check low battery (%d)\n",
				__func__, battery->capacity);
			if ((battery->charging_block)&&(battery->capacity <= 5)) {
				dev_info(battery->dev,
				"%s: Unsafe Temperature, Trun on charger\n", __func__);
				sec_bat_set_charge(battery, true);
				val.intval = (battery->pdata->swelling_low_temp_current / 2);
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_CURRENT_AVG, val);
			} else if ((!battery->charging_block)&&(battery->capacity >= 10)) {
				dev_info(battery->dev,
				"%s: Unsafe Temperature, Trun off charger\n", __func__);
				sec_bat_set_charge(battery, false);
			}
			return false;
#endif /* FEATURE_NON_CHARGER_BLOCK */
		}
	} else {
...
static void sec_bat_do_usb_chg_onoff(
				struct sec_battery_info *battery)
{
	union power_supply_propval value;


	pr_info("%s: battery->usb_chg_onoff : %d\n", __func__, battery->usb_chg_onoff);
	switch (battery->usb_chg_onoff) {
	case 1:
		if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
			(battery->status == POWER_SUPPLY_STATUS_FULL)) {
			sec_bat_set_charge(battery, false);
...
static void sec_bat_do_extreme_chg_onoff(
				struct sec_battery_info *battery)
{
	union power_supply_propval value;

	pr_info("%s: battery->extreme_chg_onoff : %d (%s)\n", __func__,
		battery->extreme_onoff, sec_bat_status_str[battery->status]);
	switch (battery->extreme_onoff) {
	case 1:
		if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
			(battery->status == POWER_SUPPLY_STATUS_FULL)) {
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	if (battery->current_avg < 0 || battery->charging_mode == SEC_BATTERY_CHARGING_NONE)
		discharging_cnt++;
	else
		discharging_cnt = 0;
...
#endif
#if defined(CONFIG_P9220_VOUT_CONTROL)
		if (battery->vout_toggle == P9220_VOUT_ON) {
			value.intval = P9220_VOUT_OFF;
			psy_do_property("wpc", set,
				POWER_SUPPLY_PROP_PRESENT, value);
			battery->vout_toggle = P9220_VOUT_OFF;
		}
#endif
	}
}
...
#if defined(CONFIG_FORCE_TO_FULL)
static bool sec_bat_fullcharged_time_check(
				struct sec_battery_info *battery)
{
	unsigned long charging_time;
	struct timespec ts;

	if (battery->full_start_time == 0) {
		dev_err(battery->dev,
			"%s: no need to check\n", __func__);
		return true;
	}
	...
	if (ts.tv_sec >= battery->full_start_time)
		charging_time = ts.tv_sec - battery->full_start_time;
	else
		charging_time = 0xFFFFFFFF - battery->full_start_time
			+ ts.tv_sec;
	...
	if ((charging_time >= battery->pdata->t_limit_topoff_1st)
		&& (battery->charging_mode == SEC_BATTERY_CHARGING_1ST)) {
			sec_bat_do_fullcharged(battery);
	}
	else if ((charging_time >= battery->pdata->t_limit_topoff_2nd)
		&& (battery->charging_mode == SEC_BATTERY_CHARGING_2ND)) {
			sec_bat_do_fullcharged(battery);
	}
...
#ifdef TEMPERATURE_SW_AGING
static int sec_bat_temperature_aging(void)
{
	static int aging_temp = 250;
	static int aging_dir = 0;
	int add_val;

	if ((aging_temp <= 380) && (aging_temp >= 170)) {
		add_val = 30;
	} else {
		add_val = 5;
	}
...
static int sec_bat_capacity_aging(void)
{
	static int aging_soc = 10;
	static int aging_dir = 0;

	if (aging_dir == 1) {
		pr_info("%s : increase soc (%d) + 1 \n",
			__func__, aging_soc);
		aging_soc += 1;
...
#ifndef TEMPERATURE_SW_AGING
		battery->temperature = value.intval;
#else
		battery->temperature = sec_bat_temperature_aging();
#endif
		psy_do_property(battery->pdata->fuelgauge_name, set,
			POWER_SUPPLY_PROP_TEMP, value);
#if 0
...
#endif
		break;
	case SEC_BATTERY_THERMAL_SOURCE_SEC_THERMISTOR:
		sec_therm_get_temp(TYPE_SEC_THREM_BATTERY, &temp);
		battery->temperature = temp * 10;
		value.intval = battery->temperature;
		psy_do_property(battery->pdata->fuelgauge_name, set,
			POWER_SUPPLY_PROP_TEMP, value);
		break;
				...
				struct sec_battery_info *battery)
{
	...
	if (battery->is_jig_on == true) {
		battery->capacity = 50;
		pr_info("%s: JIG - no need to check SOC(%d, %d)\n", __func__,
				battery->capacity, value.intval);
	} else {
#if defined(CONFIG_PREVENT_SOC_JUMP)
		if (value.intval < 0) {
			pr_info("%s : scaled capacity not ready(%d)\n",
					__func__, value.intval);
		} else {
			battery->scaled_capacity_valid = true;
...
#else
		battery->capacity = value.intval;
#endif /* CONFIG_PREVENT_SOC_JUMP */
	}

#if defined(CONFIG_FORCE_TO_FULL)
	if (((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		(battery->status == POWER_SUPPLY_STATUS_FULL)) &&
		(battery->capacity == 100) && (battery->reach_to_full == 0)) {
		struct timespec ts;

		get_monotonic_boottime(&ts);
		battery->full_start_time = ts.tv_sec;
		battery->reach_to_full = 1;
	}
#endif
#endif /* 0 */

#ifdef TEMPERATURE_SW_AGING
	battery->capacity = sec_bat_capacity_aging();
#endif
...
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
	store_external_load_factor(BATTERY_SOC, battery->capacity);
#endif
...
#endif
#ifdef CONFIG_ENERGY_MONITOR
		energy_monitor_marker(ENERGY_MON_TYPE_BOOTING);
#endif
		boot_1st = 0;
	}
	dev_info(battery->dev,
...
static void sec_bat_program_alarm(
				struct sec_battery_info *battery, int seconds)
{
	dev_info(battery->dev, "%s: polling seconds %d\n",	__func__, seconds);
#if defined(ANDROID_ALARM_ACTIVATED)
	...
	struct sec_battery_info *battery)
{
	...
	/* Do not need for wearable project */
...
#if defined(CONFIG_P9220_VOUT_CONTROL)
				if (battery->cable_type == POWER_SUPPLY_TYPE_WPC)
					/*to check wpc detach, need to poll frequently*/
...
static void sec_bat_swelling_fullcharged_check(struct sec_battery_info *battery)
{
	...
	if (battery->swelling_full_check_cnt >=
		...
		/*  Add following condition for full charging 5 ~ 10 */
		if (battery->capacity >= 96) {
			value.intval = POWER_SUPPLY_STATUS_FULL;
			psy_do_property(battery->pdata->fuelgauge_name, set,
				POWER_SUPPLY_PROP_STATUS, value);
...
static void sec_bat_demo_mode(
				struct sec_battery_info *battery)
{
	union power_supply_propval val;

	pr_info("%s : status (%s) soc(%d)\n",__func__,
		sec_bat_status_str[battery->status], battery->capacity);

	if (battery->status == POWER_SUPPLY_STATUS_CHARGING) {
		pr_debug("%s : (battery->status == POWER_SUPPLY_STATUS_CHARGING)\n"
			,__func__);
		if ((battery->ldu_charger_on == true) && (battery->capacity >= battery->pdata->ldu_full_soc)) {
			pr_info("%s : Turn off charger (%d)\n",__func__, battery->ldu_charger_on);
			battery->ldu_charger_on = false;
			val.intval = POWER_SUPPLY_TYPE_BATTERY;
			psy_do_property(battery->pdata->charger_name, set,
				POWER_SUPPLY_PROP_ONLINE, val);
			psy_do_property(battery->pdata->fuelgauge_name, set,
...
#if defined(CONFIG_P9220_VOUT_CONTROL)
			if (battery->vout_toggle == P9220_VOUT_ON) {
				val.intval = P9220_VOUT_OFF;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_PRESENT, val);
				battery->vout_toggle = P9220_VOUT_OFF;
			}
			alarm_start(&battery->p9220_alive_check_alarm,
					ktime_add(ktime_get_boottime(), ktime_set(P9220_ALIVE_CHECK_INTERVAL_S, 0)));
#endif
		} else if ((battery->ldu_charger_on == false) && (battery->capacity < battery->pdata->ldu_recharge_soc)) {
			pr_info("%s : Turn on charger (%d)\n",__func__, battery->ldu_charger_on);
			battery->ldu_charger_on = true;
			val.intval = battery->cable_type;
#if defined(CONFIG_P9220_VOUT_CONTROL)
			if (battery->vout_toggle == P9220_VOUT_OFF) {
				val.intval = P9220_VOUT_ON;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_PRESENT, val);
				battery->vout_toggle = P9220_VOUT_ON;
			}
		...
		pr_debug("%s : Do not control charger (%s)\n",__func__,
			sec_bat_status_str[battery->status]);
	}
}

#if defined(CONFIG_POP)
static void sec_bat_wpc_auth_check_work(
				struct work_struct *work)
{
	struct sec_battery_info *battery =
		container_of(work, struct sec_battery_info,
		wpc_auth_check_work.work);
	union power_supply_propval value;
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
		...
		if (value.intval == 0) {
			battery->delivered_cable_type = POWER_SUPPLY_TYPE_INCOMPATIBLE;
			if (battery->status == POWER_SUPPLY_STATUS_FULL) {
				value.intval = SEC_INCOMPATIBLE_TX_FULL_MODE;
				psy_do_property("wpc", set,
						POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
	...
	wake_unlock(&battery->wpc_auth_check_lock);
}
#endif
				...
				struct work_struct *work)
{
	...
	if (battery->polling_in_sleep) {
		...
		if ((battery->status == POWER_SUPPLY_STATUS_DISCHARGING) &&
			...
			if ((unsigned long)(c_ts.tv_sec - old_ts.tv_sec) < 10) {
					psy_do_property(battery->pdata->fuelgauge_name, get,
						POWER_SUPPLY_PROP_VOLTAGE_NOW, value);
					battery->voltage_now = value.intval;
					sec_bat_get_temperature_info(battery);
...
#if defined(CONFIG_WIRELESS_CHARGER_P9220)
	sec_bat_wpc_mode_check(battery);
#endif
	/* -2. usb chg on/off*/
	if (battery->cable_type == POWER_SUPPLY_TYPE_USB && battery->usb_chg_onoff) {
		dev_info(battery->dev, "%s: usb chg on/off\n", __func__);
		sec_bat_do_usb_chg_onoff(battery);
	}
	...
	if (battery->usb_chg_onoff != 0)
		goto continue_monitor;

	if (battery->chg_suspend == 0)
		goto continue_monitor;

	if (battery->extreme_mode == true) {
		dev_info(battery->dev, "%s: extreme_mode\n", __func__);
		sec_bat_do_extreme_chg_onoff(battery);
		goto continue_monitor;
	}
	...
	if (!battery->retail_mode) {
		...
		psy_do_property(battery->pdata->charger_name, get,
				POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		psy_do_property(battery->pdata->charger_name, get,
				POWER_SUPPLY_PROP_CURRENT_NOW, val2);
		pr_info("[SWELLING] Stat(%s), cv(%dmV), temp(%d) cur(%dmA) vol_now(%dmV)\n",
			sec_bat_status_str[battery->status], val.intval, battery->temperature, val2.intval, battery->voltage_now);
	} else
		sec_bat_fullcharged_check(battery);
#else
...
#if defined(CONFIG_FORCE_TO_FULL)
	sec_bat_fullcharged_time_check(battery);
#endif
	...
	/*8. demo mode check */
	if (battery->retail_mode == true)
		sec_bat_demo_mode(battery);

continue_monitor:
	/* calculate safety time */
	if (battery->charging_start_time)
		sec_bat_calculate_safety_time(battery);
	/* In single charger path, lcd status == AP wake-up status */
	if (battery->pdata->single_charger_path)
...
#endif
	dev_info(battery->dev,
		 "%s: CHARGER stat(%s) mode(%s) cable(%d) cable_d(%d) en(%d) cur(%d) top(%d) flt(%d) vol(%d) soc(%d) temp(%d) sw(%d) sw_p(%d)"
#if defined(CONFIG_BATTERY_AGE_FORECAST)
		", Cycle(%d)"
#endif
		"\n",
		 __func__, sec_bat_status_str[battery->status], sec_bat_charging_mode_str[battery->charging_mode],
		 battery->cable_type, battery->delivered_cable_type, s2mpw01_charger_en,
		 s2mpw01_charger_current, s2mpw01_charger_topoff, s2mpw01_charger_float,
		 battery->voltage_now, battery->capacity, battery->temperature,
		 battery->swelling_mode, battery->swelling_partial
#if defined(CONFIG_BATTERY_AGE_FORECAST)
		 , battery->batt_cycle
#endif
		 );
...
#if defined(CONFIG_PREVENT_SOC_JUMP)
	if (battery->scaled_capacity_valid)
#endif
	power_supply_changed(&battery->psy_bat);
#if defined(CONFIG_WIRELESS_CHARGER_P9220)
	if (battery->dynamic_mode == true) {
		if ((battery->status == POWER_SUPPLY_STATUS_FULL) && (battery->capacity == 100)) {
			value.intval = battery->status;
			psy_do_property("wpc", set,
				POWER_SUPPLY_PROP_STATUS, value);
			battery->dynamic_mode = false;
		}
	}

	if (battery->factory_image == true) {
		if (battery->capacity >= 85) {
				value.intval = POWER_SUPPLY_STATUS_FULL;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_STATUS, value);
		}
...
#if defined(CONFIG_P9220_VOUT_CONTROL)
static enum alarmtimer_restart sec_bat_p9220_alive_check(
	struct alarm *alarm, ktime_t now)
{
	struct sec_battery_info *battery = container_of(alarm,
				struct sec_battery_info, p9220_alive_check_alarm);

	pr_info("%s\n", __func__);

	if (battery->vout_toggle == P9220_VOUT_OFF) {
		wake_lock(&battery->p9220_alive_check_lock);
		schedule_delayed_work(&battery->p9220_alive_check_work, 0);
	}

	return ALARMTIMER_NORESTART;
}

static void sec_bat_p9220_alive_check_work(
				struct work_struct *work)
{
	struct sec_battery_info *battery =
		container_of(work, struct sec_battery_info,
		p9220_alive_check_work.work);
	union power_supply_propval value;
	...
	wake_unlock(&battery->p9220_alive_check_lock);
}
#endif
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if ((current_cable_type == battery->cable_type) && !battery->slate_mode) {
		dev_info(battery->dev,
				...
				__func__, battery->cable_type);
		if (!battery->need_to_reset_cable)
	...
	battery->delivered_cable_type = current_cable_type;
	if (battery->cable_type == POWER_SUPPLY_TYPE_WIRELESS)
		power_supply_changed(&battery->psy_bat);
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
		...
		battery->delivered_cable_type = POWER_SUPPLY_TYPE_BATTERY;
	...
	queue_delayed_work_on(0, battery->monitor_wqueue, &battery->monitor_work, 0);
end_of_cable_work:
	battery->need_to_reset_cable = false;
...
static void sec_bat_fuelgauge_switch(struct sec_battery_info *battery, int val)
{
	dev_info(battery->dev, "%s : val = %d\n", __func__, val);

	if (battery->pdata->gpio_fuelgauge_switch &&
			battery->pdata->gpio_fg_switch_enable_setting == gpio_get_value(battery->pdata->gpio_fuelgauge_switch)) {
		gpio_set_value(battery->pdata->gpio_fuelgauge_switch, !battery->pdata->gpio_fg_switch_enable_setting);
		msleep(100);
	}

	if (battery->pdata->gpio_fuelgauge_switch2 &&
			battery->pdata->gpio_fg_switch2_enable_setting == gpio_get_value(battery->pdata->gpio_fuelgauge_switch2)) {
		gpio_set_value(battery->pdata->gpio_fuelgauge_switch2, !battery->pdata->gpio_fg_switch2_enable_setting);
		msleep(100);
	}

	if (val == V_BATTERY && battery->pdata->gpio_fuelgauge_switch) {
		gpio_set_value(battery->pdata->gpio_fuelgauge_switch, battery->pdata->gpio_fg_switch_enable_setting);
		msleep(100);
	}

	if (val == V_SYS && battery->pdata->gpio_fuelgauge_switch2) {
		gpio_set_value(battery->pdata->gpio_fuelgauge_switch2, battery->pdata->gpio_fg_switch2_enable_setting);
		msleep(100);
	}
}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_TEMP_ADC:
		...
		case SEC_BATTERY_THERMAL_SOURCE_FG:
			...
			break;
		case SEC_BATTERY_THERMAL_SOURCE_SEC_THERMISTOR:
			sec_therm_get_adc(TYPE_SEC_THREM_BATTERY, &battery->temp_adc);
			break;
		default:
			break;
		}
	...
	case FG_SWITCH:
		if (battery->pdata->gpio_fuelgauge_switch) {
			int tmp;
			tmp = gpio_get_value(battery->pdata->gpio_fuelgauge_switch);
			i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", tmp);
		}
		break;
	case TEST_MODE:
	...
	case BATT_CAPACITY:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			battery->pdata->batt_capacity);
		break;
	case BATT_USB_CHG_ONFF:
		dev_info(battery->dev,"%s: BATT_USB_CHG_ONFF\n", __func__);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			battery->usb_chg_onoff);
		break;
	case CHG_RETAIL_MODE:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			(battery->retail_mode == true) ? 1 : 0);
		break;
	case CHG_EXTREME_ONOFF:
		dev_info(battery->dev,"%s: CHG_EXTREME_ONOFF\n", __func__);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			battery->extreme_onoff);
		break;
	case CHG_EXTREME_MODE:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			(battery->extreme_mode == true) ? 1 : 0);
		break;
	case CHG_ON:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", battery->chg_on);
		break;
					...
					const char *buf, size_t count)
{
	...
	case FACTORY_IMAGE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			union power_supply_propval value;
			battery->factory_image = x ? true : false;
			ret = count;
			if (x == 2) {
				value.intval = POWER_SUPPLY_STATUS_FULL;
#if defined(CONFIG_WIRELESS_CHARGER_P9220)
	...
	case UPDATE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			union power_supply_propval value;
			psy_do_property(battery->pdata->fuelgauge_name, set,
					POWER_SUPPLY_PROP_CALIBRATE, value);
			dev_info(battery->dev,"Reset SOC\n");
	...
	case FG_SWITCH:
		if (sscanf(buf, "%d\n", &x) == 1) {
			sec_bat_fuelgauge_switch(battery, x);
			ret = count;
		}
		break;
	case TEST_MODE:
	...
	case BATT_USB_CHG_ONFF:
		if (sscanf(buf, "%d\n", &x) == 1) {
			battery->usb_chg_onoff = x;// ? true : false;
			wake_lock(&battery->monitor_wake_lock);
			queue_delayed_work(battery->monitor_wqueue,
				&battery->monitor_work, 0);
			ret = count;
			dev_info(battery->dev,"%s: BATT_USB_CHG_ONFF , x : %d \n", __func__, x);
		}
		break;
	case CHG_RETAIL_MODE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			...
			dev_info(battery->dev,
					"%s: CHG_RETAIL_MODE(%d)\n", __func__, x);
			if (x) {
				battery->retail_mode = true;
				value.intval = 1;
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_CHARGE_AVG, value);
				dev_info(battery->dev,
						"%s: changed to retail mode\n", __func__);
			} else {
				battery->retail_mode = false;
				value.intval = 0;
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_CHARGE_AVG, value);
				dev_info(battery->dev,
						"%s: changed to normal mode\n", __func__);
			}
			ret = count;
		}
		break;
	case CHG_EXTREME_ONOFF:
		if (sscanf(buf, "%d\n", &x) == 1) {
			battery->extreme_onoff = x;// ? true : false;
			wake_lock(&battery->monitor_wake_lock);
			queue_delayed_work(battery->monitor_wqueue,
				&battery->monitor_work, 0);
			ret = count;
			dev_info(battery->dev,"%s: CHG_EXTREME_ONOFF , x : %d \n", __func__, x);
		}
		break;
	case CHG_EXTREME_MODE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			union power_supply_propval value;
			dev_info(battery->dev,
					"%s: CHG_EXTREME_MODE(%d)\n", __func__, x);
			if (x) {
				battery->extreme_mode = true;
				value.intval = 1;
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_POWER_AVG, value);

				dev_info(battery->dev,
						"%s: changed to extream mode\n", __func__);
			} else {
				battery->extreme_mode = false;
				value.intval = 0;
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_POWER_AVG, value);
				dev_info(battery->dev,
	...
	case CHG_ON:
		if (sscanf(buf, "%d\n", &x) == 1) {
			dev_info(battery->dev, "%s: CHG_ON (%d)\n", __func__, x);
			if (x == 0) {
				battery->chg_on = false;
#if defined(CONFIG_FORCE_TO_FULL)
				battery->full_start_time = 0;
				battery->reach_to_full = 0;
#endif
...
#endif
				wake_lock(&battery->cable_wake_lock);
				battery->need_to_reset_cable = true;
				queue_delayed_work_on(0, battery->monitor_wqueue,
					   &battery->cable_work, 0);
			}
			ret = count;
	...
	case CHG_SUSPEND:
		if (sscanf(buf, "%d\n", &x) == 1) {
			dev_info(battery->dev, "%s: CHG_SUSPEND (%d)\n", __func__, x);
			if (x == 0) {
				battery->chg_suspend = 0;
			} else if (x == 1) {
				battery->chg_suspend = 1;
			}
			ret = count;
		}
		break;
				...
				union power_supply_propval *val)
{
...
#if defined(CONFIG_WIRELESS_CHARGER_P9220)
		if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
			if (battery->charging_passed_time >= battery->pdata->wpc_auth_check_time) {
				psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX, value);
				if (value.intval == 0) {
					val->intval = POWER_SUPPLY_TYPE_INCOMPATIBLE;
					battery->delivered_cable_type = val->intval;
				} else {
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (!lpcharge)
			pr_info("%s : capacity(%d) kernel capacity(%d) %s:%d\n", __func__, val->intval, battery->capacity, current->comm, current->pid);
		break;
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_TYPE_INCOMPATIBLE:
		val->intval = 1;
		break;
	default:
		val->intval = 0;
	...
	int check_level, int caller_type)
{
	...
	if (caller_type == SLEEP_MONITOR_CALL_INIT) {
		val.intval = 1; /* SEC_FUELGAUGE_CAPACITY_TYPE_RAW */
	} else {
		val.intval = 0;
	}
				...
				muic_attached_dev_t attached_dev)
{
	...
	case ATTACHED_DEV_WIRELESS_TA_MUIC:
		current_cable_type = POWER_SUPPLY_TYPE_WPC;
		break;
	default:
		...
		unsigned long action, void *data)
{
	...
	if (cable_type == POWER_SUPPLY_TYPE_WPC) {
		cancel_delayed_work(&battery->wpc_auth_check_work);
		wake_lock(&battery->wpc_auth_check_lock);
		schedule_delayed_work(&battery->wpc_auth_check_work, msecs_to_jiffies(5000));
	}
#endif
		...
		struct sec_battery_info *battery)
{
	...
	if (ret)
		pr_info("%s: chip_vendor is Empty\n", __func__);
	...
	ret = of_property_read_u32(np, "battery,batt_capacity", &pdata->batt_capacity);
	if(ret) {
		pr_info("%s : batt size is Empty, %d\n", __func__, ret);
		pdata->batt_capacity = 0;
	}
	...
	ret = of_property_read_u32(np, "battery,swelling_high_temp_cur_block",
				   &temp);
	pdata->swelling_high_temp_cur_block = (int)temp;
	if (ret)
		pr_info("%s: swelling high temp current block is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_high_temp_cur_recov",
				   &temp);
	pdata->swelling_high_temp_cur_recov = (int)temp;
	if (ret)
		pr_info("%s: swelling high temp current recovery is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_low_temp_cur_block",
				   &temp);
	pdata->swelling_low_temp_cur_block = (int)temp;
	if (ret)
		pr_info("%s: swelling low temp current block is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_low_temp_cur_recov",
				   &temp);

	pdata->swelling_low_temp_cur_recov = (int)temp;
	if (ret)
		pr_info("%s: swelling low temp current recovery is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_low_chg_current",
					&pdata->swelling_low_chg_current);
	if (ret) {
		pr_info("%s: swelling_low_chg_current is Empty, default value 75mA\n", __func__);
		pdata->swelling_low_chg_current = 75;
	}

	ret = of_property_read_u32(np, "battery,swelling_normal_chg_current",
					&pdata->swelling_normal_chg_current);
	if (ret) {
		pr_info("%s: swelling_normal_chg_current is Empty, default value 150mA\n", __func__);
		pdata->swelling_normal_chg_current = 150;
	}

	ret = of_property_read_u32(np, "battery,swelling_high_chg_current",
					&pdata->swelling_high_chg_current);
	if (ret) {
		pr_info("%s: swelling_high_chg_current is Empty, default value 75mA\n", __func__);
		pdata->swelling_high_chg_current = 75;
	}

	ret = of_property_read_u32(np, "battery,swelling_low_temp_current",
					&pdata->swelling_low_temp_current);
	if (ret) {
		pr_info("%s: swelling_low_temp_current is Empty, Defualt value 100mA \n", __func__);
		pdata->swelling_low_temp_current = 150;
	}

	ret = of_property_read_u32(np, "battery,swelling_low_temp_topoff",
					&pdata->swelling_low_temp_topoff);
	if (ret) {
		pr_info("%s: swelling_low_temp_topoff is Empty, Defualt value 20mA \n", __func__);
		pdata->swelling_low_temp_topoff = 20;
	}

	ret = of_property_read_u32(np, "battery,swelling_high_temp_current",
					&pdata->swelling_high_temp_current);
	if (ret) {
		pr_info("%s: swelling_high_temp_current is Empty, Defualt value 150mA \n", __func__);
		pdata->swelling_high_temp_current = 150;
	}

	ret = of_property_read_u32(np, "battery,swelling_high_temp_topoff",
					&pdata->swelling_high_temp_topoff);
	if (ret) {
		pr_info("%s: swelling_high_temp_topoff is Empty, Defualt value 20mA \n", __func__);
		pdata->swelling_high_temp_topoff = 20;
	}
	...
	pr_info("%s : SWELLING_HIGH_TEMP_BLOCK(%d) SWELLING_HIGH_TEMP_RECOV(%d)\n"
		"SWELLING_LOW_TEMP_BLOCK(%d) SWELLING_LOW_TEMP_RECOV(%d)\n"
		"SWELLING_HIGH_TEMP_CUR_BLOCK(%d) SWELLING_HIGH_TEMP_CUR_RECOV(%d)\n"
		"SWELLING_LOW_TEMP_CUR_BLOCK(%d) SWELLING_LOW_TEMP_CUR_RECOV(%d)\n"
		...
		pdata->swelling_high_temp_cur_block, pdata->swelling_high_temp_cur_recov,
		pdata->swelling_low_temp_cur_block, pdata->swelling_low_temp_cur_recov,
		pdata->swelling_high_chg_current, pdata->swelling_low_chg_current);
	...
	pdata->gpio_fuelgauge_switch = of_get_named_gpio(np, "fg_switch", 0);
	ret = of_property_read_u32(np, "fg_switch_enable_setting", &pdata->gpio_fg_switch_enable_setting);
	if (pdata->gpio_fuelgauge_switch < 0 || ret) {
		pr_err("%s: failed to get gpio fuelgauge switch%d\n", __func__, pdata->gpio_fuelgauge_switch);
		pdata->gpio_fuelgauge_switch = 0;
	}
	if (pdata->gpio_fuelgauge_switch) {
		ret = gpio_request_one(pdata->gpio_fuelgauge_switch,
				GPIOF_DIR_OUT, "fuelgauge_switch");
		if (ret < 0) {
			pdata->gpio_fuelgauge_switch = 0;
			pr_err("%s:fuelgauge_switch request fail %d\n", __func__, ret);
		} else
			gpio_set_value(pdata->gpio_fuelgauge_switch,
					!pdata->gpio_fg_switch_enable_setting);
	}
	pdata->gpio_fuelgauge_switch2 = of_get_named_gpio(np, "fg_switch2", 0);
	ret = of_property_read_u32(np, "fg_switch2_enable_setting", &pdata->gpio_fg_switch2_enable_setting);
	if (pdata->gpio_fuelgauge_switch2 < 0 || ret) {
		pr_err("%s: failed to get gpio fuelgauge switch%d\n", __func__, pdata->gpio_fuelgauge_switch2);
		pdata->gpio_fuelgauge_switch2 = 0;
	}
	if (pdata->gpio_fuelgauge_switch2) {
		ret = gpio_request_one(pdata->gpio_fuelgauge_switch2,
				GPIOF_DIR_OUT, "fuelgauge_switch2");
		if (ret < 0) {
			pdata->gpio_fuelgauge_switch2 = 0;
			pr_err("%s:fuelgauge_switch2 request fail %d\n", __func__, ret);
		} else
			gpio_set_value(pdata->gpio_fuelgauge_switch2,
					pdata->gpio_fg_switch2_enable_setting);
	}

	ret = of_property_read_u32(np, "battery,vrect-mid-voltage", &pdata->vrect_mid_voltage_threshold);
	if (ret) {
		pr_err("%s: failed to get vrect-mid-voltage %d\n", __func__, ret);
		pdata->vrect_mid_voltage_threshold = 0;
	}

	ret = of_property_read_u32(np, "battery,vrect-high-voltage", &pdata->vrect_high_voltage_threshold);
	if (ret) {
		pr_err("%s: failed to get vrect-high-voltage %d\n", __func__, ret);
		pdata->vrect_high_voltage_threshold = 0;
	}
	...
	ret = of_property_read_u32(np, "battery,ldu_full_soc", &temp);
	if (ret) {
		pr_info("ldu full soc is empty\n");
		pdata->ldu_full_soc = 50;
	} else
		pdata->ldu_full_soc = (int)temp;

	ret = of_property_read_u32(np, "battery,ldu_recharge_soc", &temp);
	if (ret) {
		pr_info("ldu recharge soc is empty\n");
		pdata->ldu_recharge_soc = 40;
	} else
		pdata->ldu_recharge_soc = (int)temp;

	ret = of_property_read_u32(np, "battery,wpc_auth_check_time", &temp);
	if (ret) {
		pr_info("wpc auth check time is empty\n");
		pdata->wpc_auth_check_time = 60;
	} else
		pdata->wpc_auth_check_time = (int)temp;

#if defined(CONFIG_FORCE_TO_FULL)
	ret = of_property_read_u32(np, "battery,t_limit_topoff_1st", &temp);
	if (ret) {
		pr_info("topoff 1st time limit empty. default 20 mins\n");
		pdata->t_limit_topoff_1st = 20 * 60;
	} else
		pdata->t_limit_topoff_1st = (int)temp;

	ret = of_property_read_u32(np, "battery,t_limit_topoff_2nd", &temp);
	if (ret) {
		pr_info("topoff 2nd time limit empty. default 5 mins\n");
		pdata->t_limit_topoff_2nd = 5 * 60;
	} else
		pdata->t_limit_topoff_2nd = (int)temp;
#endif
...
static int __devinit sec_battery_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&battery->siop_wake_lock, WAKE_LOCK_SUSPEND,
		       "sec-bat-siop");
	wake_lock_init(&battery->booting_check_lock, WAKE_LOCK_SUSPEND,
		       "sec-bat-bootchk");
#if defined(CONFIG_POP)
	wake_lock_init(&battery->wpc_auth_check_lock, WAKE_LOCK_SUSPEND,
		       "sec-bat-wpc-auth-check");
#endif
	...
	battery->swelling_mode = false;
	battery->swelling_mode_cur = false;
	battery->swelling_partial = false;
#endif
	...
	battery->dynamic_mode = false;
#if defined(CONFIG_PREVENT_SOC_JUMP)
	battery->scaled_capacity_valid = false;
#endif
#if defined(CONFIG_BATTERY_AGE_FORECAST)
	...
	if (battery->pdata->vrect_mid_voltage_threshold)
		battery->vrect_mid_voltage_threshold = battery->pdata->vrect_mid_voltage_threshold;
	else
		battery->vrect_mid_voltage_threshold = DEFAULT_VRECT_MID_VOLTAGE_THRESHOLD;

	if (battery->pdata->vrect_high_voltage_threshold)
		battery->vrect_high_voltage_threshold = battery->pdata->vrect_high_voltage_threshold;
	else
		battery->vrect_high_voltage_threshold = DEFAULT_VRECT_HIGH_VOLTAGE_THRESHOLD;
...
#if defined(CONFIG_POP)
	INIT_DELAYED_WORK(&battery->wpc_auth_check_work, sec_bat_wpc_auth_check_work);
#endif

	INIT_DELAYED_WORK(&battery->batt_booting_check_work, sec_booting_check_work);
	wake_lock_timeout(&battery->booting_check_lock, HZ * 90);
	schedule_delayed_work(&battery->batt_booting_check_work,
...
#if defined(CONFIG_P9220_VOUT_CONTROL)
	alarm_init(&battery->p9220_alive_check_alarm, ALARM_BOOTTIME,
			sec_bat_p9220_alive_check);
	INIT_DELAYED_WORK(&battery->p9220_alive_check_work, sec_bat_p9220_alive_check_work);
	wake_lock_init(&battery->p9220_alive_check_lock, WAKE_LOCK_SUSPEND,
		       "sec-bat-p9220-alive-check-lock");
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(&battery->psy_bat, &bat_sleep_monitor_ops,
				SLEEP_MONITOR_BATTERY);
#endif
...
err_wake_lock:
	...
	wake_lock_destroy(&battery->booting_check_lock);
#if defined(CONFIG_POP)
	wake_lock_destroy(&battery->wpc_auth_check_lock);
#endif
#if defined(CONFIG_P9220_VOUT_CONTROL)
	wake_lock_destroy(&battery->p9220_alive_check_lock);
#endif

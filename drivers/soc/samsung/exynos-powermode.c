...
static bool cpuidle_sicd_en;
static bool cpuidle_sicd_jig_check_en = 1;

void cpuidle_set_sicd_en(bool en)
{
	cpuidle_sicd_en = en;
}

bool cpuidle_get_sicd_en(void)
{
	return cpuidle_sicd_en;
}

void cpuidle_set_sicd_jig_check_en(bool en)
{
	cpuidle_sicd_jig_check_en = en;
}

bool cpuidle_get_sicd_jig_check_en(void)
{
	return cpuidle_sicd_jig_check_en;
}

extern int muic_get_jig_state(void);
...
static int is_sicd_available(unsigned int cpu)
{
	...
	if (cpuidle_get_sicd_jig_check_en() && muic_get_jig_state())
		return false;
...
{							\
	...
	if (!sscanf(buf, "%1d", &input))		\
		return -EINVAL;				\
							\
	pr_info("%s: %d -> %d (%s)\n", \
			__func__, powermode_info->object, !!input, current->comm); \
							\

...
config:
	...
	if (frame->shot) {
		ret = fimc_is_lib_isp_set_ctrl(hw_ip, &hw_tpu->lib[param_set->instance], frame);
		if (ret)
	...
	u32 lindex, u32 hindex, u32 instance, ulong hw_map)
{
	...
	if (param->control.cmd == CONTROL_COMMAND_STOP) {
		if (param->control.bypass == CONTROL_BYPASS_ENABLE)
			fimc_is_hw_s_ctrl(hw_ip->itfc, hw_ip->id, HW_S_CTRL_FULL_BYPASS, (void *)true);
		else
			fimc_is_hw_s_ctrl(hw_ip->itfc, hw_ip->id, HW_S_CTRL_FULL_BYPASS, (void *)false);
	} else if (param->control.cmd == CONTROL_COMMAND_START) {
		fimc_is_hw_s_ctrl(hw_ip->itfc, hw_ip->id, HW_S_CTRL_FULL_BYPASS, (void *)false);
	}

...
static void fimc_is_hw_mcsc_adjust_input_stride(u32 width, u32 plane, bool conv420_flag,
...
static int fimc_is_hw_mcsc_rdma_cfg(struct fimc_is_hw_ip *hw_ip, struct fimc_is_frame *frame)
{
	...
	/* DMA in */
	dbg_hw("[%d]mcsc_rdma_cfg [F:%d][addr: %x]\n",
...
skip_addr:
	...
	/* DMA out */
	for (i = MCSC_OUTPUT0; i < cap->max_output; i++) {
		if (cap->out_dma[i] != MCSC_CAP_SUPPORT)
			continue;

		dbg_hw("[%d]mcsc_wdma_cfg [F:%d][T:%d][addr%d: %x]\n",
	...
	u32 instance)
{
	...
	fimc_is_hw_mcsc_adjust_input_stride(width, plane, false,
		&y_stride, &uv_stride);
	...
	u32 output_id, u32 instance)
{
	...
	fimc_is_scaler_set_hwfc_config(hw_ip->regs, output_id, format, plane, 0, width, height);

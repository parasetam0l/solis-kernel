	...
	struct v4l2_ext_controls *ctrls)
{
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		case V4L2_CID_IS_CANCEL_CAPTURE:
			{
				struct fast_ae *fast_ae =
					(struct fast_ae *)&device->is_region->fast_ctl.fast_ae;
				ret = copy_from_user(fast_ae, ext_ctrl->ptr, sizeof(struct fast_ae));
				if (ret) {
				...
				vb2_ion_sync_for_device(
					...
					(ulong)fast_ae - device->imemory.kvaddr,
					sizeof(struct fast_ae),
					DMA_TO_DEVICE);

				mvinfo("Fast cancel control(flag %d)\n", vctx, vctx->video, fast_ae->cancel_capture);
			}
			break;
#endif
		default:

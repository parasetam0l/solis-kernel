...
#include "fimc-is-cis-4e6-setA.h"
#include "fimc-is-cis-4e6-setB.h"
...
int sensor_4e6_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	ret = fimc_is_sensor_write16_array(client, 0x0222, dgains, 4);
	if (ret < 0)
		goto p_err;
...
int cis_4e6_probe(struct i2c_client *client,
...
static int cis_4e6_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_4e6_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-4e6",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_4e6_match);

static const struct i2c_device_id cis_4e6_idt[] = {
	{ SENSOR_NAME, 0 },
};

static struct i2c_driver cis_4e6_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_4e6_match
	},
	.probe	= cis_4e6_probe,
	.remove	= cis_4e6_remove,
	.id_table = cis_4e6_idt
};
module_i2c_driver(cis_4e6_driver);

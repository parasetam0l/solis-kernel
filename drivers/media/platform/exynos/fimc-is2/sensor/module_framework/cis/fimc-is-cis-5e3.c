...
static int cis_5e3_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_5e3_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-5e3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_5e3_match);
...
static struct i2c_driver cis_5e3_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_5e3_match
	},
	.probe	= cis_5e3_probe,
	.remove	= cis_5e3_remove,
	.id_table = cis_5e3_idt
};
module_i2c_driver(cis_5e3_driver);

...
#endif
#ifdef CONFIG_DRM_DMA_SYNC
	BUG_ON(svb->mb.vb.planes[0].dbuf == NULL);
#endif
	BUG_ON(!ctx->m2m_ctx);
...
#endif
#ifdef CONFIG_DRM_DMA_SYNC
	fence = tdm_fence(ctx->sc_dev->fence_dev, svb->mb.vb.planes[0].dbuf);
	if (!fence)
		dev_warn(ctx->sc_dev->dev, "failed to get fence:buf\n");

	svb->mb.vb.fence = fence;
#endif
			    ...
			    struct v4l2_requestbuffers *reqbufs)
{
	...
	if (IS_ENABLED(CONFIG_SYNC) || IS_ENABLED(CONFIG_DRM_DMA_SYNC)) {
...
#ifdef CONFIG_DRM_DMA_SYNC
	vb->fence_dev = ctx->sc_dev->fence_dev;
#endif
...
#ifdef CONFIG_SYNC
	if (IS_ENABLED(CONFIG_SYNC) && vb->acquire_fence) {
#endif
#ifdef CONFIG_DRM_DMA_SYNC
	if (IS_ENABLED(CONFIG_DRM_DMA_SYNC) && vb->planes[0].dbuf) {
#endif
...
static void sc_vb2_stop_streaming(struct vb2_queue *vq)
{
	struct sc_ctx *ctx = vb2_get_drv_priv(vq);
	int ret;
#if defined (CONFIG_SYNC) || defined (CONFIG_DRM_DMA_SYNC)
	struct vb2_buffer *vb = vq->bufs[0];
	struct v4l2_m2m_buffer *mb = container_of(vb, typeof(*mb), vb);
	struct vb2_sc_buffer *svb = container_of(mb, typeof(*svb), mb);
...
#ifdef CONFIG_DRM_DMA_SYNC
static int sc_notifier_ctrl(struct notifier_block *this,
		unsigned long cmd, void *_data)
{
	struct sc_dev *sc = container_of(this,
			struct sc_dev, nb_ctrl);
...
#ifdef CONFIG_DRM_DMA_SYNC
	sc->nb_ctrl.notifier_call = sc_notifier_ctrl;
	if (tdm_nb_register(&sc->nb_ctrl)) {
		dev_err(&pdev->dev,"%s: could not register notify callback\n",
				__func__);
		goto err_ver_aclk_get;
	}
#endif

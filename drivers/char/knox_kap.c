...
static int knox_kap_read(struct seq_file *m, void *v)
{
	...
	if (tz_ret == (KAP_MAGIC | 3)) {
		/* RKP and/or DMVerity says device is tampered */
		ret_val = 0x03;
	} else if (tz_ret == (KAP_MAGIC | 1)) {
		/* KAP is ON */
		if (kap_off_reboot == 1) {
			/* KAP is ON and will turn OFF upon next reboot */
			ret_val = 0x10;
		} else {
			/* KAP is ON and will stay ON */
			ret_val = 0x11;
		}
	} else if (tz_ret == (KAP_MAGIC)) {
		/* KAP is OFF */
		if (kap_on_reboot == 1) {
			/* KAP is OFF but will turn on upon next reboot */
			ret_val = 0x01;
		} else {
			/* KAP is OFF and will stay OFF upon next reboot */
			ret_val = 0;
		}
	} else {
		/* The magic string is not there. KAP mode not implemented */
		ret_val = 0x04;
	}

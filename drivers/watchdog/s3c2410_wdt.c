...
static int g_kicktimer_state;
static struct timer_list watchdog_timer;
static unsigned long g_watchdog_kick_count;
static unsigned long g_wdt_con;
#endif
...
static void watchdog_timer_handler(unsigned long data)
{
	...
	if(value_for_debug == 1) {
		pr_info("s3c2410wdt_keepalive disable\n");
		return;
	}

	s3c2410wdt_keepalive(&wdt->wdt_device);

	if (g_watchdog_kick_count < 4000000000UL)
		g_watchdog_kick_count++;
	else
		g_watchdog_kick_count = 0;

	mod_timer(&watchdog_timer, (jiffies + 2*HZ));
}
#endif


#if defined(CONFIG_ARM_S3C24XX_CPUFREQ) && !defined(CONFIG_SEC_WATCHDOG_RESET)
...
#ifdef CONFIG_SEC_WATCHDOG_RESET
	if (tmr_atboot && started == 0) {
		setup_timer(&watchdog_timer, watchdog_timer_handler, (unsigned long)pdev);
		mod_timer(&watchdog_timer, (jiffies + 2*HZ));
		g_kicktimer_state = WDT_KICKTIMER_ON;
	}
#endif
...
#ifdef CONFIG_SEC_WATCHDOG_RESET
	if (g_kicktimer_state) {
		del_timer(&watchdog_timer);
		g_kicktimer_state = WDT_KICKTIMER_OFF;
		g_wdt_con = 0;
	}
#endif
...
#ifdef CONFIG_SEC_WATCHDOG_RESET
	if (g_wdt_con)
		writel(g_wdt_con, wdt->reg_base + S3C2410_WTCON);
	else
		writel(wdt->wtcon_save, wdt->reg_base + S3C2410_WTCON);
	pr_info("watchdog %sabled\n",
		(g_wdt_con & S3C2410_WTCON_ENABLE) ? "en" : "dis");
	if (g_kicktimer_state)
		mod_timer(&watchdog_timer, (jiffies + 2*HZ));
#else
	writel(wdt->wtcon_save, wdt->reg_base + S3C2410_WTCON);

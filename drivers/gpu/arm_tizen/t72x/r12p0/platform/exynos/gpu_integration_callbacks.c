...
#ifdef MALI_TIZEN_MEM_INFO
	kctx->ctx_pid = task_pid_nr(current);
	kctx->ctx_tgid = task_tgid_nr(current);
#endif
...
int gpu_memory_seq_show(struct seq_file *sfile, void *data)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		ret = seq_printf(sfile, "===========================================================\n\n");
#ifdef MALI_TIZEN_MEM_INFO
		ret = seq_printf(sfile, "%8s %8s %26s     %20s  %16s  %12s\n",
				"pid", "tgid", "context name", "context addr",
				"used pages", "shrink pages");
		ret = seq_printf(sfile, "===================================");
		ret = seq_printf(sfile, "=============================");
#else
...
#ifdef MALI_TIZEN_MEM_INFO
			ret = seq_printf(sfile,
				"%8d %8d  (%24s), %s-0x%p    %12u  %10zu\n",
				element->kctx->ctx_pid, element->kctx->ctx_tgid,
				element->kctx->name, "kctx", element->kctx,

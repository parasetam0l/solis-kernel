...
static void ion_dma_buf_release(struct dma_buf *dmabuf)
{
	...
	if (buffer->private_ops) {
		if (!buffer->private_obj)
			pr_err("%s:failed to find private_obj:buf[%p %d]\n", __func__,
				buffer, atomic_read(&buffer->ref.refcount));
		else {
			if (atomic_read(&buffer->ref.refcount) > 1)
				pr_info("%s:buf[%p %d]\n", __func__,
					buffer, atomic_read(&buffer->ref.refcount));
			buffer->private_ops(buffer->private_obj);
		}
	}
#endif
}
			  ...
			  unsigned long dummy, void *parm)
{
	...
	pr_info("-------------ION SYSTEM HEAP USAGE---------------\n");
	pr_info("%4.s %16.s %10.s %6.s\n", "pid", "task", "size", "flag");
	pr_info("-------------------------------------------------\n");
	for (n = rb_first(&g_idev->buffers); n; n = rb_next(n)) {
		struct ion_buffer *buffer = rb_entry(n, struct ion_buffer,
						     node);
		total_size += buffer->size;
		if (buffer->heap->type == ION_HEAP_TYPE_SYSTEM)
			pr_info("%4u %16.s %10zu %6lx\n",
				buffer->pid, buffer->task_comm,
				buffer->size, buffer->flags);
	}

	pr_info("-------------------------------------------------\n");
	pr_info("[%16.s : %16zu]\n", "ION TOTAL SIZE", total_size);
	pr_info("-------------------------------------------------\n");
	return NOTIFY_OK;
}
...
static int ion_oom_notify(struct notifier_block *self,
			  unsigned long dummy, void *parm)
{
	struct ion_heap *heap;
	unsigned long *nr_freed = (unsigned long *)parm;

	/* check if the page pool has the pages */
	plist_for_each_entry(heap, &g_idev->heaps, node) {
		if (heap->type == ION_HEAP_TYPE_SYSTEM) {
			*nr_freed += reclaim_ion_for_oom(heap);
		}
	}
	return NOTIFY_OK;
}

static struct notifier_block ion_oom_nb = {
...
static int ion_debug_buffer_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "%20.s %16.s %4.s %16.s %4.s %10.s %4.s %3.s %10.s %6.s "
			"%24.s %9.s\n",
	...
	for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
		...
		seq_printf(s, "%20.s %16.s %4u %16.s %4u %10zu %4d %3d %6d "
				"%24.s %p %9lx", buffer->heap->name,
				buffer->task_comm, buffer->pid,
				buffer->thread_comm,
				buffer->tid, buffer->size, buffer->kmap_cnt,
				atomic_read(&buffer->ref.refcount),
				buffer->handle_count, master_name, buffer,
				buffer->flags);
		seq_printf(s, "(");
...
void ion_device_destroy(struct ion_device *dev)
{
	...
	kfree(dev);
#ifdef CONFIG_ION_EXYNOS_STAT_LOG
	unregister_oom_notifier(&ion_oom_nb);
	unregister_oomdebug_notifier(&ion_oomdebug_nb);
#endif
}

...
void sec_debug_save_last_kmsg(unsigned char* head_ptr, unsigned char* curr_ptr)
{
	...
	last_kmsg_size = min((size_t)(1 << (CONFIG_LOG_BUF_SHIFT+1)), size);
...
static int __init sec_last_kmsg_setup(char *str)
{
	unsigned size = memparse(str, &str);
	unsigned long base = 0;
...
#ifdef CONFIG_TIMA_RKP
	last_kmsg_buffer = (char *)phys_to_virt(base);
#else
	last_kmsg_buffer = (char *)__phys_to_coherent_virt(base);
#endif

	pr_info("%s: base 0x%lx last_kmsg_buffer 0x%p\n", __func__, base, last_kmsg_buffer);

	return 1;
out:
	return 0;
}
__setup("sec_last_kmsg=", sec_last_kmsg_setup);

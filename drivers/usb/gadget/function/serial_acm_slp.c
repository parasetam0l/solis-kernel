...
{
	if (s_acm_dev->busy) {
		pr_warn("ACM_dun is already opened by %s(%d),  tried %s(%d)\n",
	...
	if (!atomic_read(&link_state)) {
		pr_err("Cannot open ACM_dun(not connected)\n");
		return -ENODEV;
	}
	...
	pr_debug("Opened ACM_dun by processor: %s(%d), and real_parent: %s(%d)\n",
		 current->comm , current->pid,
...
{
	pr_debug("Closed ACM_dun from processor: %s(%d), but opend by: %s(%d)\n",
			current->comm , current->pid,
			acm_dbg_task ? acm_dbg_task->comm  : "NULL",
			acm_dbg_task ? acm_dbg_task->pid : 0);
		...
		size_t count, loff_t *ppos)
{
	...
	if (!atomic_read(&link_state)) {
		pr_err("Cannot read ACM_dun(not connected)\n");
		return -ENODEV;
	}
...
{
	if (s_acm_dev == NULL) {
		pr_err("ACM_dun notify control failed. s_acm_dev is null.\n");
		return ;
	}
...
static long modem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_debug("ACM_dun modem_ioctl: cmd=0x%x, arg=%lu\n", cmd, arg);

	if (!atomic_read(&link_state)) {
		pr_debug("Cannot ioctl ACM_dun(not connected)\n");
		return -ENODEV;
	}
	...
	default:
		pr_err("ACM_dun: Unknown ioctl cmd(0x%x).\n", cmd);
		status = -ENOIOCTLCMD;
	}
...
int modem_register(void *data)
{
	if ((data == NULL) || (s_acm_dev == NULL)) {
		pr_err("ACM_dun connect failed. data or s_acm_dev is null.\n");
		return -EINVAL;
	}
...
{
	if (s_acm_dev == NULL) {
		pr_err("ACM_dun disconnect failed. s_acm_dev is null.\n");
		return ;
	}
	...
	pr_info("ACM_dun is disconnected\n");
}
...
static int modem_misc_register(void)
{
	...
	if (ret) {
		pr_err("[serial_acm] misc_register failed\n");
		kfree(dev);
		goto err_reg;
	}

...
static struct usb_ss_ep_comp_descriptor sdb_bulk_comp_desc = {
	.bLength = sizeof sdb_bulk_comp_desc,
...
static struct usb_descriptor_header *ss_sdb_descs[] = {
	...
	(struct usb_descriptor_header *) &sdb_bulk_comp_desc,
...
requeue_req:
	...
	if (!dev->sdb_func || !dev->rx_req)
		...
		/* Restriction by DWC3 gadget controller.
		 * DWC3 does not support unaligned bulk out request,
		 * So, align request length with maxpacket.
		 */
		maxpacket = dev->sdb_func->ep_out->maxpacket;
		dev->rx_req->length += (maxpacket - 1);
		dev->rx_req->length -= (dev->rx_req->length % maxpacket);
	...
	if (ret < 0) {
#ifdef TIZEN_SEC_KERNEL_ENG
		panic("sdb read queue failed(%d)\n", ret);
#else
		r = -EIO;
		dev->error = 1;
		goto done;
	...
	ret = wait_event_interruptible(dev->read_wq,
			(dev->rx_done || dev->error));

	if (ret < 0) {
		pr_info("sdb read_wq wakeup by signal(%d)\n", ret);
		dev->error = 1;
		r = ret;

		mutex_lock(&sdb_lock);
		if (!dev->sdb_func || !dev->rx_req)
			pr_err("sdb already unbinded\n");
		else {
			pr_info("sdb rx_req was forcely dequeue\n");
			ret = usb_ep_dequeue(dev->sdb_func->ep_out,
					dev->rx_req);
			if (ret < 0)
				pr_err("sdb rx_req dequeue failed(%d)\n", ret);
			else
				wait_event(dev->read_wq, dev->rx_done);
		}
		mutex_unlock(&sdb_lock);
...
sdb_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (gadget_is_superspeed(cdev->gadget)) {
		...
		f->ss_descriptors = usb_copy_descriptors(ss_sdb_descs);
		if (!f->ss_descriptors)
			goto desc_alloc_fail;
	}
		...
		unsigned intf, unsigned alt)
{
	...
	if (!sdb_func->ep_in->desc)
		if (config_ep_by_speed(cdev->gadget, f, sdb_func->ep_in))
			return -EINVAL;
	...
	if (!sdb_func->ep_out->desc) {
		if (config_ep_by_speed(cdev->gadget, f, sdb_func->ep_out)) {
			usb_ep_disable(sdb_func->ep_in);
			sdb_func->ep_in->driver_data = NULL;
			return -EINVAL;
		}

...
#ifdef MTP_USE_EMGQ_MAIN
#define MTP_BULK_BUFFER_SIZE (32768)
#define MTP_RX_REQ_MAX		 6
#define MTP_TX_EMG_MAX		 8
#define MTP_INT_REQ_MAX		 2
#define MTP_TX_REQ_MAX		 DIV_ROUND_UP(MTP_TX_EMG_MAX, 4)
#else
#define MTP_RX_REQ_MAX		 8
#define MTP_TX_REQ_MAX		 10
#define MTP_INT_REQ_MAX		 2
#define MTP_TX_EMG_MAX		 DIV_ROUND_UP(MTP_TX_REQ_MAX, 4)
#endif
...
/*
 * To avoid compiler & architecture issue (32bit/64bit),
 * We use fixed one size - "int" for ioctl command size
 * but real data size are different !!
 */
...
#define MTP_WRITE_INT_DATA	_IOW(MTP_IOCTL_LETTER, 4, int)
#define SET_MTP_USER_PID	_IOW(MTP_IOCTL_LETTER, 5, int)
#define GET_SETUP_DATA		_IOR(MTP_IOCTL_LETTER, 6, int)
#define SET_SETUP_DATA		_IOW(MTP_IOCTL_LETTER, 7, int)
#define SEND_RESET_ACK		_IO(MTP_IOCTL_LETTER, 8)
#define SET_ZLP_DATA		_IO(MTP_IOCTL_LETTER, 9)
#define GET_MAX_PKT_SIZE	_IOR(MTP_IOCTL_LETTER, 10, int)
#define SIG_SETUP			44
...
static struct usb_endpoint_descriptor mtpg_notify_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_IN,
...
static struct usb_descriptor_header *fs_mtpg_desc[] = {
	...
	(struct usb_descriptor_header *)&mtpg_notify_desc,
	NULL,
};
...
static struct usb_endpoint_descriptor ss_mtpg_in_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_IN,
	.bmAttributes = USB_ENDPOINT_XFER_BULK,
...
static struct usb_endpoint_descriptor ss_mtpg_out_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_OUT,
	.bmAttributes = USB_ENDPOINT_XFER_BULK,
...
static struct usb_ss_ep_comp_descriptor ss_mtpg_bulk_comp_desc = {
	.bLength = sizeof ss_mtpg_bulk_comp_desc,
...
static struct usb_ss_ep_comp_descriptor ss_mtpg_intr_comp_desc = {
	.bLength = sizeof ss_mtpg_intr_comp_desc,
	.bDescriptorType =	USB_DT_SS_ENDPOINT_COMP,
...
static struct usb_descriptor_header *ss_mtpg_desc[] = {
	(struct usb_descriptor_header *) &mtpg_interface_desc,
	(struct usb_descriptor_header *) &ss_mtpg_in_desc,
	(struct usb_descriptor_header *) &ss_mtpg_bulk_comp_desc,
	(struct usb_descriptor_header *) &ss_mtpg_out_desc,
	(struct usb_descriptor_header *) &ss_mtpg_bulk_comp_desc,
	(struct usb_descriptor_header *) &mtpg_notify_desc,
	(struct usb_descriptor_header *) &ss_mtpg_intr_comp_desc,
	NULL,
};
			 ...
			 size_t count, loff_t *pos)
{
	...
	while (!dev->online) {
		...
		if (ret < 0 || dev->error) {
			(dev->error) ? (r = -EIO) : (r = ret);
			goto fail;
		}
	}

	if ((!dev->error) && (dev->set_alted)) {
		/* First read-qeue for open-session */
		req = mtp_req_get(dev, dev->rx_idle);
		if (!req) {
			pr_err("[mtpg] can't get req for open-session\n");
			r = -ENODEV;
			goto fail;
		}
		...
		pr_info("[mtpg] SET_ALT continue handled\n");
		...
		if (tx_q_head == dev->tx_idle) {
			...
			if (!req->buf) {
				pr_info("[%s]malloc retry stop:bytes(%d), try_cnt(%d)\n",
					__func__, (int)start_size, try_cnt);
...
{
	return mtpg_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}
#endif
			      ...
			      struct usb_function *f)
{
	...
	rc = mtp_create_bulk_endpoints(mtp_func, &fs_mtpg_in_desc,
				   &fs_mtpg_out_desc, &mtpg_notify_desc);
	if (rc) {
	...
	if (gadget_is_superspeed(cdev->gadget)) {
		DEBUG_MTPB("[%s] \tsuper speed line = [%d]\n", __func__,
			   __LINE__);

		/* Assume endpoint addresses are the same for both speeds */
		ss_mtpg_in_desc.bEndpointAddress =
		    fs_mtpg_in_desc.bEndpointAddress;
		ss_mtpg_out_desc.bEndpointAddress =
		    fs_mtpg_out_desc.bEndpointAddress;

		f->ss_descriptors = usb_copy_descriptors(ss_mtpg_desc);
		if (!f->ss_descriptors)
			goto desc_alloc_fail;
	}
				 ...
				 unsigned intf, unsigned alt)
{
	...
	if (intf == mtp_func->allocated_intf_num) {
		...
		/* First quick read-qeue for open-session */
		req = mtp_req_get(dev, &mtp_func->bulk_out_q);
		if (!!req) {
			req->length = dev->bulkBufSize;
			ret = usb_ep_queue(mtp_func->bulk_out,
					req, GFP_ATOMIC);
			...
			pr_info("[mtpg] return by DELAYED_STAUS for SET_ALT\n");
			ret = USB_GADGET_DELAYED_STATUS;
		}
...
static int mtp_setup(struct usb_composite_dev *cdev)
{
...
#ifdef MTP_USE_EMGQ_MAIN
	/*
	 * To reduce memory usage & stable mtp working,
	 *  we use fixed pre-allocated buffers
	 */
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
			mtpg->bulkBufSize = MTP_BULK_BUFFER_SIZE;
		mtpg->intrBufSize = 10;
#else
	/*
	...
	if (gadget_is_s3c_hsotg(gadget)) {
		/* S.LSI udc's max transferable size at once is 512KB-1 */
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
		...
		/* MSM udc's max transferable size is 16KB */
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
			round_down(0x4000, 512);

		mtpg->bulkBufSize = round_down(0x4000, 512);
		mtpg->intrBufSize = 22;
	} else if (gadget_is_dwc3(gadget)) {
		/* dwc3-udc's max transferable size at once is 16MB-1 */
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
			round_down(0xFFFFFF, 1024);

		/* bulkBufSize & intrBufSize have to smaller
		 * than udc's max transferable size or same
		 * and will be considering memory usage
		 */
		mtpg->bulkBufSize = round_down(0x8000, 1024);
		mtpg->intrBufSize = 22;
	} else {
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
			round_down(PAGE_SIZE, 512);
		mtpg->bulkBufSize = round_down(PAGE_SIZE, 512);
		mtpg->intrBufSize = 10;
	}
#endif

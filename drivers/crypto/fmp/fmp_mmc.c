...
#if defined(CONFIG_MMC_DW_FMP_ECRYPT_FS)
static int configure_fmp_file_enc(struct dw_mci *host, struct idmac_desc_64addr *desc,
				uint32_t enc_mode, uint32_t idx, uint32_t sector,
				struct mmc_data *data, uint32_t size)
{
...
#endif
	loff_t index;
	struct page *page = sg_page(&data->sg[idx]);
	int ret;

	/* File algorithm selector*/
	if (!strncmp(page->mapping->alg, "aes", sizeof("aes")))
		aes_alg = AES_CBC;
	else if (!strncmp(page->mapping->alg, "aesxts", sizeof("aesxts")))
		aes_alg = AES_XTS;
	else {
	...
	/* File enc key size */
	switch (page->mapping->key_length) {
	case 16:
	...
	index = page->index;
	if ((last_index != index) || (last_inode != page->mapping->host->i_ino)) {
	...
	last_index = page->index;
	last_inode = page->mapping->host->i_ino;
	...
	/* File Enc key */
	for (j = 0; j < page->mapping->key_length >> 2; j++)
		*(&(desc->des12) + j) =
			word_in(page->mapping->key, (page->mapping->key_length >> 2) - (j + 1));

#if defined(CONFIG_FIPS_FMP)
	if (!strncmp(page->mapping->alg, "aesxts", sizeof("aesxts"))) {
		if (fmp_xts_check_key((uint8_t *)&desc->des12,
				(uint8_t *)&desc->des20,
				page->mapping->key_length)) {
...
#if defined(CONFIG_MMC_DW_FMP_ECRYPT_FS)
	/* File Encryption */
	if (!(enc_mode & MMC_FMP_FILE_ENC_MODE) ||
			!(host->pdata->quirks & DW_MCI_QUIRK_USE_SMU))
		return 0;

	ret = configure_fmp_file_enc(host, desc, enc_mode, idx, sector, data, size);
	if (ret) {

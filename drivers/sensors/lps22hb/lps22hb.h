...
struct lps22_prs_transfer_function {
	int (*write)(struct lps22_prs_data *cdata,
					u8 reg_addr, int len, u8 *data);
	int (*read)(struct lps22_prs_data *cdata,
					u8 reg_addr, int len, u8 *data);
};
...
#ifdef CONFIG_TIZEN
	struct device *prs_dev;

	s32 iPressureCal;
	int sealevelpressure;

	/* for interface pinctrl */
	bool intf_pinctl;
	struct pinctrl *pinctrl;
	struct pinctrl_state *intf_pw_on;
	struct pinctrl_state *intf_pw_off;
#endif
};

...
{
	ssp_pkt_dump((const void *)buf, NULL, size);
...
/**
 *
 * bbd_on_read_buf_cnt - To check read buffer count of specified char device.
 *
 */
int bbd_on_read_buf_cnt(unsigned int minor)
{
	struct circ_buf *circ = &bbd.priv[minor].read_buf;
	int remain_cnt;
	...
	mutex_lock(&bbd.priv[minor].lock);
	remain_cnt = CIRC_CNT(circ->head, circ->tail, BBD_BUFF_SIZE);
	mutex_unlock(&bbd.priv[minor].lock);

	return remain_cnt;
}
EXPORT_SYMBOL(bbd_on_read_buf_cnt);

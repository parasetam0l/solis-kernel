...
int k6ds_accel_open_calibration(struct ssp_data *data)
{
	int iRet = 0;
	mm_segment_t old_fs;
...
int k6ds_set_accel_cal(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
	s16 accel_cal[3];
	...
	struct device_attribute *attr, char *buf)
{
	...
	iRet = k6ds_accel_open_calibration(data);
	if (iRet < 0)
	...
	struct device_attribute *attr, char *buf)
{
	...
	int read_fail;
	int sum_count;
	int16_t tilt_x, tilt_y, tilt_z;
	int16_t raw_x = 0, raw_y = 0, raw_z = 0;
	int16_t x_sum, y_sum, z_sum;
	int32_t dcm_Matrix[][3] = { {100000, 0, 0},
							{0, 96593, 25882},
							{0, -25882, 96593} };
	...
	while (try_count) {
		...
		for (read_count = 0; read_count < 4; read_count++) {
			tilt_x = data->buf[ACCELEROMETER_SENSOR].x;
			tilt_y = data->buf[ACCELEROMETER_SENSOR].y;
			tilt_z = data->buf[ACCELEROMETER_SENSOR].z;

			raw_x = (uint16_t)((dcm_Matrix[0][0]*tilt_x +
					dcm_Matrix[0][1]*tilt_y +
					dcm_Matrix[0][2]*tilt_z) / 100000);
			raw_y = (uint16_t)((dcm_Matrix[1][0]*tilt_x +
					dcm_Matrix[1][1]*tilt_y +
					dcm_Matrix[1][2]*tilt_z) / 100000);
			raw_z = (uint16_t)((dcm_Matrix[2][0]*tilt_x +
					dcm_Matrix[2][1]*tilt_y +
					dcm_Matrix[2][2]*tilt_z) / 100000);
			...
			if (((raw_z >= 0)
				&& ((raw_z < ACCEL_Z_SPEC_MAX - 1000)
				|| (raw_z > ACCEL_Z_SPEC_MAX + 1000))) ||
				((raw_z < 0)
				&& ((raw_z < ACCEL_Z_SPEC_MIN - 1000)
				|| (raw_z > ACCEL_Z_SPEC_MIN + 1000)))) {
				pr_err("[SSP] %s: spec out!!.x = %d, y = %d, z = %d\n",
					__func__, raw_x, raw_y, raw_z);
				read_fail = 1;
				break;
			}
...
void initialize_k6ds_accel_factorytest(struct ssp_data *data)
{
	sensors_register(data->acc_device, data, acc_attrs,
		"accelerometer_sensor");
}

void remove_k6ds_accel_factorytest(struct ssp_data *data)
{

...
int k6ds_gyro_open_calibration(struct ssp_data *data)
{
	int iRet = 0;
	mm_segment_t old_fs;
...
int k6ds_set_gyro_cal(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
	s16 gyro_cal[3];
...
#define GYRO_SELFTEST_FAIL_VAL_RETURN
...
#ifdef GYRO_SELFTEST_FAIL_VAL_RETURN
			goto exit;
#else
...
exit:
	ssp_dbg("[SSP]: %s - "
		...
		gyro_self_diff[0], gyro_self_diff[1], gyro_self_diff[2],
		fifo_ret, cal_ret, hw_result);

#ifndef GYRO_SELFTEST_FAIL_VAL_RETURN
	// Gyro Calibration pass / fail, buffer 1~6 values.
	if( (fifo_ret == 0) || (cal_ret == 0) ) {
		ssp_dbg("[SSP]: %s - fifo_ret: %d, cal_reg: %d self_diff %d,%d,%d fail.\n",
			__func__, fifo_ret, cal_ret, gyro_self_diff[0], gyro_self_diff[1], gyro_self_diff[2]);
		return sprintf(buf, "%d,%d,%d\n", gyro_self_diff[0], gyro_self_diff[1], gyro_self_diff[2]);
	}
#endif
	return sprintf(buf,
...
void initialize_k6ds_gyro_factorytest(struct ssp_data *data)
{
	sensors_register(data->gyro_device, data, gyro_attrs, "gyro_sensor");
}

void remove_k6ds_gyro_factorytest(struct ssp_data *data)
{

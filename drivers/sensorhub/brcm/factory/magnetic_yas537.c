...
#define CHIP_ID		"YAS537"
#define MAG_HW_OFFSET_FILE_PATH	"/csa/sensor/mag_cal_data"

static int mag_open_hwoffset(struct ssp_data *data)
{
	int iRet = 0;
	mm_segment_t old_fs;
...
#if 0
static int get_hw_offset(struct ssp_data *data)
{
	int iRet = 0;
	char buffer[3] = { 0, };
...
static int mag_store_hwoffset(struct ssp_data *data)
{
	int iRet = 0;
	struct file *cal_filp = NULL;
	mm_segment_t old_fs;
...
static int set_hw_offset(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
	...
	pr_info("[SSP]: %s: x: %d, y: %d, z: %d\n", __func__,
		(s8)data->magoffset.x, (s8)data->magoffset.y,
		(s8)data->magoffset.z);
	return iRet;
}
#endif

static int set_static_matrix(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
...
int initialize_yas537_magnetic_sensor(struct ssp_data *data)
{
	int ret;
...
void initialize_yas537_magnetic_factorytest(struct ssp_data *data)
{
	sensors_register(data->mag_device, data, mag_attrs, "magnetic_sensor");
}

void remove_yas537_magnetic_factorytest(struct ssp_data *data)
{

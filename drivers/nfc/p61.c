...
static int p61_set_clk(struct p61_dev *p61_device)
{
	...
	spi_pclk = clk_get(NULL, "ese-spi-pclk");

	if (IS_ERR(spi_pclk))
		pr_err("Can't get spi_pclk\n");

	spi_sclk = clk_get(NULL, "ese-spi-sclk");
				 ...
				 unsigned long arg)
{
	...
	if (err) {
		pr_err("%s: p61_xfer failed! ret = %d\n", __func__, err);
		kfree(dup);
		return err;
	}
...
static int p61_dev_open(struct inode *inode, struct file *filp)
{
	...
	*p61_dev = container_of(filp->private_data,
	...
	if (gpio_direction_output(p61_dev->cspin, 1) < 0) {
		P61_ERR_MSG("%s: not able to set cspin as output\n", __func__);
		return  -ENODEV;
	}
#endif
	return 0;
...
static int p61_dev_release(struct inode *inode, struct file *file)
{
	...
	p61_suspend();
#else
	if (gpio_direction_input(p61_dev->cspin) < 0) {
		P61_ERR_MSG("%s: not able to set cspin as output\n", __func__);
		return  -ENODEV;
	}
#endif
	...
	if (pwr_req_on) {
		P61_DBG_MSG("%s: release spi session\n", __func__);
		pwr_req_on = 0;
	...
	struct p61_dev *data)
{
	...
	data->mosipin = of_get_named_gpio_flags(np, "p61-mosipin", 0, &flags);
	data->misopin = of_get_named_gpio_flags(np, "p61-misopin", 0, &flags);
	data->cspin = of_get_named_gpio_flags(np, "p61-cspin", 0, &flags);
	data->clkpin = of_get_named_gpio_flags(np, "p61-clkpin", 0, &flags);
	//data->tz_mode = true;

	pr_info("%s: mosi=%d, miso=%d, cs=%d, clk=%d\n", __func__,
...
/* TBD: This is only for Panda as we are passing NULL for platform data */
	...
	/*
	 * SOLIS: workaround for VDD_ESE
	 * exynos spi driver always control the cspin as gpio in manual mode
	 * then CS pin has high level atfer spi probed. so it cannot controlled by DT.
	 * ESE_SPI_CS_N pin is connected to VDD_ESE then voltage level of VDD_ESE
	 * is 1.2v by CS pin High config.
	 */
	if (gpio_direction_input(p61_dev->cspin) < 0) {
		P61_ERR_MSG("%s: not able to set cspin as output\n", __func__);
		return  -ENODEV;

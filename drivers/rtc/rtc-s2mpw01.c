...
static void s2m_data_to_tm(u8 *data, struct rtc_time *tm)
{
	...
	tm->tm_usec = (int)((((data[RTC_USEC] & 0xf0) >> 4) * S2MPW01_HR_RESOLUTION) / 10);
#endif
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	/*
	 * CAPSEL REGISTER's MSB 4bits are assigned to the
	 * high resolution RTC expression.
	 * These 4 bits represent the 1/16 second.
	 */
#ifdef CONFIG_RTC_DRV_S2MPW01_HR
	reg = S2MP_RTC_REG_CAPSEL;
#else
	reg = S2MP_RTC_REG_SEC;
#endif

	ret = s2mpw01_bulk_read(info->i2c, reg, NR_RTC_CNT_REGS,
			data);
	...
	dev_info(info->dev, "%s: %d-%02d-%02d %02d:%02d:%02d + %03d(0x%02x)%s\n",
			...
			data[RTC_SEC], (((data[RTC_USEC] & 0xf0) >> 4) * S2MPW01_HR_RESOLUTION) / 10,
			data[RTC_WEEKDAY], data[RTC_HOUR] & HOUR_PM_MASK ? "PM" : "AM");
#else
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
...
#ifdef CONFIG_RTC_DRV_S2MPW01_HR
	count = NR_RTC_CNT_REGS - 1;
#else
	count = NR_RTC_CNT_REGS;
#endif
	buf = &data[RTC_SEC];
	...
	/* workaround for correcting the error occuring when writing 59s into the RTC-SEC register */
	if(data[RTC_SEC] == 59) {
		data[RTC_SEC] = 0;
		ret = s2mpw01_bulk_write(info->i2c, S2MP_RTC_REG_SEC, count,
				buf);
		if(ret < 0) {
		...
		mdelay(1);
		//dev_info(info->dev, "%s: Work-around at 59 sec", __func__);
		data[RTC_SEC] = 59;
	}
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	ret = s2mpw01_bulk_read(info->i2c, S2MP_RTC_REG_SEC, count,
			buf);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to read time reg(%d)\n", __func__,
			ret);
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpw01_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, count,
			buf);
	if (ret < 0) {
	...
	s2m_data_to_tm(buf, &alrm->time);

#ifdef CONFIG_RTC_DRV_S2MPW01_HR
	dev_info(info->dev, "%s: %d-%02d-%02d %02d:%02d:%02d:%03d(%d)\n", __func__,
			alrm->time.tm_year + 1900, alrm->time.tm_mon + 1,
			alrm->time.tm_mday, alrm->time.tm_hour,
			alrm->time.tm_min, alrm->time.tm_sec,
			(((alrm->time.tm_usec & 0xf0) >> 4) * S2MPW01_HR_RESOLUTION) / 10,
			alrm->time.tm_wday);
#else
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
...
#ifdef CONFIG_RTC_DRV_S2MPW01_HR
	dev_info(info->dev, "%s: %d-%02d-%02d %02d:%02d:%02d:%04d(0x%02x)%s\n",
			__func__, data[RTC_YEAR] + 2000, data[RTC_MONTH],
			data[RTC_DATE], data[RTC_HOUR] & 0x1f, data[RTC_MIN],
			data[RTC_SEC],
			(((data[RTC_USEC] & 0xf0) >> 4) * S2MPW01_HR_RESOLUTION) / 10,
			data[RTC_WEEKDAY],
			data[RTC_HOUR] & HOUR_PM_MASK ? "PM" : "AM");
#else
	dev_info(info->dev, "%s: %d-%02d-%02d %02d:%02d:%02d(0x%02x)%s\n",
	...
	if (info->alarm_check) {
		for (i = 0; i < count; i++)
			buf[i] &= ~ALARM_ENABLE_MASK;

		ret = s2mpw01_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, count,
				buf);
		if (ret < 0) {
...
/* if the weekday is not correct with the date,
   the alarm is not able to wake up */
	...
	buf[RTC_WEEKDAY] &= ~ALARM_ENABLE_MASK;
#endif
				...
				struct s2mpw01_platform_data *pdata)
{
	...
	if ((ctrl_val & MODEL24_MASK) && ((capsel_val & 0x08) == 0x08))
		return 0;

...
static void kbase_dma_fence_work(struct work_struct *pwork);
 ...
 * submit the atom.
 */
static void kbase_dma_fence_queue_work(struct kbase_jd_atom *katom)
{
	struct kbase_context *kctx = katom->kctx;
	bool ret;
...
kbase_dma_fence_work(struct work_struct *pwork)
{
	 ...
	 * depedency. Run jd_done_nolock() on the katom if it is completed.
	 */
					 ...
					 bool exclusive)
{
	...
	if (excl_fence) {
		 ...
		 * taken by reservation_object_get_fences_rcu(), to the fence.
		 * We have set up our callback (if that was possible), and it's
		 * the fence's owner is responsible for signaling the fence
...
int kbase_dma_fence_init(struct kbase_context *kctx)
{
	...
	kctx->dma_fence.wq = alloc_workqueue("mali-fence-%d",
						WQ_UNBOUND | __WQ_ORDERED, 1, kctx->pid);

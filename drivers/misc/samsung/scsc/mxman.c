...
#define SCSC_RF_HW_S610 0xb0
	...
	SCSC_TAG_INFO(MXMAN, "RF_CHIP_VERSION: 0x%04x: %s (0x%02x), EVT%x.%x\n",
		mxman->rf_hw_ver,
		((mxman->rf_hw_ver & 0x00ff) == SCSC_RF_HW_S610) ? "S610" : "Unknown", (mxman->rf_hw_ver & 0x00ff),
...
static void process_panic_record(struct mxman *mxman)
{
	u32 *r4_panic_record;
	u32 *m4_panic_record;
	u32 r4_panic_record_length;
	u32 m4_panic_record_length;
	bool r4_panic_record_ok = false;
	bool m4_panic_record_ok = false;
	bool r4_sympathetic_panic_flag;
	bool m4_sympathetic_panic_flag;

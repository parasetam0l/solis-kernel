...
int sipc5_init_io_device(struct io_device *iod)
{
	...
	case IODEV_NET:
		...
		if (modem_mon_idx < MODEM_SLEEP_IDX_MAX) {
			modem_smon[modem_mon_idx].ndev = iod->ndev;
			modem_smon[modem_mon_idx].rx_packets = 0;
			modem_smon[modem_mon_idx].tx_packets = 0;
			modem_smon[modem_mon_idx].rx_bytes = 0;
			modem_smon[modem_mon_idx].tx_bytes = 0;
			modem_mon_idx++;
		}
#endif
		mif_debug("iod 0x%p\n", iod);

...
static int modem_get_sleep_monitor64_cb(void *priv,
							long long *raw_val, int check_level, int caller_type)
{
	int i;
	struct net_device *ndev;
	unsigned int total_act_cnt = 0;
	unsigned long rx_bytes = 0, tx_bytes = 0;
	unsigned long rx_packets = 0, tx_packets = 0;

	if (caller_type == SLEEP_MONITOR_CALL_SUSPEND) {
		for (i = 0; i < modem_mon_idx; i++) {
			ndev = modem_smon[i].ndev;

			rx_bytes += ndev->stats.rx_bytes - modem_smon[i].rx_bytes;
			tx_bytes += ndev->stats.rx_bytes - modem_smon[i].tx_bytes;
			rx_packets += ndev->stats.rx_packets - modem_smon[i].rx_packets;
			tx_packets += ndev->stats.tx_packets - modem_smon[i].tx_packets;
		}

		/* mif_info("%s rx_bytes[%ld], tx_bytes[%ld], rx_packets[%ld], tx_packets[%ld]\n",
			__func__, rx_bytes, tx_bytes, rx_packets, tx_packets);*/
	}
	mif_info("%s rx_bytes[%ld], tx_bytes[%ld], rx_packets[%ld], tx_packets[%ld]\n",
		__func__, rx_bytes, tx_bytes, rx_packets, tx_packets);
	...
	if (caller_type == SLEEP_MONITOR_CALL_RESUME) {
		for (i = 0; i < modem_mon_idx; i++) {
			...
			modem_smon[i].rx_bytes = ndev->stats.rx_bytes;
			modem_smon[i].tx_bytes = ndev->stats.tx_bytes;
			modem_smon[i].rx_packets = ndev->stats.rx_packets;
			modem_smon[i].tx_packets = ndev->stats.tx_packets;
		}
	}

	*raw_val = *raw_val | ((long long) (rx_bytes & MODEM_SLEEP_MON_LENGTH) <<
		(0 * MODEM_SLEEP_MON_SHIFT));
	*raw_val = *raw_val | ((long long) (rx_packets & MODEM_SLEEP_MON_LENGTH) <<
		(1 * MODEM_SLEEP_MON_SHIFT));
	*raw_val = *raw_val | ((long long) (tx_bytes & MODEM_SLEEP_MON_LENGTH) <<
		(2 * MODEM_SLEEP_MON_SHIFT));
	*raw_val = *raw_val | ((long long) (tx_packets & MODEM_SLEEP_MON_LENGTH) <<
		(3 * MODEM_SLEEP_MON_SHIFT));

	if ((rx_packets + tx_packets) == 0) {
		total_act_cnt = 0;
	} else if ((rx_packets + tx_packets) < 50) {
		total_act_cnt = 5;
	} else if ((rx_packets + tx_packets) < 100) {
		total_act_cnt = 10;
	} else
		total_act_cnt = 15;

	/* mif_info("%s total_act_cnt [%d]\n",__func__, total_act_cnt); */

	return (total_act_cnt > DEVICE_UNKNOWN) ? DEVICE_UNKNOWN : total_act_cnt;
}

static struct sleep_monitor_ops modem_sleep_monitor_ops = {
	.read64_cb_func = modem_get_sleep_monitor64_cb,
};

struct modem_sleep_mon modem_smon[10];
unsigned int modem_mon_idx = 0;
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(NULL, &modem_sleep_monitor_ops,
				SLEEP_MONITOR_CP);
#endif

...
#ifdef CONFIG_KITT_PROJECT_UART_SEL_SWITCH
#define BASE_GPIO  902
#define UART_SEL (BASE_GPIO + 28)
#endif
...
#define MANSW1_OPEN_RUSTPROOF	((0 << 5)| (3 << 2) |(1 << 0))
...
static ssize_t sm5502_show_jig_state(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
...
static DEVICE_ATTR(jig_state, S_IRUGO, sm5502_show_jig_state, NULL);
...
#endif
/*Handle UART_SEL gpio*/
#ifdef CONFIG_KITT_PROJECT_UART_SEL_SWITCH
	int ret = 0;
	if(check_sm5502_jig_state()){
		ret = gpio_direction_output(UART_SEL, 0);
		if(ret){
			pr_err("UART_SEL gpio is not in LOW");
			return ret;
		}
	}
#endif
...
static void sm5502_init_detect(struct work_struct *work)
{
	...
	{/*check usr binary*/
		...
		val_reg13 = sm5502_read_reg(usbsw->client, REG_MANUAL_SW1);
		if (val_reg13 == MANSW1_OPEN_RUSTPROOF)
		{
			is_usrbin = 1;
			val_reg2 = sm5502_read_reg(usbsw->client, REG_CONTROL);
			if (!(val_reg2 & 0x04))
				i2c_smbus_write_byte_data(usbsw->client, REG_CONTROL, 1 << 2); //manual switching -> auto switching
		}
		dev_info(&usbsw->client->dev,"Kernel Binary: %s(0x%x)\n",
			is_usrbin ? "USR" : "ENG", val_reg13);
	}
#endif
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(client->dev.of_node) {
        ...
        /* Change Mux config and Electrical properties in pinctrl dtsi*/
	...
	ret = device_create_file(switch_dev, &dev_attr_jig_state);
	if (ret < 0) {
		pr_err("[SM5502] Failed to create file (jig_state)!\n");
		goto err_create_file_jig_state;
	}
...
err_create_file_jig_state:
	device_remove_file(switch_dev, &dev_attr_jig_state);

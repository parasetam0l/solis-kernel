...
void config_pmu_sel(int path)
{
	...
	if (path == AP) {	/* AP */
		...
		sel_uart_dbg_gpio = SEL_CP_UART_DBG;
	...
	val = sel_uart_dbg_gpio | func_iso_en;
	ret = exynos_pmu_write(EXYNOS_PMU_UART_IO_SHARE_CTRL, val);
	if (ret < 0)
		pr_err("%s: ERR(%d) set EXYNOS_PMU_UART_IO_SHARE_CTRL\n", __func__, ret);
	...
	exynos_pmu_read(EXYNOS_PMU_UART_IO_SHARE_CTRL, &val);
	pr_info("%s EXYNOS_PMU_UART_IO_SHARE_CTRL 0x%x \n", __func__, val);
	exynos_pmu_read(PMU_USBDEV_PHY_CONTROL, &val);
	pr_info("%s PMU_USBDEV_PHY_CONTROL 0x%x \n", __func__, val);
}
#endif

void restore_usb_path(void)
{
	unsigned int val;

	val = SEL_UART_DBG_GPIO;
	exynos_pmu_write(EXYNOS_PMU_UART_IO_SHARE_CTRL, val);
	val = USBDEV_PHY_ENABLE;
	exynos_pmu_write(PMU_USBDEV_PHY_CONTROL, val);
	...
	switch (attached_dev) {
		...
		case ATTACHED_DEV_JIG_USB_ON_MUIC:
			if ((action == MUIC_NOTIFY_CMD_DETACH) ||
				(action == MUIC_NOTIFY_CMD_LOGICALLY_DETACH)) {
				pr_info("%s: DEV_USB_MUIC CMD_DETACH\n", __func__);
			} else if ((action == MUIC_NOTIFY_CMD_ATTACH) ||
						(action == MUIC_NOTIFY_CMD_LOGICALLY_ATTACH)) {
				pr_info("%s: DEV_USB_MUIC CMD_ATTACH\n", __func__);
				restore_usb_path();
			}
			break;
...
static int set_uart_sel(void)
{
	...
	exynos_pmu_read(EXYNOS_PMU_UART_IO_SHARE_CTRL, &val);
	pr_info("%s EXYNOS_PMU_UART_IO_SHARE_CTRL %x \n", __func__, val);
	exynos_pmu_read(PMU_USBDEV_PHY_CONTROL, &val);
	pr_info("%s PMU_USBDEV_PHY_CONTROL %x \n", __func__, val);

	return 0;
}

static int set_uart_sel2(void)
{
	int ret;
	unsigned int val;
...
static ssize_t
uart_sel_store2(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	pr_err("%s Change UART port path2\n", __func__);
...
static DEVICE_ATTR(uart_sel2, 0664, uart_sel_show, uart_sel_store2);

static struct attribute *modemif_sysfs_attributes[] = {
	&dev_attr_uart_sel.attr,
	&dev_attr_uart_sel2.attr,
...
static int uart_sel_probe(struct platform_device *pdev)
{
	...
	err = device_create_file(dev, &dev_attr_uart_sel2);
	if (err)
		pr_err("can't create modem_ctrl2!!!\n");

...
static unsigned int cpufreq_range;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_JAVAW
static
#endif
struct cpufreq_governor cpufreq_gov_tizen_javaw = {
	.name                   = "tizen_javaw",
...
static struct workqueue_struct *tizen_javaw_cpu_hotplug_work;
...
int needed_cpu_online(unsigned int cpu_load[], unsigned int cpu_freq_mhz)
{
	...
	unsigned int max_cpu_freq = 1014;  // 1014Mhz
	...
	switch (num_online_cpu) {
		...
		case 2:
			if ((get_run_task(1) <= 400) && (cpu_load[0] < 100) &&
					(get_avg_load_freq(500) < (max_cpu_freq * 8))) {
				need_cpu_online_num = 1;
			} else if ((get_run_task(1) <= 500) && (cpu_load[0] < 100) &&
					(get_avg_load_freq(100) < (max_cpu_freq * 8))) {
				need_cpu_online_num = 1;
			}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
...
#if defined(BUILD_ERROR)
	dbs_tuners_ins.up_threshold = cpu_freq_get_threshold();
	dbs_tuners_ins.up_freq = cpufreq_range * cpu_gov_get_up_level() / 100;
#endif
	...
	if (!smp_processor_id() && target_cpu_online != num_online_cpus()) {
		queue_work_on(0, tizen_javaw_cpu_hotplug_work,
				      &this_dbs_info->sim_hotplug_work);
	}
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
		...
		if (cpu_gov_get_up_level() > 0) {
			...
			if (policy->cur == 100000) {
				...
				target_freq = min(policy->max, policy->cur + dbs_tuners_ins.up_freq);
			}
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, tizen_javaw_cpu_hotplug_work, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	/* To avoid duplicated Initialization of work */
	if (!hotplug_work_enabled) {
		INIT_WORK(&dbs_info->sim_hotplug_work, cpu_sim_hotplug_work);
		hotplug_work_enabled = 1;
	}

	queue_delayed_work_on(dbs_info->cpu, tizen_javaw_cpu_hotplug_work,
			      &dbs_info->work, delay);
}
...
static int cpufreq_governor_dbs(struct cpufreq_policy *policy, unsigned int event)
{
	...
	switch (event) {
		case CPUFREQ_GOV_START:
			...
			cpufreq_range = policy->max - policy->min;
...
static struct workqueue_struct *booting_cpu_boost_wq;
struct delayed_work booting_cpu_boost_work;
static struct pm_qos_request pm_qos_cpu_online_min_req;

static void booting_cpu_boost_completed(struct work_struct *work)
{
	pr_info("%s: +++", __func__);

	if (pm_qos_request_active(&pm_qos_cpu_online_min_req))
		pm_qos_remove_request(&pm_qos_cpu_online_min_req);
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	tizen_javaw_cpu_hotplug_work = create_workqueue("cpu_hotplug");
	if (!tizen_javaw_cpu_hotplug_work) {
		pr_err("%s cannot create tizen_javaw_cpu_hotplug_work\n", __func__);
		goto err;
	}
	...
	booting_cpu_boost_wq = alloc_workqueue("booting_cpu_boost_work", WQ_HIGHPRI, 0);
	if (!booting_cpu_boost_wq)
		printk(KERN_ERR "Failed to create booting_cpu_boost_wq workqueue\n");

	INIT_DELAYED_WORK(&booting_cpu_boost_work, booting_cpu_boost_completed);

	queue_delayed_work(booting_cpu_boost_wq, &booting_cpu_boost_work, CPU_BOOTING_TIME * HZ);

	if (!pm_qos_request_active(&pm_qos_cpu_online_min_req))
		pm_qos_add_request(&pm_qos_cpu_online_min_req, PM_QOS_CPU_ONLINE_MIN, 2);

	return cpufreq_register_governor(&cpufreq_gov_tizen_javaw);

err:
	destroy_workqueue(tizen_javaw_cpu_hotplug_work);
	return -1;
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_tizen_javaw);
	destroy_workqueue(tizen_javaw_cpu_hotplug_work);
}

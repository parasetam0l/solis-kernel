...
static void dw_mci_emmc_pwr_control(struct mmc_host *mmc, unsigned int power_mode)
{
	...
	switch(power_mode){
		case MMC_POWER_UP:
			...
			break;
		case MMC_POWER_OFF:
			if (priv->pinctrl && priv->clk_drive_suspend)
				pinctrl_select_state(priv->pinctrl, priv->clk_drive_suspend);
...
#if defined(CONFIG_MMC_DW_DEBUG)
			for (i = 0; i < 0x80; i++) {
				dev_info(host->dev, "Ch0 [%d] send_time: %llu \tdone_time: %llu\tcmd %d\targ: 0x %08x\tdata_size: %d\tseq_status: %d\trint_sts: 0x %08x\tstatus_count: %d\n", i,
						dw_mci_debug.debug_info[0].cmd_log[i].send_time,
						dw_mci_debug.debug_info[0].cmd_log[i].done_time,
						dw_mci_debug.debug_info[0].cmd_log[i].cmd,
						dw_mci_debug.debug_info[0].cmd_log[i].arg,
						dw_mci_debug.debug_info[0].cmd_log[i].data_size,
						dw_mci_debug.debug_info[0].cmd_log[i].seq_status,
						dw_mci_debug.debug_info[0].cmd_log[i].rint_sts,
						dw_mci_debug.debug_info[0].cmd_log[i].status_count);
				dev_info(host->dev, "Ch1 [%d] send_time: %llu \tdone_time: %llu\tcmd %d\targ: 0x %08x\tdata_size: %d\tseq_status: %d\trint_sts: 0x %08x\tstatus_count: %d\n", i,
						dw_mci_debug.debug_info[1].cmd_log[i].send_time,
						dw_mci_debug.debug_info[1].cmd_log[i].done_time,
						dw_mci_debug.debug_info[1].cmd_log[i].cmd,
						dw_mci_debug.debug_info[1].cmd_log[i].arg,
						dw_mci_debug.debug_info[1].cmd_log[i].data_size,
						dw_mci_debug.debug_info[1].cmd_log[i].seq_status,
						dw_mci_debug.debug_info[1].cmd_log[i].rint_sts,
						dw_mci_debug.debug_info[1].cmd_log[i].status_count);
			}
#endif /* defined (CONFIG_MMC_DW_DEBUG) */
		}
		spin_lock(&host->lock);
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	if(pdata->cd_type != DW_MCI_CD_EXTERNAL) {

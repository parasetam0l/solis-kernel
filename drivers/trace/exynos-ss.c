...
#define KEY_TIMEOUT_MS  1200
#define KEY_COUNT       7

enum crash_key_type {
	CRASH_KEY_TYPE_VOLDOWN_POWER,
	CRASH_KEY_TYPE_POWER,
};

static int exynos_ss_check_crash_key_by_type(enum crash_key_type key_type,
		unsigned int code, int value)
{
	static bool volup_p, voldown_p;
	static u64 pressed_time[KEY_COUNT];
	static int start, end, prev;
	static int key_cnt;

	switch (key_type) {
		case CRASH_KEY_TYPE_VOLDOWN_POWER:
			/*  Forced upload by Volume down + Power
			 *  Hold volume down key first
			 *  and then press power key twice
			 *  and volume up key should not be pressed
			 */
			if (code == KEY_VOLUMEUP)
				volup_p = (value)? true : false;

			if (code == KEY_VOLUMEDOWN) {
				voldown_p = (value)? true : false;
			...
			if (!volup_p && voldown_p) {
				if (code == KEY_POWER && value) {
					key_cnt++;
					pr_info("exynos-snapshot: count for entering forced upload [%d]\n", key_cnt);
				}
		...
		case CRASH_KEY_TYPE_POWER:
			/* Forced upload by pressing power key repeatedly
			 * Press a key KEY_COUNT times within KEY_TIMEOUT_MS
			 */
			if (code == KEY_POWER && value) {
				u64 time = cpu_clock(0);

				do_div(time, 1000000);

				pressed_time[end] = time;

				pr_debug("exynos-snapshot: crash-key: Pressed %d times during %lldms\n",
				...
				if ((pressed_time[end] - pressed_time[prev]) > KEY_TIMEOUT_MS ||
					...
					start = end;
					key_cnt = 1;
				} else if (key_cnt + 1 >= KEY_COUNT) {
					if ((pressed_time[end] - pressed_time[start]) < KEY_TIMEOUT_MS) {
						start = end = prev = key_cnt = 0; /* Reset count and goto panic */
						return true;
					}

					start = (start + 1) % KEY_COUNT;
				} else {
					key_cnt++;
				}
...
{
#ifdef CONFIG_EXYNOS_SNAPSHOT_CRASH_ONE_KEY
	enum crash_key_type key_type = CRASH_KEY_TYPE_POWER;
#else
	...
	if (code == KEY_POWER)
		pr_debug("exynos-snapshot: POWER-KEY %s\n", value ? "pressed" : "released");

	debug_enable = sec_debug_get_debug_level();
	if (!debug_enable)
		return;

	if (exynos_ss_check_crash_key_by_type(key_type, code, value))
		panic("Crash Key");
}
#endif /* CONFIG_EXYNOS_SNAPSHOT_CRASH_KEY */
...
{
	enum sec_debug_upload_cause_t upload_cause;

	if (!strncmp(buf, "User Fault", 10))
		upload_cause = UPLOAD_CAUSE_USER_FAULT;
	else if (!strncmp(buf, "Crash Key", 9))
		upload_cause = UPLOAD_CAUSE_FORCED_UPLOAD;
	else if (!strncmp(buf, "CP Crash", 8))
		upload_cause = UPLOAD_CAUSE_CP_ERROR_FATAL;
	else if (!strncmp(buf, "HSIC Disconnected", 17))
		upload_cause = UPLOAD_CAUSE_HSIC_DISCONNECTED;
	else
		upload_cause = UPLOAD_CAUSE_KERNEL_PANIC;

	exynos_pmu_write(EXYNOS_INFORM3, upload_cause);

	pr_emerg("%s: Upload cause: '%s' (0x%x)\n",
			__func__, (buf)? (char *)buf : "(NULL)", upload_cause);
	if (unlikely(!ess_base.enabled))
		return 0;
...
static int __init exynos_ss_fixmap(void)
{
	...
	for (i = 0; ARRAY_SIZE(ess_items); ++i) {
		if(!strcmp(ess_items[i].name, "log_kernel")) {
			sec_debug_save_last_kmsg(ess_items[i].head_ptr,
...
static ssize_t ess_force_panic_show(struct kobject *kobj,
			         struct kobj_attribute *attr, char *buf)
{
	ssize_t n = sprintf(buf,
			"1: kernel panic\n"
			"2: cp crash\n"
			"3: sw watchdog\n"
			);
...
static ssize_t ess_force_panic_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	long n = simple_strtol(buf, NULL, 0);
	spinlock_t slock;

	switch (n) {
		case 1:
			pr_info("%s: [%ld] Kernel Panic\n", __func__, n);
			panic("ess force panic");
			break;

		case 2:
			pr_info("%s: [%ld] CP Crash\n", __func__, n);
			panic("CP Crash");
			break;

		case 3:
			pr_info("%s: [%ld] S/W Watchdog\n", __func__, n);
			local_irq_disable();
			irqs_disabled();
...
#ifdef CONFIG_TIZEN_SEC_KERNEL_ENG
static struct kobj_attribute ess_force_panic_attr =
        __ATTR(force_panic, 0644, ess_force_panic_show, ess_force_panic_store);
#endif

...
static int exynos_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	if(strstr(pdev->name, "mif") != NULL) {
		*target_freq = max((unsigned long)pm_qos_request(PM_QOS_BUS_THROUGHPUT), *target_freq);
		*target_freq = min((unsigned long)pm_qos_request(PM_QOS_BUS_THROUGHPUT_MAX), *target_freq);
	} else if (strstr(pdev->name, "int") != NULL) {
		*target_freq = max((unsigned long)pm_qos_request(PM_QOS_DEVICE_THROUGHPUT), *target_freq);
	#ifdef CONFIG_ARM_EXYNOS_DEVFREQ_DEBUG
		*target_freq = min((unsigned long)pm_qos_request(PM_QOS_DEVICE_THROUGHPUT_MAX), *target_freq);
	#endif
	}
...
out:
	if (strstr(pdev->name, "mif") != NULL)
		store_external_load_factor(MIF_BUS_FREQ, data->new_freq);
	else if (strstr(pdev->name, "int") != NULL)
		store_external_load_factor(INT_BUS_FREQ, data->new_freq);
...
static int exynos_devfreq_suspend(struct device *dev)
{
	...
	pr_info("%s: %s: suspend_freq: %ld\n", __func__,
			dev_name(dev), data->devfreq_profile.suspend_freq);

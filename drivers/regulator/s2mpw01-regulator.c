...
static int s2mpw01_pmic_suspend(struct device *dev)
{
...
#ifdef CONFIG_POP
	s2mpw01_write_reg(s2mpw01->i2c, S2MPW01_PMIC_REG_B3CTRL2, 0x38); /* 1.30V */
#else
	...
	/* W/A for 0x46[1:0] = 00 */
	s2mpw01_read_reg(s2mpw01->i2c, 0x46, &val);
	temp = val & 0x03;
	val &= ~0x03;
	if(temp != 0x00) {
#ifdef CONFIG_SOLIS_LTE_KOR
		panic("0x46[1:0] != 00 !! \n");
#else
		s2mpw01_write_reg(s2mpw01->i2c, 0x46, val);
		dev_err(dev, "0x46[1:0] != 00 !! \n");
#endif
	}
	return 0;
}

...
#define REG_MMU_CAPA_V7		0x870 /* sysmmu v7 only */
#define REG_PUBLIC_WAY_CFG	0x120 /* sysmmu v7 only */
#define REG_PRIVATE_WAY_CFG(n)	(0x200 + ((n) * 0x10)) /* sysmmu v7 only */
#define REG_PRIVATE_ID(n)	(0x20C + ((n) * 0x10)) /* sysmmu v7 only */
#define REG_FAULT_ADDR		0x070 /* sysmmu v7 only */
#define REG_FAULT_TRANS_INFO	0x078 /* sysmmu v7 only */
#define REG_SA_TLB_READ		0x1000 /* sysmmu v7 only */
#define REG_SA_TLB_VPN		0x1004 /* sysmmu v7 only */
#define REG_SA_TLB_PPN		0x1008 /* sysmmu v7 only */
#define REG_SA_TLB_ATTR		0x100C /* sysmmu v7 only */
#define REG_SBB_READ		0x1100 /* sysmmu v7 only */
#define REG_SBB_VPN		0x1104 /* sysmmu v7 only */
#define REG_SBB_LINK		0x1108 /* sysmmu v7 only */
#define REG_SBB_ATTR		0x110C /* sysmmu v7 only */
...
#define MMU_CAPA_MAX_PTW_MO(reg)	((reg >> 26) & 0x3F)
#define MMU_CAPA_MAX_AT_MO(reg)		((reg >> 20) & 0x3F)
#define MMU_CAPA_NUM_CONTEXT(reg)	((reg >> 16) & 0xF)
...
#define MMU_WAY_CFG_AXID_MATCHING	(1 << 4)
#define MMU_WAY_CFG_MASK_PREFETCH_DIR	(3 << 2)
#define MMU_WAY_CFG_PRIVATE_ENABLE	(1 << 0)
#define MMU_WAY_CFG_MASK_TARGET_CH	(3 << 8)
#define MMU_WAY_CFG_TARGET_CH_READ	(1 << 8)
#define MMU_WAY_CFG_TARGET_CH_WRITE	(2 << 8)
#define MMU_WAY_CFG_MASK_TARGET_ID	(0xFFFF << 16)
#define MMU_AW_TRANS_INFO(reg)		((reg >> 20) & 0x1)
#define MMU_SBB_ENTRY_VALID(reg)	((reg >> 28) & 0x1)
#define MMU_DEFAULT_BURST_LENGTH	SYSMMU_BL4
#define MMU_READ_CH			0x1
#define MMU_WRITE_CH			0x2
...
static void dump_l2_tlb(void __iomem *sfrbase, u32 tlb_entry_count)
{
...
static phys_addr_t __get_sfrbase_to_phys(void __iomem *sfrbase)
{
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
...
static void dump_set_associative_tlb(void __iomem *sfrbase, int i, int j)
{
	if (MMU_TLB_ENTRY_VALID(__raw_readl(sfrbase + REG_SA_TLB_ATTR))) {
		pr_crit("[%01d][%01d] VPN: %#010x, PPN: %#010x, ATTR: %#010x\n",
			i, j, __raw_readl(sfrbase + REG_SA_TLB_VPN),
			__raw_readl(sfrbase + REG_SA_TLB_PPN),
			__raw_readl(sfrbase + REG_SA_TLB_ATTR));
	}
}

#define MMU_NUM_TLB_SUBLINE		4

static void dump_sysmmu_tlb_v7(void __iomem *sfrbase)
{
	int i, j, k;
	u32 capa = __raw_readl(sfrbase + REG_MMU_CAPA_V7);

	pr_crit("------------- TLB[WAY][SET][ENTRY] -------------\n");
	for (i = 0; i < MMU_CAPA_NUM_TLB_WAY(capa); i++) {
		for (j = 0; j < (1 << MMU_CAPA_NUM_TLB_SET(capa)); j++) {
			for (k = 0; k < MMU_NUM_TLB_SUBLINE; k++) {
				__raw_writel(MMU_SET_TLB_READ_ENTRY(j, i, k),
					sfrbase + REG_SA_TLB_READ);
				dump_set_associative_tlb(sfrbase, i, j);
			}
		}
	}

	pr_crit("--- SBB(Second-Level Page Table Base Address Buffer ---\n");
	for (i = 0; i < (1 << MMU_CAPA_NUM_SBB_ENTRY(capa)); i++) {
				   ...
				   int flags, unsigned long fault_addr)
{
	...
	if (MMU_MAJ_VER(drvdata->hw_ver) >= 7) {
		info = __raw_readl(sfrbase + REG_FAULT_TRANS_INFO);
		is_write_fault = MMU_AW_TRANS_INFO(info);
	} else {
		is_write_fault = (flags & IOMMU_FAULT_WRITE);
		info = __raw_readl(sfrbase + (is_write_fault ?
	...
	phys = __get_sfrbase_to_phys(sfrbase);
	if (!phys)
		return;
...
irqreturn_t exynos_sysmmu_irq_secure(int irq, void *dev_id)
{
	...
	if (!drvdata->securebase) {
		...
		dev_err(drvdata->sysmmu, "Secure reg value = %#lx\n",
...
static void __sysmmu_set_public_way(struct sysmmu_drvdata *data)
{
	u32 cfg = __raw_readl(data->sfrbase + REG_PUBLIC_WAY_CFG);
	cfg &= ~(MMU_WAY_CFG_MASK_FETCH_SIZE | MMU_WAY_CFG_MASK_PREFETCH);
	cfg |= MMU_DEFAULT_BURST_LENGTH;

	__raw_writel(cfg, data->sfrbase + REG_PUBLIC_WAY_CFG);
}

static void __sysmmu_set_private_way(struct sysmmu_drvdata *data, unsigned way_idx)
{
	u32 cfg = __raw_readl(data->sfrbase + REG_PRIVATE_WAY_CFG(way_idx));

	cfg &= ~(MMU_WAY_CFG_MASK_FETCH_SIZE | MMU_WAY_CFG_MASK_PREFETCH_DIR |
			MMU_WAY_CFG_MASK_TARGET_CH | MMU_WAY_CFG_MASK_PREFETCH);

	/*
	 * In sysmmu v7, all sysmmu use ID matching
	 */
	cfg |= (MMU_WAY_CFG_AXID_MATCHING | MMU_WAY_CFG_PRIVATE_ENABLE |
			data->tlbprops[way_idx].attr);
	cfg |= (is_ar_axi_id(data->tlbprops[way_idx].axid)) ?
		MMU_WAY_CFG_TARGET_CH_READ : MMU_WAY_CFG_TARGET_CH_WRITE;

	__raw_writel(cfg, data->sfrbase + REG_PRIVATE_WAY_CFG(way_idx));
	__raw_writel(MMU_WAY_CFG_MASK_TARGET_ID | (data->tlbprops[way_idx].axid & 0xFF),
			data->sfrbase + REG_PRIVATE_ID(way_idx));

	dev_dbg(data->sysmmu, "way_cfg[%d] : %#x, id[%d] : %#x\n",
		way_idx, __raw_readl(data->sfrbase + REG_PRIVATE_WAY_CFG(way_idx)),
		way_idx, __raw_readl(data->sfrbase + REG_PRIVATE_ID(way_idx)));
}
...
void __sysmmu_init_config(struct sysmmu_drvdata *drvdata)
{
	...
	if (MMU_MAJ_VER(drvdata->hw_ver) >= 7) {
		...
		WARN_ON(tlb_way_num < drvdata->props_num);

		if (tlb_way_num > drvdata->props_num)
			__sysmmu_set_public_way(drvdata);

		for (i = 0; i < min(drvdata->props_num, tlb_way_num); i++)
			__sysmmu_set_private_way(drvdata, i);
	} else {
		if (has_sysmmu_capable_pbuf(drvdata->sfrbase))
			__exynos_sysmmu_set_prefbuf_axi_id(drvdata, NULL,
					0, 0, NULL, NULL);
...
static int __sysmmu_init_pb_info(struct device *sysmmu, struct device *master,
...
static int __sysmmu_update_owner(struct device *master, struct device *sysmmu)
{
	struct exynos_iommu_owner *owner;
...
static int __init __sysmmu_init_master_info_v7(struct device *sysmmu,
					struct sysmmu_drvdata *data)
{
	int props_num, ret, i = 0;
	struct device_node *np;
	struct _tlbprops *props;
	const char *props_name = "sysmmu,v7tlb_property";

	props_num = of_property_count_u32_elems(sysmmu->of_node, props_name);
	if (props_num < 0) {
		dev_err(sysmmu, "failed to count the number of properties\n");
		return props_num;
	}

	props_num /= 2;
	data->props_num = props_num;

	props = kzalloc(sizeof(struct _tlbprops) * props_num, GFP_KERNEL);
	if (!props) {
		dev_err(sysmmu, "failed to allocate memory for properties\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < props_num; i++) {
		ret = of_property_read_u32_index(sysmmu->of_node,
			props_name, i * 2, &data->tlbprops[i].axid);
		if (ret)
			goto err_prop;

		ret = of_property_read_u32_index(sysmmu->of_node,
			props_name, (i * 2) + 1, &data->tlbprops[i].attr);
		if (ret)
			goto err_prop;
	}

	i = 0;
	while ((np = of_parse_phandle(sysmmu->of_node, "master_ip_list", i++))) {
		struct platform_device *master;
		master = of_find_device_by_node(np);
		if (!master) {
			dev_err(sysmmu, "master IP in '%s' not found\n",
					 np->name);
			of_node_put(np);
			goto err_prop;
		}
		...
		ret = __sysmmu_update_owner(&master->dev, sysmmu);
		if (ret) {
			dev_err(sysmmu, "failed to update iommu owner '%s'\n",
					dev_name(&master->dev));
			goto err_prop;
		}
	}
...
static int __init __sysmmu_init_master_info_v6(struct device *sysmmu,
					struct sysmmu_drvdata *data)
{
				     ...
				     struct sysmmu_drvdata *drvdata)
{
	...
	ret = of_property_read_u32(sysmmu->of_node,
				"sysmmu,secure_reg", &secure_reg);
	if (!ret) {
		drvdata->securebase = secure_reg;
		dev_info(sysmmu, "Secure reg = %#x\n", drvdata->securebase);
	} else {
				     ...
				     struct sysmmu_drvdata *drvdata)
{
	...
	if (of_find_property(sysmmu->of_node, "sysmmu,handle_secure-irq", NULL)) {
				...
				struct sysmmu_drvdata *drvdata)
{
	...
	if (MMU_MAJ_VER(drvdata->hw_ver) >= 7)
		ret = __sysmmu_init_master_info_v7(sysmmu, drvdata);
	else
		ret = __sysmmu_init_master_info_v6(sysmmu, drvdata);
...
static int __init exynos_sysmmu_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(dev, "failed to enable local power(%d)\n", ret);
		return ret;
	}

	data->hw_ver = MMU_RAW_VER(__raw_readl(data->sfrbase +
				REG_MMU_VERSION));

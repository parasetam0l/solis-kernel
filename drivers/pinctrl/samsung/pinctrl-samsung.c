...
#ifdef CONFIG_BACKKEY_SECURE_GPIO
#include <linux/smc.h>
#endif
...
/* block the pin control for eSE port based GPIO bank */
static inline int samsung_pinctrl_request_is_ese_secure_bank(char *bank_name)
{
	return !strncmp(bank_name, CONFIG_ESE_SECURE_GPIO, 4);
}
#endif
					...
					unsigned group, bool enable)
{
	...
	pin_to_reg_bank(drvdata, grp->pins[0] - drvdata->ctrl->base,
			&reg, &pin_offset, &bank);
#if defined(CONFIG_ESE_SECURE_ENABLE)
	if (samsung_pinctrl_request_is_ese_secure_bank(bank->name))
		return;
#endif
...
#ifdef CONFIG_BACKKEY_SECURE_GPIO
	if (exynos_smc(0x83000083, 0, 0, 0))
		pr_info("HB: resume failed\n");
	else
		pr_info("HB: resume success\n");
#endif
}
...
static u16 gpiodvs_combine_data(u32 *data, unsigned char phonestate)
{
		...
		int alive, char *bank_name, int pin_num, u32 *data)
{
	...
	if (status == PHONE_INIT || alive == 1) {
		if (alive == 1)
			len = snprintf(buf, sizeof(buf) - len, "gpio sleep-state: %s-%d ", bank_name, pin_num);
		else
			len = snprintf(buf, sizeof(buf) - len, "gpio initial-state: %s-%d ", bank_name, pin_num);
		...
		switch (data[PINCFG_TYPE_FUNC]) {
			...
			case 0x1:   /* output */
				...
				break;
			case 0xf:   /* eint */
				len += snprintf(&buf[len], sizeof(buf) - len, "INT");
				break;
			default:    /* func */
				len += snprintf(&buf[len], sizeof(buf) - len, "FUNC");
				break;
		}

		switch (data[PINCFG_TYPE_PUD]) {
			case 0x0:
				len += snprintf(&buf[len], sizeof(buf) - len, "/NP");
				break;
			case 0x1:
				len += snprintf(&buf[len], sizeof(buf) - len, "/PD");
				break;
			case 0x2:
				len += snprintf(&buf[len], sizeof(buf) - len, "/PU");
				break;
			default:
				len += snprintf(&buf[len], sizeof(buf) - len, "/UN");   /* unknown */
				break;
		}

		switch (data[PINCFG_TYPE_DAT]) {
			case 0x0:
				len += snprintf(&buf[len], sizeof(buf) - len, "/L");
				break;
			case 0x1:
				len += snprintf(&buf[len], sizeof(buf) - len, "/H");
				break;
			default:
				len += snprintf(&buf[len], sizeof(buf) - len, "/U");    /* unknown */
				break;
		}
	} else {
		// PHONE_SLEEP
		...
		switch (data[PINCFG_TYPE_CON_PDN]) {
			...
			case 0x2:   /* input */
				...
				break;
			case 0x3:   /* previous state */
				len += snprintf(&buf[len], sizeof(buf) - len, "PREV");
				pin_dat = data[PINCFG_TYPE_DAT];
				break;
			default:    /* func */
		...
		unsigned pin)
{
	...
	if ((!strncmp(bank->name, "gpc0", 4) && pin_num == 0) ||
		(!strncmp(bank->name, "gpc1", 4) && pin_num == 0) ||
		(!strncmp(bank->name, "gpc1", 4) && pin_num == 1) ||
		(!strncmp(bank->name, "gpe0", 4) && pin_num == 1))
		goto out;

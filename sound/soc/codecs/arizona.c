...
#if defined(CONFIG_SOLIS) || defined(CONFIG_SOLIS_LTE)
int arizona_set_tristate_direct(struct snd_soc_codec *codec,
					int aifnum, int tristate)
{
	int reg;
	int mask;
	int val;
	...
	switch (aifnum) {
	case 1:
		reg = ARIZONA_AIF1_RATE_CTRL;
		mask = ARIZONA_AIF1_TRI_MASK;
		if (tristate)
			val = ARIZONA_AIF1_TRI;
		else
			val = 0;
		break;
	case 2:
		reg = ARIZONA_AIF2_RATE_CTRL;
		mask = ARIZONA_AIF2_TRI_MASK;
		if (tristate)
			val = ARIZONA_AIF2_TRI;
		else
			val = 0;
		break;
	case 3:
		reg = ARIZONA_AIF3_RATE_CTRL;
		mask = ARIZONA_AIF3_TRI_MASK;
		if (tristate)
			val = ARIZONA_AIF3_TRI;
		else
			val = 0;
		break;
	case 4:
		reg = ARIZONA_AIF4_RATE_CTRL;
		mask = ARIZONA_AIF4_TRI_MASK;
		if (tristate)
			val = ARIZONA_AIF4_TRI;
		else
			val = 0;
		break;
	default:
	...
	dev_info(codec->dev, "set tristate aif%d : 0x%d, 0x%d, 0x%d\n",
						aifnum, reg, mask, val);
	return snd_soc_update_bits(codec, reg, mask, val);
}
EXPORT_SYMBOL_GPL(arizona_set_tristate_direct);
#endif

...
#if IS_ENABLED(CONFIG_SND_SOC_LARGO)
enum SCENARIO_TYPE {
	ST_NONE = 0,
	ST_WB,
	ST_NB,
...
#define REG_SCO 1
static int wm_adsp_scenario_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(codec);
	...
	/* support DSP2 & DSP3 only on largo */
	if (e->shift_l < 1 || e->shift_l > 2) {
		dev_err(codec->dev,
			"%s: unsupported DSP%d\n", __func__, e->shift_l + 1);
		return -EINVAL;
	}
	switch (e->reg) {
	case REG_NETWORK:
		value = dsp[e->shift_l].network_type;
		break;
	case REG_SCO:
		value = dsp[e->shift_l].sco_type;
		break;
	default:
		dev_err(codec->dev, "%s: invalid reg %d\n", __func__, e->reg);
		return -EINVAL;
	}
	if (value < ST_NONE || value > ST_LOOPBACK) {
		dev_err(codec->dev, "%s: invalid value %d\n", __func__, value);
		return -EINVAL;
	}
	dev_dbg(codec->dev, "%s: DSP%d - %s\n",
				__func__, e->shift_l + 1, e->texts[value]);
	ucontrol->value.integer.value[0] = value;

	return 0;
}
static int wm_adsp_scenario_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(codec);
	...
	if (value < ST_NONE || value > ST_LOOPBACK) {
		dev_err(codec->dev, "%s: invalud value %d\n", __func__, value);
		return -EINVAL;
	}
	switch (e->reg) {
	case REG_NETWORK:
		dsp[e->shift_l].network_type = value;
		break;
	case REG_SCO:
		dsp[e->shift_l].sco_type = value;
		break;
	default:
		dev_err(codec->dev, "%s: invalid reg %d\n", __func__, e->reg);
		return -EINVAL;
	...
	dev_info(codec->dev, "%s: DSP%d - %s\n",
				__func__, e->shift_l + 1, e->texts[value]);
	return 1;
}
static const char *const wm_adsp_network_text[] = {
	"None", "NW_WB", "NW_NB", "LOOPBACK",
};
static const char *const wm_adsp_sco_text[] = {
	"None", "SCO_WB", "SCO_NB",
};
static const struct soc_enum wm_adsp_scenario_enum[] = {
	SOC_ENUM_SINGLE(REG_NETWORK,	1, 4, wm_adsp_network_text),
	SOC_ENUM_SINGLE(REG_SCO,	1, 3, wm_adsp_sco_text),
	SOC_ENUM_SINGLE(REG_NETWORK,	2, 4, wm_adsp_network_text),
	SOC_ENUM_SINGLE(REG_SCO,	2, 3, wm_adsp_sco_text),
};
const struct snd_kcontrol_new wm_adsp_scenario_controls[] = {
	SOC_ENUM_EXT("DSP2 Network Type", wm_adsp_scenario_enum[0],
		wm_adsp_scenario_get, wm_adsp_scenario_put),
	SOC_ENUM_EXT("DSP2 SCO Type", wm_adsp_scenario_enum[1],
		wm_adsp_scenario_get, wm_adsp_scenario_put),
	SOC_ENUM_EXT("DSP3 Network Type", wm_adsp_scenario_enum[2],
		wm_adsp_scenario_get, wm_adsp_scenario_put),
	SOC_ENUM_EXT("DSP3 SCO Type", wm_adsp_scenario_enum[3],
		wm_adsp_scenario_get, wm_adsp_scenario_put),
};
EXPORT_SYMBOL_GPL(wm_adsp_scenario_controls);
#endif
...
static int wm_adsp_load_coeff(struct wm_adsp *dsp)
{
...
#if IS_ENABLED(CONFIG_SND_SOC_LARGO)
	if (dsp->firmwares[dsp->fw].binfile)
		snprintf(file, PAGE_SIZE, "%s-dsp%d-%s", dsp->part,
			 dsp->num, dsp->firmwares[dsp->fw].binfile);
	else
		snprintf(file, PAGE_SIZE, "%s-dsp%d-%s", dsp->part,
			 dsp->num, dsp->firmwares[dsp->fw].file);

	if (dsp->network_type && dsp->sco_type)
		snprintf(postfix, sizeof(postfix) - 1, "-%s-%s.bin",
				wm_adsp_network_text[dsp->network_type],
				wm_adsp_sco_text[dsp->sco_type]);
	else if (dsp->network_type && !dsp->sco_type)
		snprintf(postfix, sizeof(postfix) - 1, "-%s.bin",
				wm_adsp_network_text[dsp->network_type]);
	else if (!dsp->network_type && dsp->sco_type)
		snprintf(postfix, sizeof(postfix) - 1, "-%s.bin",
				wm_adsp_sco_text[dsp->sco_type]);
	else
		snprintf(postfix, sizeof(postfix) - 1, ".bin");
	...
	if (ret != 0) {
		...
		adsp_info(dsp, "%s file has loaded at 0x%p, %zu bytes\n",
				file, firmware->data, firmware->size);
	}
	ret = -EINVAL;
	...
	blocks = 0;
	sum_coeff_len += le32_to_cpu(hdr->len);
	while (pos < firmware->size &&
		...
		if (reg) {
			sum_coeff_len += le32_to_cpu(blk->len);
			if (sum_coeff_len > firmware->size) {
				adsp_err(dsp,
					"abnormal size %s.%d (%d,%zu)\n",
						file, blocks, sum_coeff_len,
						firmware->size);
				WARN_ON(1);
				ret = -EFAULT;
				goto out_fw;
			}
			adsp_dbg(dsp, "%s.%d: %d, %zu\n", file, blocks,
						sum_coeff_len, firmware->size);

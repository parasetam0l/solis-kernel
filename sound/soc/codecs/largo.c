...
SOC_SINGLE("IN1L Mute", ARIZONA_ADC_DIGITAL_VOLUME_1L,
		ARIZONA_IN1L_MUTE_SHIFT, 1, 0),
SOC_SINGLE("IN1R Mute", ARIZONA_ADC_DIGITAL_VOLUME_1R,
		ARIZONA_IN1R_MUTE_SHIFT, 1, 0),
...
int largo_get_adsp_status(struct snd_soc_codec *codec, int dsp_num)
{
	...
	case 1: /* DSP2 */
	case 2: /* DSP3 */
		if (largo->core.adsp[dsp_num].running)
			return 1;
		else
			return 0;
	default:
...
EXPORT_SYMBOL_GPL(largo_get_adsp_status);
#endif
...
static int largo_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_add_codec_controls(codec, wm_adsp_scenario_controls, 4);
	if (ret != 0)
		return ret;

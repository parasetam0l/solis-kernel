...
static int audmixer_startup(struct snd_pcm_substream *stream, struct snd_soc_dai *dai)
{
	...
	dev_dbg(g_audmixer->dev, "aif%d:(%s) %s called\n",
			interface, stream->stream ? "C" : "P", __func__);
...
#endif
		dev_info(g_audmixer->dev, "cp dmix path disabled\n");
	}
}
				...
				unsigned long action, void *data)
{
	...
	dev_info(g_audmixer->dev, "%s called, event = %ld\n", __func__,
							action);

	if (!is_cp_aud_enabled()) {
		dev_info(g_audmixer->dev, "Mixer not active, Exiting..\n");
		return 0;
	}

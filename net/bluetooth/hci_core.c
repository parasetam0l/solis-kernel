...
int hci_inquiry(void __user *arg)
{
	...
	if (do_inquiry) {
		...
		if (wait_on_bit(&hdev->flags, HCI_INQUIRY, wait_inquiry,
			TASK_INTERRUPTIBLE))
#else
		/* Signature of the function "wait_on_bit" is changed in latest kernel
		 * So, if kernel is migrated to latest, then below code should be enabled
		 */
...
int bt_get_sleep_monitor_cb(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	int state = 0;
	struct hci_dev *hdev = (struct hci_dev*)priv;

	struct _sleep_pkt sleep_pkt = {0, };
	struct hci_conn *conn = NULL;
	int idx = 0;

	if (hci_dev_test_flag(hdev, HCI_SC_ONLY))
		sleep_pkt.common |= 0x01;

	if (hci_dev_test_flag(hdev, HCI_ADVERTISING))
		sleep_pkt.common |= 0x02;

	if (hci_dev_test_flag(hdev,HCI_CONNECTABLE))
		sleep_pkt.common |= 0x04;

	if (hci_dev_test_flag(hdev,HCI_DISCOVERABLE))
		sleep_pkt.common |= 0x08;

	if (test_bit(HCI_ISCAN, &hdev->flags))
		sleep_pkt.common |= 0x10;

	if (test_bit(HCI_PSCAN, &hdev->flags))
		sleep_pkt.common |= 0x20;
	...
	list_for_each_entry(conn, &hdev->conn_hash.list, list) {
		...
		sleep_pkt.device[idx] |= (0x7f & conn->handle);
		if (conn->mode != HCI_CM_SNIFF)
			sleep_pkt.device[idx] |= 0x80;
...
max_device:
	memcpy(raw_val, &sleep_pkt, sizeof(unsigned int));
	...
	BT_DBG("%s: check_level[%d], state[%d], raw[%X]\n", __func__,
			check_level, state, *raw_val);

	return state;
}

static struct sleep_monitor_ops bt_sleep_monitor_ops = {
	.read_cb_func = bt_get_sleep_monitor_cb,
};
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(hdev, &bt_sleep_monitor_ops,
			SLEEP_MONITOR_BT);
#endif
#ifdef CONFIG_TIZEN_WIP
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_unregister_ops(SLEEP_MONITOR_BT);
#endif
#ifdef CONFIG_TIZEN_WIP
